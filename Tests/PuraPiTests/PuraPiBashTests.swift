...
@MainActor
final class PuraPiBashTests: XCTestCase {
    // MARK: - 协议层
    ...
    func testShellPrefixDetection() {
        XCTAssertEqual(PuraPiCommandCatalog.shellCommand(for: "!ls -la"), "ls -la")
        XCTAssertEqual(PuraPiCommandCatalog.shellCommand(for: "  ! git status "), "git status")
        // 只有 `!` 没有命令时不触发。
        XCTAssertNil(PuraPiCommandCatalog.shellCommand(for: "!"))
        XCTAssertNil(PuraPiCommandCatalog.shellCommand(for: "/compact"))
        XCTAssertNil(PuraPiCommandCatalog.shellCommand(for: "解释一下 ! 的用法"))
    }
    ...
    func testFullWidthExclamationIsRecognized() {
        XCTAssertEqual(PuraPiCommandCatalog.shellCommand(for: "！ls"), "ls")
        XCTAssertEqual(PuraPiCommandCatalog.shellCommand(for: "！ ls -a"), "ls -a")
        XCTAssertNil(PuraPiCommandCatalog.shellCommand(for: "！"))
    }

    /// 命令模式判定：只输前缀时也要成立，界面才能立刻反馈。
    func testShellCommandModeDetection() {
        XCTAssertTrue(PuraPiCommandCatalog.isShellCommandMode("!"))
        XCTAssertTrue(PuraPiCommandCatalog.isShellCommandMode("！"))
        XCTAssertTrue(PuraPiCommandCatalog.isShellCommandMode("!ls"))
        XCTAssertFalse(PuraPiCommandCatalog.isShellCommandMode(""))
        XCTAssertFalse(PuraPiCommandCatalog.isShellCommandMode("/compact"))
        XCTAssertFalse(PuraPiCommandCatalog.isShellCommandMode("你好"))
    }
