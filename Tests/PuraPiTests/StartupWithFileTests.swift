...
final class StartupWithFileTests: XCTestCase {
    ...
    override func setUp() {
        ...
        for k in [PuraPiLayoutState.sidebarWidthKey,
                  PuraPiLayoutState.inspectorWidthKey,
                  PuraPiLayoutState.sidebarVisibleKey] {
    ...
    func testLayoutIsCorrectWhenFileSelectedAtStartup() {
        ...
        XCTAssertEqual(
            widths[0] - PuraPiLayoutState.sidebarInset * 2, 290, accuracy: 4,
            "Sidebar 应为持久化宽度，实测栏位 \(widths[0])")
        // 右栏宽度应来自持久化的 410。
        XCTAssertEqual(
            widths[2] - PuraPiLayoutState.inspectorInset * 2, 410, accuracy: 4,
