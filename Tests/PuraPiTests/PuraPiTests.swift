...
@MainActor
final class PuraPiTests: XCTestCase {
    func testProjectTabsDeduplicateAndSelectNeighborAfterClose() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-tabs-\(UUID().uuidString)", isDirectory: true)
        ...
        let manager = PuraPiTabManager(makeTransport: { collector.make() })
    ...
    func testContinueRecentSessionLoadsHistoryAndSettlesAfterMessages() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-continue-\(UUID().uuidString)", isDirectory: true)
    ...
    func testContinueRecentSessionWithEmptyHistoryHidesEntryState() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-continue-empty-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testRuntimeTerminationCleansPendingStateAndRetainsUserInput() async throws {
        ...
        session.queuedPrompts = [
            PuraPiQueuedPrompt(text: "排队任务", attachments: session.pendingAttachments)
        ]
    ...
    private func makeSession() throws -> (PiSessionController, FakePiRPCTransport, URL) {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-session-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
    ...
    private func waitUntil(
        ...
        XCTFail("Timed out waiting for PuraPi state")
    }
