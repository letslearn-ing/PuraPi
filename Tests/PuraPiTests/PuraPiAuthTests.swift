...
final class PuraPiAuthTests: XCTestCase {
    ...
    func testLocatorFindsOfficialSDKForInstalledPiWhenAvailable() throws {
        ...
        let result = PuraPiAuthBridgeLocator.locate()
    ...
    func testLocatorHonorsConfiguredAgentDirectoryWithoutPassingNodeInjection() throws {
        ...
        environment["PI_CODING_AGENT_DIR"] = "/tmp/purapi-auth-agent"
        ...
        let result = PuraPiAuthBridgeLocator.locate(environment: environment)
        guard case .success(let configuration) = result else {
            XCTFail("无法定位认证桥接配置：\(result)")
            return
        }
        XCTAssertEqual(configuration.authPath.path, "/tmp/purapi-auth-agent/auth.json")
        XCTAssertNil(configuration.environment["NODE_OPTIONS"])
    ...
    func testRuntimeErrorSanitizerHidesStructuredAndBearerCredentials() {
        ...
        let sanitized = PuraPiSensitiveText.redacted(message)
    ...
    func testAuthenticationFailureFreezesSteeringAndOffersReconnectWhenIdle() {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("purapi-auth-gate-\(UUID().uuidString)")
    ...
    func testAuthStoragePermissionsAreTightenedWithoutFollowingSymlink() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("purapi-auth-permissions-\(UUID().uuidString)", isDirectory: true)
        ...
        try PuraPiAuthStorageSecurity.ensureUserOnlyPermissions(at: authURL)
        XCTAssertEqual(
        ...
        XCTAssertThrowsError(
            try PuraPiAuthStorageSecurity.ensureUserOnlyPermissions(at: linkURL)
        )
    }

    func testNodeValidatorFallsBackToCompatibleCandidate() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("purapi-auth-node-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let configuration = PuraPiAuthBridgeConfiguration(
            nodeURL: oldNode,
        ...
        let validated = try await PuraPiAuthBridgeNodeValidator.validatedConfiguration(
            configuration,
            runner: runner
        )
    ...
    func testAuthenticationChangeAllowsReconnectOnlyWhenRuntimeIsIdle() {
        let session = PiSessionController()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("purapi-auth-session-\(UUID().uuidString)")
        session.workspace = WorkspaceDescriptor(rootURL: root)
        session.runtimeReady = true
        session.phase = .idle
    ...
    func testCoordinatorCompletesRealSidecarAPIKeyFlowInOptInIntegration() async throws {
        guard ProcessInfo.processInfo.environment["PURAPI_AUTH_BRIDGE_TEST"] == "1" else {
            throw XCTSkip("设置 PURAPI_AUTH_BRIDGE_TEST=1 才运行真实 Pi SDK 认证桥接测试")
        }
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("purapi-auth-coordinator-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        guard case .success(let discovered) = PuraPiAuthBridgeLocator.locate() else {
        ...
        let coordinator = PuraPiAuthCoordinator(configurationOverride: configuration)
        coordinator.refreshStatus()
    ...
    func testDefaultBridgeCanReadAndStoreAPIKeyInOptInIntegration() async throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("purapi-auth-bridge-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        guard FileManager.default.isReadableFile(atPath: packageEntry.path),
              PuraPiAuthBridgeResources.scriptURL != nil,
              let nodeURL = PuraPiAuthTestSupport.findNode()
        else {
            throw XCTSkip("当前机器没有可用的 Pi SDK/Node 测试安装")
        }
        ...
        let client = PuraPiDefaultAuthBridgeClient()
        let status = try await client.perform(
            configuration: configuration,
            request: .status(id: UUID().uuidString),
            onPrompt: { _ in throw PuraPiAuthError.cancelled },
            onEvent: { _ in }
        )
    ...
    private func makeConfiguration() -> PuraPiAuthBridgeConfiguration {
        PuraPiAuthBridgeConfiguration(
...
private final class FakeAuthBridgeClient: PuraPiAuthBridgeClient, @unchecked Sendable {
    private let sendEvent: Bool
    private let failure: PuraPiAuthError?
    ...
    func perform(
        ...
        if request.type == "login" {
            ...
            let prompt = PuraPiAuthPrompt(
                ...
                options: request.authType == .oauth
                    ? [PuraPiAuthPrompt.Option(id: "browser", label: "Browser", description: nil)]
                    : []
            )
        ...
        let configured = request.type == "login"
            ? PuraPiAuthStatus(configured: true, type: request.authType, source: "stored", subscription: request.authType == .oauth)
            : .unconfigured
        let provider = PuraPiAuthProvider(
            id: "anthropic",
            name: "Anthropic",
            authTypes: [
                PuraPiAuthMethod(type: .oauth, name: "Claude OAuth", isSubscription: true, loginLabel: nil, canLogin: true),
                PuraPiAuthMethod(type: .apiKey, name: "Anthropic API key", isSubscription: false, loginLabel: nil, canLogin: true),
            ],
            status: configured
        )
        var credentials: [PuraPiStoredCredential] = request.type == "login"
            ? [PuraPiStoredCredential(providerId: "anthropic", type: request.authType ?? .apiKey)]
            : []
        ...
        if shouldIncludeOrphan {
            credentials.append(PuraPiStoredCredential(providerId: "legacy-provider", type: .apiKey))
        }
        let snapshot = PuraPiAuthSnapshot(
            providers: [provider],
            credentials: credentials,
            models: [PuraPiAuthModel(
...
private final class ScriptedAuthProcessRunner: PuraPiProcessRunner, @unchecked Sendable {
...
private enum PuraPiAuthTestSupport {
