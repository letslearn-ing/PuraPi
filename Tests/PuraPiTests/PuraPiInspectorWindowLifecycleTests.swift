...
@MainActor
final class PuraPiInspectorWindowLifecycleTests: XCTestCase {
    private struct SplitHost: View {
        ...
        let coordinator: PuraPiInspectorWindowCoordinator
    ...
    func testDetachAndReattachKeepTheSameSessionAndEditorState() {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-inspector-window-")
        ...
        let defaultsName = "PuraPi-inspector-window-state-\(UUID().uuidString)"
        ...
        if let detachedWindow = coordinator.detachedWindows.first {
            ...
            let proxies = descendants(of: detachedContent)
                .compactMap { $0 as? PuraPiInspectorHeaderInteractionView }
            XCTAssertEqual(proxies.count, 1, "独立窗口应有唯一的标题按钮命中代理")
            let detachProxies = descendants(of: detachedContent)
                .compactMap { $0 as? PuraPiInspectorDetachInteractionView }
    ...
    func testDetachedWindowsStayBoundToTheirProjectTab() {
        let firstRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-inspector-tab-one-\(UUID())", isDirectory: true)
        let secondRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-inspector-tab-two-\(UUID())", isDirectory: true)
        ...
        let defaultsName = "PuraPi-inspector-tabs-state-\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: defaultsName)!
        defer { defaults.removePersistentDomain(forName: defaultsName) }
        let coordinator = PuraPiInspectorWindowCoordinator(
            tabs: tabs,
            layoutState: PuraPiLayoutState(),
            appearanceState: PuraPiAppearanceState(defaults: defaults)
        )
        defer {
            coordinator.shutdown()
            tabs.closeAll()
    ...
    func testDetachWithoutASelectedFileDoesNothing() {
        let tabs = PuraPiTabManager(makeTransport: { _ in FakePiRPCTransport() })
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-inspector-empty-\(UUID())", isDirectory: true)
        ...
        let defaultsName = "PuraPi-inspector-empty-state-\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: defaultsName)!
        defer { defaults.removePersistentDomain(forName: defaultsName) }
        let coordinator = PuraPiInspectorWindowCoordinator(
            tabs: tabs,
