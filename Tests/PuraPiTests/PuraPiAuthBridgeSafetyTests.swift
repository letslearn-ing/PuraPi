...
final class PuraPiAuthBridgeSafetyTests: XCTestCase {
    func testStatusDoesNotCreateAuthFileWhenUsingReadOnlyStorage() async throws {
        ...
        let result = try await PuraPiDefaultAuthBridgeClient().perform(
    ...
    func testCommandConfigurationIsRejectedBeforeSDKImport() async throws {
        ...
        writeFileSync(process.env.PURAPI_IMPORT_MARKER, "imported");
        ...
        environment["PURAPI_IMPORT_MARKER"] = importMarker.path
        ...
        do {
            _ = try await PuraPiDefaultAuthBridgeClient().perform(
                configuration: configuration,
                request: .status(id: UUID().uuidString),
                onPrompt: { _ in throw PuraPiAuthError.cancelled },
                onEvent: { _ in }
    ...
    func testModelsSnapshotDoesNotCopyLiteralAPIKeys() async throws {
        ...
        export class ModelRuntime {
          static async create(options) {
            ...
            writeFileSync(process.env.PURAPI_CONFIG_MARKER,
        ...
        let baseConfiguration = try makeConfiguration(root: root, entryURL: entryURL)
        var environment = baseConfiguration.environment
        environment["PURAPI_CONFIG_MARKER"] = markerURL.path
        let configuration = PuraPiAuthBridgeConfiguration(
            nodeURL: baseConfiguration.nodeURL,
            nodeCandidates: baseConfiguration.nodeCandidates,
            entryURL: baseConfiguration.entryURL,
    ...
    func testPinnedModelsSnapshotBlocksLaterCommandInjection() async throws {
        ...
        const official = await import(pathToFileURL(process.env.PURAPI_OFFICIAL_ENTRY).href);
        export class ModelRuntime {
          static async create(options) {
            const runtime = await official.ModelRuntime.create(options);
            writeFileSync(process.env.PURAPI_MODELS_PATH, JSON.stringify({
        ...
        let baseConfiguration = try makeConfiguration(root: root, entryURL: entryURL)
        var environment = baseConfiguration.environment
        environment["PURAPI_OFFICIAL_ENTRY"] = discovered.entryURL.path
        environment["PURAPI_MODELS_PATH"] = modelsURL.path
        let configuration = PuraPiAuthBridgeConfiguration(
            nodeURL: baseConfiguration.nodeURL,
            nodeCandidates: baseConfiguration.nodeCandidates,
            entryURL: baseConfiguration.entryURL,
    ...
    func testCommittedCredentialIsReportedWhenSnapshotFails() async throws {
        ...
        export class ModelRuntime {
          ...
          async login(_provider, _type, interaction) {
            ...
            writeFileSync(process.env.PURAPI_AUTH_PATH, JSON.stringify({ anthropic: { type: "api_key", key } }));
        ...
        let baseConfiguration = try makeConfiguration(root: root, entryURL: entryURL)
        var environment = baseConfiguration.environment
        environment["PURAPI_AUTH_PATH"] = baseConfiguration.authPath.path
        let configuration = PuraPiAuthBridgeConfiguration(
            nodeURL: baseConfiguration.nodeURL,
            nodeCandidates: baseConfiguration.nodeCandidates,
            entryURL: baseConfiguration.entryURL,
    ...
    private func requireAuthBridgeIntegration() throws {
        guard ProcessInfo.processInfo.environment["PURAPI_AUTH_BRIDGE_TEST"] == "1" else {
            throw XCTSkip("设置 PURAPI_AUTH_BRIDGE_TEST=1 才运行认证 sidecar 安全集成测试")
        }
    }

    private func makeTemporaryRoot() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("purapi-auth-safety-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeConfiguration(
        root: URL,
        entryURL: URL? = nil
    ) throws -> PuraPiAuthBridgeConfiguration {
        guard case .success(let discovered) = PuraPiAuthBridgeLocator.locate() else {
            throw XCTSkip("当前机器没有可用的 Pi SDK/Node")
        }
