...
@MainActor
final class PuraPiMarkdownQuoteTests: XCTestCase {
    func testNestedQuoteMarkerConversionPreservesDepth() throws {
        ...
        let converted = try XCTUnwrap(
            PuraPiMarkdownBlockConverter.convert(block: block, displayText: "> > 深层引用")
        )
    ...
    private func makeDocument(_ text: String) throws -> (URL, URL) {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("purapi-markdown-quote-\(UUID().uuidString)", isDirectory: true)
