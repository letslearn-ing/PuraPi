...
@MainActor
final class PuraPiScrollEdgeEffectTests: XCTestCase {
    ...
    func testNativeContainerUsesPublicTopAccessory() {
        ...
        XCTAssertEqual(
            ...
            PuraPiTitlebarTabs.tabBarHeight,
            accuracy: 0.01,
        ...
        XCTAssertTrue(
            descendants(of: controller.view).allSatisfy {
                !($0 is PuraPiScrollEdgeEffectView)
            },
    ...
    func testDetachRestoresScrollNotificationSettings() {
        ...
        let effect = PuraPiScrollEdgeEffectView(frame: .zero)
    ...
    func testAttachmentFindsTheNearestSwiftUIScrollView() throws {
        ...
        let effects = descendants(of: root).compactMap { $0 as? PuraPiScrollEdgeEffectView }
    ...
    ) -> (NSScrollView, PuraPiScrollEdgeEffectView, NSWindow) {
        let scrollView = NSScrollView(
            frame: NSRect(x: 0, y: 0, width: 400, height: 500)
        )
    ...
    private func cleanup(
        ...
        effect: PuraPiScrollEdgeEffectView,
        window: NSWindow
    ) {
        effect.detach()
