...
final class RuntimeProvisioningFakeRunner: PuraPiProcessRunner, @unchecked Sendable {
    private let lock = NSLock()
    private var recordedRequests: [PuraPiProcessRequest] = []
    var requests: [PuraPiProcessRequest] {
        lock.lock()
        defer { lock.unlock() }
    ...
    let extractedNodeVersion = PuraPiRuntimeVersion(major: 22, minor: 23, patch: 0)
    ...
    private func record(_ request: PuraPiProcessRequest) {
        lock.lock()
...
final class RuntimeProvisioningFakeDownloader: PuraPiArtifactDownloader, @unchecked Sendable {
    private let lock = NSLock()
    ...
    func download(url: URL, to destination: URL, maximumBytes: Int) async throws {
        guard let payload = recordAndGetPayload(for: url), payload.count <= maximumBytes else {
            throw PuraPiRuntimeProvisioningError.network("测试下载内容不存在")
        }
...
final class SlowRuntimeProvisioningClient: PuraPiRuntimeProvisioningClient, @unchecked Sendable {
    let installation: PuraPiRuntimeInstallation
    private let lock = NSLock()
    private(set) var installCount = 0

    init(installation: PuraPiRuntimeInstallation) {
        self.installation = installation
    }

    func discover() async -> PuraPiRuntimeDiscoverySnapshot {
        PuraPiRuntimeDiscoverySnapshot(installation: nil, node: .missing, diagnostics: [])
    }
...
final class RuntimeProvisioningFakeClient: PuraPiRuntimeProvisioningClient, @unchecked Sendable {
    let snapshot: PuraPiRuntimeDiscoverySnapshot
    let installation: PuraPiRuntimeInstallation
    ...
    init(
        ...
        installation: PuraPiRuntimeInstallation
    ) {
