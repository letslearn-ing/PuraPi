...
/// 附件输入与单条消息复制。
@MainActor
final class PuraPiAttachmentTests: XCTestCase {
    private var root: URL!

    override func setUpWithError() throws {
        root = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("purapi-attach-\(UUID().uuidString)")
    ...
    func testAttachmentMemoryEstimateIncludesBase64Copy() {
        ...
        let attachment = PuraPiAttachment(
            url: nil,
    ...
    func testWorkspaceParentSymlinkIsRejectedWithoutReadingTarget() throws {
        let outside = root.deletingLastPathComponent()
            .appendingPathComponent("purapi-attach-outside-\(UUID().uuidString)")
    ...
    func testCopyWritesToPasteboard() {
        PuraPiMessageActions.copy("要复制的内容")
        XCTAssertEqual(
    ...
    func testEditPostsNotificationWithText() {
        ...
        PuraPiMessageActions.edit("原来的问题")
    ...
    func testRetryPostsSeparateNotification() {
        ...
        PuraPiMessageActions.retry("再来一次")
        wait(for: [expectation], timeout: 2)
    }
