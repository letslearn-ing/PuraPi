...
@MainActor
final class PuraPiInspectorFloatingSurfaceTests: XCTestCase {
    private final class TintState: ObservableObject {
        @Published var value: PuraPiPaneTint

        init(_ value: PuraPiPaneTint) {
            self.value = value
        }
    }
    ...
    private func makeSession() -> PiSessionController {
        ...
        session.selectedPreview = FilePreview(
            url: URL(fileURLWithPath: "/tmp/purapi-surface.md"),
            relativePath: "purapi-surface.md",
            kind: .markdown,
            text: "# surface",
            byteCount: 9,
    ...
    func testInspectorUsesIsolatedRegularItemAndAlignedSurfaceSlot() {
        ...
        let inset = PuraPiLayoutState.inspectorInset
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            panes[0].frame.width,
            layoutState.sidebarWidth + PuraPiLayoutState.sidebarInset * 2,
            accuracy: 2,
        ...
        XCTAssertEqual(
            inspectorVisualGap,
            PuraPiLayoutState.inspectorInset + split.dividerThickness,
            accuracy: 1,
        ...
        let inspectorHosts = inspectorViews.filter {
            ...
            return name.contains("PuraPiInspectorHostView")
        }
        ...
        if #available(macOS 26.1, *) {
            XCTAssertTrue(
                inspectorViews.allSatisfy { !($0 is PuraPiScrollEdgeEffectView) },
        ...
        XCTAssertEqual(
            PuraPiLayoutState.inspectorContentTopInset,
            0,
            accuracy: 0.01,
