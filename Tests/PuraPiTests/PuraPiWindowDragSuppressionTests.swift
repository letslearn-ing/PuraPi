...
@MainActor
final class PuraPiWindowDragSuppressionTests: XCTestCase {
    ...
    private func findController(_ view: NSView) -> PuraPiWorkspaceSplitViewController? {
        var node: NSResponder? = view
        while let current = node {
            if let c = current as? PuraPiWorkspaceSplitViewController { return c }
    ...
    func testHitRegionCentersOnVisibleRoundedRectEdge() {
        ...
        let visibleEdge = panes[2].frame.minX + PuraPiLayoutState.inspectorInset

        func isHandle(atX x: CGFloat) -> Bool {
            let pt = splitView.convert(NSPoint(x: x, y: splitView.bounds.midY), to: nil)
            return window.contentView?.hitTest(pt) is PuraPiDividerWindowDragBlocker
        }
