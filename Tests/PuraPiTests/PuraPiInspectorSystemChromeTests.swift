...
@MainActor
final class PuraPiInspectorSystemChromeTests: XCTestCase {
    ...
    override func tearDown() {
        let defaults = PuraPiPreferences.shared
        if let savedSidebarWidth { defaults.set(savedSidebarWidth, forKey: PuraPiLayoutState.sidebarWidthKey) }
        else { defaults.removeObject(forKey: PuraPiLayoutState.sidebarWidthKey) }
        if let savedInspectorWidth { defaults.set(savedInspectorWidth, forKey: PuraPiLayoutState.inspectorWidthKey) }
    ...
    private func makeWindow(tintState: TintState) -> (NSWindow, PuraPiLayoutState) {
        let session = PiSessionController()
        session.selectedPreview = FilePreview(
            url: URL(fileURLWithPath: "/tmp/purapi-chrome.md"),
            relativePath: "purapi-chrome.md",
            kind: .markdown,
            text: "# chrome",
            byteCount: 8,
