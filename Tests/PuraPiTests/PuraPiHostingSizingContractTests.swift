...
///
/// 这组用例不含任何 PuraPi 代码，纯粹记录 AppKit/SwiftUI 的互操作行为：
/// 不实现 `sizeThatFits` 时 divider 拖动是坏的（窗口被缩、或压力转移到相邻栏）。
/// 将来 SDK 行为变化时能立刻发现。
@MainActor
final class PuraPiHostingSizingContractTests: XCTestCase {
    ...
    private struct IsolatedRep: NSViewControllerRepresentable {
        func makeNSViewController(context: Context) -> NSViewController {
            let split = PuraPiHostingSizingContractTests.makeStockSplitController()
    ...
    private struct SizedRep: NSViewControllerRepresentable {
        func makeNSViewController(context: Context) -> NSSplitViewController {
            PuraPiHostingSizingContractTests.makeStockSplitController()
        }
        func updateNSViewController(_ c: NSSplitViewController, context: Context) {}
        func sizeThatFits(
    ...
    ///
    /// 这些用例不含任何 PuraPi 代码，纯粹记录 AppKit/SwiftUI 的互操作缺陷，
    /// 以便将来 SDK 行为变化时能立刻发现。
