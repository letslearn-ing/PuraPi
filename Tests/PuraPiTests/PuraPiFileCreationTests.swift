...
@MainActor
final class PuraPiFileCreationTests: XCTestCase {
    private var root: URL!
    private var session: PiSessionController!

    override func setUpWithError() throws {
        root = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("purapi-create-\(UUID().uuidString)")
    ...
    func testCreationRejectsSymlinkedParent() throws {
        let outside = root.deletingLastPathComponent()
            .appendingPathComponent("purapi-create-outside-\(UUID().uuidString)")
