...
@MainActor
final class PuraPiMarkdownCollaborationTests: XCTestCase {
    func testUnapprovedMarkdownChangeIsNotInjectedIntoPrompt() async throws {
        ...
        XCTAssertFalse(prompt.fields["message"]?.stringValue?.contains("purapi.markdown.diff") == true)
    ...
    func testApprovedMarkdownDiffIsInjectedAndAcknowledgedAfterRuntimeAcceptsPrompt() async throws {
        ...
        XCTAssertTrue(message.contains("purapi.markdown.diff"))
    ...
    func testQueuedPromptWithoutDiffDoesNotConsumeLaterApproval() async throws {
        ...
        let queued = PuraPiQueuedPrompt(text: "先前已排队的任务")
    ...
    private func makeSession(fileText: String) throws -> (
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-markdown-collab-\(UUID().uuidString)", isDirectory: true)
