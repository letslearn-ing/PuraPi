...
@MainActor
final class PuraPiCommandAuthorizationTests: XCTestCase {
    func testCommandCatalogLocalizesFiltersAndHandlesSlashQueries() {
        ...
        let chinese = PuraPiCommandCatalog.items(
        ...
        XCTAssertEqual(
            chinese.first(where: { $0.name == "purapi-ui-demo" })?.sourceLabel(language: .chinese),
            "扩展"
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            PuraPiCommandCatalog.filtered(chinese, query: "面板").map(\.name),
            ["purapi-ui-demo"]
        )

        let english = PuraPiCommandCatalog.items(
            piCommands: piCommands,
            language: .english
        )
        ...
        XCTAssertEqual(
            ...
            1,
            "Pi 命令发现不能覆盖 PuraPi 内置命令"
        )

        XCTAssertEqual(PuraPiCommandCatalog.slashQuery(for: "/"), "")
        XCTAssertEqual(PuraPiCommandCatalog.slashQuery(for: "/con"), "con")
        XCTAssertEqual(PuraPiCommandCatalog.slashQuery(for: "/compact 关注上下文"), "compact")
        XCTAssertNil(PuraPiCommandCatalog.slashQuery(for: "普通文本"))
        XCTAssertFalse(PuraPiCommandCatalog.hasCommandArgument("/con"))
        XCTAssertTrue(PuraPiCommandCatalog.hasCommandArgument("/compact 关注上下文"))
        XCTAssertEqual(PuraPiCommandCatalog.action(for: "/NEW"), .newSession)
        XCTAssertEqual(PuraPiCommandCatalog.action(for: "/compact 关注上下文"), .compact)
        XCTAssertNil(PuraPiCommandCatalog.action(for: "普通文本"))
        XCTAssertEqual(
            PuraPiCommandCatalog.customInstructions(from: "/compact 关注上下文"),
            "关注上下文"
        )
        XCTAssertNil(PuraPiCommandCatalog.customInstructions(from: "/compact"))
        XCTAssertEqual(
            PuraPiCommandCatalog.movedSelection(
                currentIndex: 99,
                direction: .down,
                resultCount: 2
            ),
        ...
        XCTAssertNil(
            PuraPiCommandPaletteScrollPolicy.targetFirstVisibleIndex(
                selectedIndex: 2,
        ...
        XCTAssertEqual(
            PuraPiCommandCatalog.discoveredCommand(
                for: "/purapi-ui-demo 参数",
    ...
    func testProjectAuthorizationDetectsScopedResourcesAndStopsAtFilesystemRoot() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-auth-detection-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        XCTAssertFalse(PuraPiProjectAuthorization.requiresAuthorization(for: root))
        ...
        XCTAssertTrue(PuraPiProjectAuthorization.requiresAuthorization(for: root))
    ...
    func testProjectAuthorizationBlocksRuntimeUntilApprovedAndUsesApprovedLaunchMode() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-auth-flow-\(UUID().uuidString)", isDirectory: true)
    ...
    func testInitialAuthorizationDenialKeepsLoadedWorkspaceWithoutStartingRuntime() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-auth-deny-\(UUID().uuidString)", isDirectory: true)
        let extensionDirectory = root
            .appendingPathComponent(".pi", isDirectory: true)
            .appendingPathComponent("extensions", isDirectory: true)
        try FileManager.default.createDirectory(at: extensionDirectory, withIntermediateDirectories: true)
    ...
    func testSlashBuiltInCommandsUseNativeRuntimeActions() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-slash-actions-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testCommandsResponsePopulatesPickerWithoutChangingRuntimeError() async throws {
        ...
        try await waitUntil {
            session.piCommands.map(\.name) == ["purapi-ui-demo"]
        }
        XCTAssertEqual(session.lastError, "保留错误")
        XCTAssertEqual(
            PuraPiCommandCatalog.items(piCommands: session.piCommands, language: .chinese)
                .first(where: { $0.name == "purapi-ui-demo" })?.description,
            "演示面板"
        )
    }
    ...
    private func makeSession() throws -> (PiSessionController, FakePiRPCTransport, URL) {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-command-tests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
    ...
    private func waitUntil(
        ...
        XCTFail("Timed out waiting for PuraPi command/authorization state")
    }
