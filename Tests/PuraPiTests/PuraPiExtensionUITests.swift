...
@MainActor
final class PuraPiExtensionUITests: XCTestCase {
    func testExtensionUIDialogsReturnNativeResultsExactlyOnce() async throws {
        ...
        let requests: [(String, String, PuraPiExtensionUIResult)] = [
    ...
    func testUnsupportedExtensionMethodIsCancelledAndVisible() async throws {
        ...
        XCTAssertEqual(session.lastError, "Pi Extension 请求了 PuraPi 尚未支持的 UI 方法：custom")
    }
    ...
    func testRealPiRPC02ExtensionUIProtocol() async throws {
        guard ProcessInfo.processInfo.environment["PURAPI_REAL_RPC_TEST"] == "1" else {
            throw XCTSkip("Set PURAPI_REAL_RPC_TEST=1 to run the real Pi Extension UI integration test")
        }

        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-real-extension-ui-\(UUID().uuidString)", isDirectory: true)
        ...
        session.draftPrompt = "/purapi-ui-test"
        session.submitPrompt()
    ...
    private func makeSession() throws -> (PiSessionController, FakePiRPCTransport, URL) {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPi-extension-ui-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
    ...
    private func waitUntil(
        ...
        XCTFail("Timed out waiting for PuraPi Extension UI state")
    }
    ...
    private static func realExtensionUISource(resultPath: String) -> String {
        ...
        export default function (pi: ExtensionAPI) {
          pi.registerCommand("purapi-ui-test", {
