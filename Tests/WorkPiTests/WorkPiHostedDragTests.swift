...
///
/// 与 `WorkPiRealMouseDragTests` 的区别，也正是之前一直没覆盖到的部分：
...
@MainActor
final class WorkPiHostedDragTests: XCTestCase {
    ...
    private func assertHostedDrag(divider: DividerUnderTest) {
        ...
        session.selectedPreview = FilePreview(
            url: URL(fileURLWithPath: "/tmp/workpi-hosted.txt"),
            relativePath: "workpi-hosted.txt",
            kind: .text,
        ...
        let toolbar = NSToolbar(identifier: "WorkPiTests.toolbar")
        ...
        // 完全复刻 WorkPiApp.applicationDidFinishLaunching 的宿主方式：
        ...
        case .right:
            ...
            startX = panes[2].frame.minX + WorkPiLayoutState.inspectorInset
            endX = startX - 160
        case .left:
