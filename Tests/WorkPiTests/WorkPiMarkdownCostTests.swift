...
/// TUI 侧调 `Markdown.render(width)` 得到行数组，WorkPi 侧调
/// `WorkPiMarkdownRenderer.render` 得到块结构。两边都不含终端转义生成，
/// 也都不含最终绘制。
///
/// 必须说明的差异：TUI 输出的是字符行，WorkPi 输出的是待交给 SwiftUI 的块，
/// 后者的真实屏幕成本还要加上 CoreText 测量与视图树构建
/// （见 `WorkPiViewportPerfTests`）。因此这里只能比较解析与结构化这一层。
@MainActor
final class WorkPiMarkdownCostTests: XCTestCase {
    ...
    private func medianRenderCost(_ text: String) -> (ms: Double, blocks: Int) {
        ...
        for _ in 0..<3 { _ = WorkPiMarkdownRenderer.render(text) }
        var samples: [(Double, Int)] = []
        for _ in 0..<5 {
            let start = Date()
            let result = WorkPiMarkdownRenderer.render(text)
    ...
    func testMarkdownRenderCostAcrossSizes() {
        for paragraphs in [10, 40, 100, 250] {
            ...
            print(
                "WORKPI paragraphs=\(paragraphs) chars=\(text.count) "
    ...
    func testRenderCostIsLinear() {
        ...
        print("WORKPI ratio(250/40)=\(String(format: "%.1f", ratio)) charRatio=6.4")
