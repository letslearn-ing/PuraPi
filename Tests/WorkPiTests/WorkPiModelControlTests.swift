...
@MainActor
final class WorkPiModelControlTests: XCTestCase {
    // MARK: - 协议解析
    ...
    private func makeSession() throws -> (PiSessionController, FakePiRPCTransport, URL) {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-model-control-\(UUID().uuidString)", isDirectory: true)
    ...
    private func waitUntil(
        ...
        XCTFail("Timed out waiting for WorkPi model control state")
    }
