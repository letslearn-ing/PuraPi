...
/// `WorkPiTabManager → ContentView → SessionWorkspaceView` 链路也覆盖上，
/// 直接对应设置窗口改色时的生产路径。
@MainActor
final class WorkPiContentViewTintIntegrationTests: XCTestCase {
    private struct Root: View {
        @ObservedObject var tabs: WorkPiTabManager
    ...
    func testInspectorNoColorAtInitialInstallPreservesCenterConversation() {
        let projectURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-initial-none-\(UUID())", isDirectory: true)
        do {
        ...
        let defaultsDomain = "WorkPi-initial-none-state-\(UUID())"
        ...
        let toolbar = NSToolbar(identifier: "WorkPi.initialNoneTintTest")
        ...
        let conversationViewports = descendants(of: panes[1]).filter {
            String(describing: type(of: $0)).contains("WorkPiConversationViewport")
        }
    ...
    func testInspectorNoColorDoesNotRemoveCenterConversation() {
        let projectURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-content-tint-\(UUID())", isDirectory: true)
        do {
            try FileManager.default.createDirectory(
                at: projectURL,
                withIntermediateDirectories: true
        ...
        let defaultsDomain = "WorkPi-content-tint-state-\(UUID())"
        let defaults = UserDefaults(suiteName: defaultsDomain)!
        defer { defaults.removePersistentDomain(forName: defaultsDomain) }
        let appearanceState = WorkPiAppearanceState(defaults: defaults)
        let layoutState = WorkPiLayoutState()
        ...
        let toolbar = NSToolbar(identifier: "WorkPi.contentTintTest")
        toolbar.delegate = toolbarDelegate
        toolbar.displayMode = .iconOnly
        window.toolbar = toolbar
        toolbarDelegate.attach(to: toolbar)
        ...
        let neutralSurfacesInCenter = descendants(of: panes[1]).filter {
            String(describing: type(of: $0)).contains("WorkPiInspectorNeutralSurfaceView")
        }
        XCTAssertTrue(
        ...
        appearanceState.setTheme(id: "workpi.midnight")
        settle(0.8)
        window.layoutIfNeeded()
    ...
    func testComposerAcceptsMouseAndKeyboardWhenMarkdownInspectorIsOpen() throws {
        ...
        let projectURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-composer-input-\(UUID())", isDirectory: true)
        do {
            try FileManager.default.createDirectory(
                at: projectURL,
                withIntermediateDirectories: true
        ...
        let defaultsDomain = "WorkPi-composer-input-\(UUID())"
        let defaults = UserDefaults(suiteName: defaultsDomain)!
        defer { defaults.removePersistentDomain(forName: defaultsDomain) }
        let appearanceState = WorkPiAppearanceState(defaults: defaults)
        let layoutState = WorkPiLayoutState()
    ...
    func testComposerAcceptsMouseAndKeyboardAfterSelectingMarkdownInExistingWorkspace() throws {
        ...
        let projectURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-composer-transition-\(UUID())", isDirectory: true)
        do {
            try FileManager.default.createDirectory(
                at: projectURL,
                withIntermediateDirectories: true
        ...
        let defaultsDomain = "WorkPi-composer-transition-\(UUID())"
        let defaults = UserDefaults(suiteName: defaultsDomain)!
        defer { defaults.removePersistentDomain(forName: defaultsDomain) }
        let appearanceState = WorkPiAppearanceState(defaults: defaults)
        let layoutState = WorkPiLayoutState()
        ...
        let headerViews = descendants(of: hosting).filter {
            String(describing: type(of: $0)).contains("WorkPiInspectorHeaderInteractionView")
        }
        XCTAssertFalse(
