...
@MainActor
final class WorkPiMarkdownLifecycleTests: XCTestCase {
    ...
    func testSessionFileMonitorPathUsesConflictHandler() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("workpi-lifecycle-session-\(UUID().uuidString)")
    ...
    func testRetryAfterInvalidMarkdownLoadEntersEditor() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("workpi-lifecycle-retry-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testCloseWorkspaceRefusesToDiscardUnresolvedConflict() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("workpi-lifecycle-close-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let url = root.appendingPathComponent("doc.md")
        try "原始\n".write(to: url, atomically: true, encoding: .utf8)
    ...
    func testSelectingAnotherFileDoesNotHideUnresolvedConflict() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("workpi-lifecycle-select-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testOpeningAnotherWorkspaceDoesNotDiscardUnresolvedConflict() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("workpi-lifecycle-open-\(UUID().uuidString)")
        let otherRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("workpi-lifecycle-open-other-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
    ...
    private func makeState(with text: String) throws -> (
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("workpi-lifecycle-state-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        let url = root.appendingPathComponent("doc.md")
        try text.write(to: url, atomically: true, encoding: .utf8)
        return (WorkPiMarkdownEditorState(), url, root)
    }
}
