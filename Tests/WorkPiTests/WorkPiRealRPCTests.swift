...
@MainActor
final class WorkPiRealRPCTests: XCTestCase {
    func testRealPiRPC01ToolAndFileLoop() async throws {
        guard ProcessInfo.processInfo.environment["WORKPI_REAL_RPC_TEST"] == "1" else {
            throw XCTSkip("Set WORKPI_REAL_RPC_TEST=1 to run the real Pi integration test")
        }

        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-real-rpc-\(UUID().uuidString)", isDirectory: true)
        ...
        Finally reply with exactly WORKPI_RPC_01_OK.
        """
        ...
        XCTAssertTrue(
            session.conversation.contains(where: {
                $0.kind == .assistant && $0.text.contains("WORKPI_RPC_01_OK")
            })
        )
    ...
    func testRealPiRPC03SubagentCreatesPersistentChildAndPublishesPanel() async throws {
        guard ProcessInfo.processInfo.environment["WORKPI_REAL_RPC_TEST"] == "1" else {
            throw XCTSkip("Set WORKPI_REAL_RPC_TEST=1 to run the real Pi SubAgent integration test")
        }

        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-real-subagent-\(UUID().uuidString)", isDirectory: true)
        ...
        let childItems = WorkPiSubagentSessionReader.conversation(
