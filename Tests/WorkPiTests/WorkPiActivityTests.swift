...
/// 否则指示器会一直转。
@MainActor
final class WorkPiActivityTests: XCTestCase {
    // MARK: - 文案
    ...
    func testIndicatorFramesMatchUpstreamLoader() {
        XCTAssertEqual(
            WorkPiActivityIndicator.frames,
            ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]
        )
        XCTAssertEqual(WorkPiActivityIndicator.frameInterval, 0.08, accuracy: 0.0001)
    }
    ...
    func testIndicatorAlignsWithCenteredRowLeadingEdge() {
        ...
        XCTAssertEqual(WorkPiConversationLayout.rowWidth(availableWidth: wide), 900)
        XCTAssertEqual(
            WorkPiConversationLayout.rowLeadingOffset(availableWidth: wide),
            250
        )
        XCTAssertGreaterThan(
            WorkPiConversationLayout.rowLeadingOffset(availableWidth: wide),
            WorkPiConversationLayout.rowHorizontalInset
        )

        // 窄窗口：未达上限，偏移退回固定内边距。
        let narrow: CGFloat = 600
        XCTAssertEqual(
            WorkPiConversationLayout.rowWidth(availableWidth: narrow),
            narrow - WorkPiConversationLayout.rowHorizontalInset * 2
        )
        XCTAssertEqual(
            WorkPiConversationLayout.rowLeadingOffset(availableWidth: narrow),
            WorkPiConversationLayout.rowHorizontalInset
        )

        // 异常小宽度不能产生负偏移。
        XCTAssertGreaterThanOrEqual(
            WorkPiConversationLayout.rowLeadingOffset(availableWidth: 10),
            0
        )
    }
    ...
    private func makeSession() throws -> (PiSessionController, FakePiRPCTransport, URL) {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-activity-\(UUID().uuidString)", isDirectory: true)
    ...
    private func waitUntil(
        ...
        XCTFail("Timed out waiting for WorkPi activity state")
    }
