...
@MainActor
final class WorkPiMarkdownSelectionEditTests: XCTestCase {
    func testCrossBlockDeleteRemovesCoveredBlocksAndIsUndoable() throws {
        ...
        state.setSelection(
            anchor: WorkPiMarkdownSelectionEndpoint(blockID: first.id, offset: first.displayText.utf16.count),
            focus: WorkPiMarkdownSelectionEndpoint(blockID: last.id, offset: 0)
        )

        let editor = WorkPiMarkdownEditorView(state: state, language: .chinese)
    ...
    func testCrossBlockReplacementParsesMultilineReplacement() throws {
        ...
        let last = try XCTUnwrap(editable.last)
        state.setSelection(
            anchor: WorkPiMarkdownSelectionEndpoint(blockID: first.id, offset: 1),
            focus: WorkPiMarkdownSelectionEndpoint(blockID: last.id, offset: 0)
        )
    ...
    func testSingleBlockSelectionFallsBackToTextView() throws {
        ...
        state.setSelection(
            anchor: WorkPiMarkdownSelectionEndpoint(blockID: block.id, offset: 0),
            focus: WorkPiMarkdownSelectionEndpoint(blockID: block.id, offset: 1)
        )
    ...
    private func makeState(with text: String) throws -> (
        WorkPiMarkdownEditorState,
        URL,
        URL
    ) {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-markdown-selection-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        let url = root.appendingPathComponent("doc.md")
        try text.write(to: url, atomically: true, encoding: .utf8)
        return (WorkPiMarkdownEditorState(), root, url)
    }
}
