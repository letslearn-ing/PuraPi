...
@MainActor
final class WorkPiInspectorButtonInteractionTests: XCTestCase {
    ...
    func testHeaderProxyOwnsMouseHitAreaAndActions() {
        let root = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("workpi-inspector-button-test")
        ...
        let tabManager = WorkPiTabManager()
        ...
        let toolbar = NSToolbar(identifier: "WorkPi.inspector-button-test")
        ...
        guard let content = window.contentView,
              ...
              let proxy = parent.subviews.compactMap({
                  $0 as? WorkPiInspectorHeaderInteractionView
              }).first,
              let detachProxy = parent.subviews.compactMap({
                  $0 as? WorkPiInspectorDetachInteractionView
              }).first
        else {
        ...
        let expectedSplitRect = NSRect(
            ...
            y: split.isFlipped
                ? split.bounds.minY + WorkPiLayoutState.inspectorInset
                : split.bounds.maxY
                    - WorkPiLayoutState.inspectorInset
                    - WorkPiInspectorHeaderMetrics.totalHeight,
        ...
        XCTAssertEqual(
            detachProxy.frame.maxX + WorkPiInspectorHeaderMetrics.detachSpacing,
            proxy.frame.minX,
            accuracy: 1,
            "独立按钮应紧邻原有标题操作组左侧"
        )
