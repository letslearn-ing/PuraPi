...
@MainActor
final class WorkPiMarkdownPasteTests: XCTestCase {
    ...
    func testSlashInsertionCreatesTableAndCodeBlocks() throws {
        let table = try XCTUnwrap(WorkPiMarkdownBlockConverter.slashInsertion("/表格"))
        XCTAssertEqual(table.kind, .table)
        XCTAssertTrue(table.source.contains("| --- | --- |"))

        let code = try XCTUnwrap(WorkPiMarkdownBlockConverter.slashInsertion("/code"))
    ...
    private func makeDocument(_ text: String) throws -> (URL, URL) {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("workpi-markdown-paste-\(UUID().uuidString)", isDirectory: true)
