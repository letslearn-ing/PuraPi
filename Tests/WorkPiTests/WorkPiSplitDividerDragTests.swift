...
@MainActor
final class WorkPiSplitDividerDragTests: XCTestCase {
    ...
    override func tearDown() {
        ...
        if let savedSidebarWidth {
            defaults.set(savedSidebarWidth, forKey: WorkPiLayoutState.sidebarWidthKey)
        } else {
            defaults.removeObject(forKey: WorkPiLayoutState.sidebarWidthKey)
        }
        if let savedInspectorWidth {
            defaults.set(savedInspectorWidth, forKey: WorkPiLayoutState.inspectorWidthKey)
        } else {
    ...
    ) -> (WorkPiWorkspaceSplitViewController, NSWindow) {
...
@available(macOS 26.0, *)
@MainActor
final class WorkPiNarrowWindowSqueezeTests: XCTestCase {
...
@available(macOS 26.0, *)
@MainActor
final class WorkPiInspectorLimitTests: XCTestCase {
...
@available(macOS 26.0, *)
@MainActor
final class WorkPiLoadedInspectorDragTests: XCTestCase {
    private func makeLoaded(
        windowWidth: CGFloat
    ) -> (WorkPiWorkspaceSplitViewController, NSWindow, WorkPiLayoutState) {
        let defaults = WorkPiPreferences.shared
        defaults.set(290.0, forKey: WorkPiLayoutState.sidebarWidthKey)
        defaults.set(410.0, forKey: WorkPiLayoutState.inspectorWidthKey)
        ...
        session.selectedPreview = FilePreview(
            url: URL(fileURLWithPath: "/tmp/workpi-drag-fixture.txt"),
            relativePath: "workpi-drag-fixture.txt",
            kind: .text,
...
final class WorkPiMouseDragTests: XCTestCase {
    ...
    func testRightDividerHitAreaDoesNotClaimSidebarDivider() {
        ...
        let session = PiSessionController()
        session.selectedPreview = FilePreview(
            url: URL(fileURLWithPath: "/tmp/workpi-hit.txt"),
            relativePath: "workpi-hit.txt",
            kind: .text,
            text: "sample",
            byteCount: 6,
            modificationDate: Date()
