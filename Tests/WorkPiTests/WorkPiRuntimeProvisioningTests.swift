...
@MainActor
final class WorkPiRuntimeProvisioningTests: XCTestCase {
    func testVersionParserAcceptsCommonCLIOutputAndComparesSemver() {
        XCTAssertEqual(WorkPiRuntimeVersion("v24.19.0\n"), WorkPiRuntimeVersion(major: 24, minor: 19, patch: 0))
        XCTAssertEqual(WorkPiRuntimeVersion("pi 0.84.4 (darwin)"), WorkPiRuntimeVersion(major: 0, minor: 84, patch: 4))
        XCTAssertNil(WorkPiRuntimeVersion("24.19"))
        XCTAssertNil(WorkPiRuntimeVersion("unknown"))
        XCTAssertTrue(
            WorkPiRuntimeVersion(major: 22, minor: 19, patch: 0)
                >= WorkPiRuntimePolicy.minimumNodeVersion
        )
        XCTAssertLessThan(
            WorkPiRuntimeVersion(major: 0, minor: 84, patch: 3),
            WorkPiRuntimePolicy.managedPiVersion
        )
    }

    func testSelectionCarriesManagedNodePathIntoTransportEnvironment() {
        let installation = WorkPiRuntimeInstallation(
            executableURL: URL(fileURLWithPath: "/tmp/workpi/pi"),
            version: WorkPiRuntimePolicy.managedPiVersion,
            source: .workPiManaged,
            nodeURL: URL(fileURLWithPath: "/tmp/workpi/node/bin/node")
        )
        ...
        XCTAssertTrue(snapshot.environmentOverrides["PATH"]?.hasPrefix("/tmp/workpi/node/bin") == true)
    }

    func testExplicitExecutableOverrideWinsOverRememberedPath() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-provision-override-\(UUID().uuidString)", isDirectory: true)
        ...
        let client = WorkPiDefaultRuntimeProvisioningClient(
            locations: WorkPiRuntimeLocations(rootURL: root.appendingPathComponent("managed", isDirectory: true)),
    ...
    func testDiscoveryPrefersRememberedExecutableBeforePATH() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-provision-preferred-\(UUID().uuidString)", isDirectory: true)
    ...
    func testDiscoveryReusesExistingPiAndFindsNodeWithoutNetwork() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-provision-discovery-\(UUID().uuidString)", isDirectory: true)
        ...
        let client = WorkPiDefaultRuntimeProvisioningClient(
            ...
            homeDirectory: root,
            runner: runner,
            managedPiVersion: WorkPiRuntimePolicy.managedPiVersion
        )
        let snapshot = await client.discover()
        ...
        XCTAssertEqual(snapshot.installation?.version, WorkPiRuntimePolicy.managedPiVersion)
    ...
    func testDiscoveryFindsOfficialUserLocalPiLocationsWithoutShellPATH() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-provision-user-local-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testDiscoveryReportsMissingPiAndNodeSeparately() async {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-provision-missing-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let runner = RuntimeProvisioningFakeRunner()
        let client = WorkPiDefaultRuntimeProvisioningClient(
            locations: WorkPiRuntimeLocations(rootURL: root.appendingPathComponent("managed", isDirectory: true)),
    ...
    func testManagedInstallUsesUserDirectoryAndDoesNotReplaceExistingPi() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-provision-install-\(UUID().uuidString)", isDirectory: true)
        ...
        let runner = RuntimeProvisioningFakeRunner()
        let client = WorkPiDefaultRuntimeProvisioningClient(
            locations: WorkPiRuntimeLocations(rootURL: managed),
            environment: ["PATH": bin.path],
            homeDirectory: root,
            runner: runner
        )
        ...
        XCTAssertEqual(installation.source, .workPiManaged)
        XCTAssertEqual(
        ...
        let stored = WorkPiRuntimeManagedStore(
            locations: WorkPiRuntimeLocations(rootURL: managed)
        ).currentInstallation()
    ...
    func testFailedPiInstallCleansStagingAndLeavesNoCurrentPointer() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-provision-failed-install-\(UUID().uuidString)", isDirectory: true)
        let bin = root.appendingPathComponent("bin", isDirectory: true)
        let managed = root.appendingPathComponent("managed", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        try makeExecutable(at: bin.appendingPathComponent("node"))
        ...
        let store = WorkPiRuntimeManagedStore(locations: WorkPiRuntimeLocations(rootURL: managed))
    ...
    func testInstallPreparesPrivateNodeWhenNoCompatibleNodeExists() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-provision-private-node-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let locations = WorkPiRuntimeLocations(rootURL: root.appendingPathComponent("managed", isDirectory: true))
        let runner = RuntimeProvisioningFakeRunner()
        ...
        XCTAssertNotNil(
            WorkPiRuntimeManagedStore(locations: locations).currentNode()
        )
        XCTAssertTrue(
    ...
    func testManagedStoreRejectsSymlinkedReleaseDirectory() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-provision-symlink-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertNil(WorkPiRuntimeManagedStore(locations: locations).currentInstallation())
    }

    func testArchiveSafetyRejectsTraversalAndMixedTopLevelDirectories() {
        XCTAssertTrue(
            WorkPiNodeArchiveSafety.validate(
    ...
    func testInstallerEnvironmentDropsSecretsAndNpmOverrides() {
        let environment = WorkPiRuntimeEnvironment.safeEnvironment(
            base: [
                "PATH": "/tmp/bin",
            ...
            homeDirectory: URL(fileURLWithPath: "/tmp/workpi-home")
        )
    ...
    func testNodeArtifactParserSelectsHighestCompatibleDarwinArchive() {
        ...
        """
        let artifact = WorkPiNodeArtifact.parse(
            checksumText: text,
            platform: "darwin",
            architecture: "arm64"
        )

        XCTAssertEqual(artifact?.version, WorkPiRuntimeVersion(major: 22, minor: 20, patch: 1))
    ...
    func testCurlDownloaderRejectsNonOfficialArtifactURL() async throws {
        let runner = RuntimeProvisioningFakeRunner()
        let downloader = WorkPiCurlArtifactDownloader(runner: runner)
        let destination = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-rejected-download-\(UUID().uuidString)")
    ...
    func testNodeArtifactVerificationAcceptsMatchingDigestAndRejectsMismatch() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-node-digest-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertNoThrow(try WorkPiNodeArtifact.verify(archiveURL: archive, expectedSHA256: digest))
        XCTAssertThrowsError(
            try WorkPiNodeArtifact.verify(
                archiveURL: archive,
    ...
    func testDefaultProcessRunnerBoundsTimeoutAndCapturesOutput() async throws {
        let runner = WorkPiDefaultProcessRunner()
        ...
        do {
            _ = try await runner.run(WorkPiProcessRequest(
    ...
    func testMissingExecutableMarksSessionAsWaitingForProvisioning() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-provision-session-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
    ...
    func testCancellingInstallKeepsSecondInstallBehindStopBarrier() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-provision-cancel-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let installation = WorkPiRuntimeInstallation(
        ...
        let provisioner = WorkPiRuntimeProvisioner(
            locations: WorkPiRuntimeLocations(rootURL: root),
            selection: WorkPiRuntimeSelection(),
            client: client
        )
        try await waitForRuntimeProvisioning {
    ...
    func testProvisionerExposesInstallFailureAndKeepsRetryAvailable() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-provision-failure-state-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let installation = WorkPiRuntimeInstallation(
            executableURL: root.appendingPathComponent("pi"),
            version: WorkPiRuntimePolicy.managedPiVersion,
        ...
        fakeClient.installError = WorkPiRuntimeProvisioningError.network("测试网络失败")
        let provisioner = WorkPiRuntimeProvisioner(
            locations: WorkPiRuntimeLocations(rootURL: root),
            selection: WorkPiRuntimeSelection(),
            client: fakeClient
    ...
    func testProvisionerPublishesDiscoveryAndInstallWithoutAutoInstalling() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-provision-state-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let installation = WorkPiRuntimeInstallation(
            executableURL: root.appendingPathComponent("pi"),
            version: WorkPiRuntimePolicy.managedPiVersion,
