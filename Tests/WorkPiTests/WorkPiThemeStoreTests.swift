...
@MainActor
final class WorkPiThemeStoreTests: XCTestCase {
    func testLoadsPreviewAssetsAndChecksums() throws {
        ...
        let store = WorkPiThemeStore(directoryURL: root)
    ...
    func testMissingSchemaCanBeMigratedExplicitly() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-theme-migrate-\(UUID().uuidString)")
        let packageURL = root.appendingPathComponent("com.example.migrate.workpitheme")
    ...
    func testChecksumMismatchAndExecutableAssetAreRejected() throws {
        ...
        XCTAssertThrowsError(try store.loadPackage(at: packageURL)) { error in
            guard case .checksumMismatch = error as? WorkPiThemeStoreError else {
                return XCTFail("应报告 checksum mismatch，实际为 \(error)")
            }
        }

        let executablePackage = root.appendingPathComponent("com.example.script.workpitheme")
    ...
    func testAppearanceStateExposesValidatedUserThemes() throws {
        ...
        defer { try? FileManager.default.removeItem(at: root) }
        let defaultsName = "WorkPi-theme-state-\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: defaultsName))
        defer { defaults.removePersistentDomain(forName: defaultsName) }

        let state = WorkPiAppearanceState(
            defaults: defaults,
            themeStore: WorkPiThemeStore(directoryURL: root)
        )
    ...
    func testMergeReplacesUserPackageAndDeleteProtectsBuiltIns() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-theme-store-\(UUID().uuidString)")
        let sourceRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-theme-source-\(UUID().uuidString)")
        ...
        XCTAssertThrowsError(try store.delete(id: WorkPiThemeCatalog.defaultID)) { error in
            guard case .cannotDeleteBuiltIn = error as? WorkPiThemeStoreError else {
    ...
    private func makePackage(
        ...
        let root = parent ?? FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-theme-package-\(UUID().uuidString)")
        let packageURL = root.appendingPathComponent("\(id).workpitheme")
        try FileManager.default.createDirectory(
    ...
    private func definitionData(id: String, includeSchema: Bool = true) throws -> Data {
        let original = try JSONEncoder().encode(WorkPiThemeCatalog.defaultDefinition)
