...
@MainActor
final class WorkPiToolbarLayoutTests: XCTestCase {
    private func makeDelegate() -> (WorkPiToolbarDelegate, WorkPiTabManager, WorkPiLayoutState) {
        let manager = WorkPiTabManager()
        let layoutState = WorkPiLayoutState()
        let delegate = WorkPiToolbarDelegate(
            manager: manager,
