...
@MainActor
final class WorkPiDragBlockerLifecycleTests: XCTestCase {
    ...
    override func setUp() {
        ...
        savedSidebar = d.object(forKey: WorkPiLayoutState.sidebarWidthKey)
        savedInspector = d.object(forKey: WorkPiLayoutState.inspectorWidthKey)
        ...
        savedSidebarVisible = d.object(forKey: WorkPiLayoutState.sidebarVisibleKey)
        d.set(true, forKey: WorkPiLayoutState.sidebarVisibleKey)
    }

    override func tearDown() {
        let d = WorkPiPreferences.shared
        if let savedSidebar { d.set(savedSidebar, forKey: WorkPiLayoutState.sidebarWidthKey) }
        else { d.removeObject(forKey: WorkPiLayoutState.sidebarWidthKey) }
        if let savedInspector { d.set(savedInspector, forKey: WorkPiLayoutState.inspectorWidthKey) }
        else { d.removeObject(forKey: WorkPiLayoutState.inspectorWidthKey) }
        if let savedSidebarVisible {
            d.set(savedSidebarVisible, forKey: WorkPiLayoutState.sidebarVisibleKey)
        } else {
            d.removeObject(forKey: WorkPiLayoutState.sidebarVisibleKey)
        }
        super.tearDown()
    }
    ...
    private func findController(_ v: NSView) -> WorkPiWorkspaceSplitViewController? {
        var node: NSResponder? = v
        while let cur = node {
            if let c = cur as? WorkPiWorkspaceSplitViewController { return c }
    ...
    private struct Rig {
        ...
        let controller: WorkPiWorkspaceSplitViewController
        let session: PiSessionController
        let layoutState: WorkPiLayoutState
    }
    ...
    private func blockerCount(_ rig: Rig) -> Int {
        ...
        return parent.subviews.filter { $0 is WorkPiDividerWindowDragBlocker }.count
    }
    ...
    func testBlockerIsReinstalledAfterRemoval() {
        ...
        guard let parent = rig.splitView.superview,
              let blocker = parent.subviews.first(where: { $0 is WorkPiDividerWindowDragBlocker })
    ...
    func testMaximizeAndRestoreInspectorWidth() {
        ...
        XCTAssertEqual(
            originalWidth - WorkPiLayoutState.inspectorInset * 2, 410, accuracy: 4,
...
@available(macOS 26.0, *)
@MainActor
final class WorkPiDragBlockerPassthroughTests: XCTestCase {
    private var savedSidebar: Any?
    private var savedInspector: Any?
    ...
    func testInteriorPointsAreNotSwallowedByBlocker() {
        ...
        for (label, point) in probes {
            ...
            XCTAssertFalse(
                hit is WorkPiDividerWindowDragBlocker,
                "\(label) 被覆盖层吞掉了，正常交互会失灵")
        }

        // 距分隔线 20pt（远超 ±7pt 命中区）也不应被拦。
        let rightEdge = panes[2].frame.minX + WorkPiLayoutState.inspectorInset
