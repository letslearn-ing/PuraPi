...
/// 会话树的命令构造、响应处理与安全门控。
@MainActor
final class WorkPiSessionTreeTests: XCTestCase {
    // MARK: - 轮次定位
    ...
    func testSessionStateClearsOnWorkspaceClose() async throws {
        ...
        session.pendingAttachments = [
            WorkPiAttachment(url: nil, kind: .text("待发送"))
        ]
    ...
    private func makeSession() throws -> (PiSessionController, FakePiRPCTransport, URL) {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WorkPi-session-tree-\(UUID().uuidString)", isDirectory: true)
    ...
    private func waitUntil(
        ...
        XCTFail("Timed out waiting for WorkPi session tree state")
    }
