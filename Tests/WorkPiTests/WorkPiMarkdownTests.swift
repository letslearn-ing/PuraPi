...
@MainActor
final class WorkPiMarkdownTests: XCTestCase {
    func testMarkdownParserPreservesBlockStructure() {
        let blocks = WorkPiMarkdownParser.parse("""
        # 标题
    ...
    func testMarkdownParserChunksVeryLongParagraphWithoutDataLoss() {
        ...
        let blocks = WorkPiMarkdownParser.parse(source)
    ...
    func testCompletedMessageUsesStructuredBlocksInsteadOfFlattenedText() {
        ...
        **重点**、`代码` 和 [链接](https://example.com)。
        ...
        let result = WorkPiMarkdownRenderer.render(source)
        ...
        // 行内标记保留在源文中，由 WorkPiMarkdownInlineRenderer 在块内渲染。
    ...
    func testInlineRendererStripsMarkersAndKeepsLink() {
        let value = WorkPiMarkdownInlineRenderer.render(
            "**重点**、`代码` 和 [链接](https://example.com)。"
        )
    ...
    func testUserMarkdownMapsTablesAndFencesToStructuredBlocks() {
        ...
        XCTAssertNotNil(WorkPiMarkdownRenderer.render(source).fallbackBlocks)
    }
    ...
    func testMarkdownPendingPreviewRemovesBlockAndInlineMarkers() {
        ...
        ```
        """
        let preview = WorkPiMarkdownSanitizer.inlinePreview(source)
    ...
    func testIncrementalMarkdownHandlesDeltaSplitInlineMarker() {
        ...
        XCTAssertEqual(
            WorkPiMarkdownSanitizer.inlinePreview(state.snapshot.tailText),
            "说明：重点 和 代码"
        )
    }

    /// 逐字符馈入，模拟真实的细粒度 `text_delta`。
    private func streamCharByChar(_ text: String) -> WorkPiIncrementalMarkdownState {
        var state = WorkPiIncrementalMarkdownState()
        var accumulated = ""
    ...
    func testIncrementalMarkdownFinishMatchesFullParse() {
        ...
        XCTAssertEqual(
            ...
            WorkPiMarkdownParser.parse(source).map(\.kind)
        )
    ...
    func testCompletedMessageKeepsParagraphAndCodeBoundaries() {
        ...
        **profile + bundle 分层组装**：`dsh-base` 打底。
        ...
        let blocks = try? XCTUnwrap(WorkPiMarkdownRenderer.render(source).fallbackBlocks)
