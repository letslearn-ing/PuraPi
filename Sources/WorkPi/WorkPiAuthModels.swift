...
enum WorkPiAuthType: String, Codable, CaseIterable, Identifiable, Sendable {
    case oauth
    case apiKey = "api_key"
...
struct WorkPiAuthStatus: Codable, Equatable, Sendable {
    let configured: Bool
    let type: WorkPiAuthType?
    let source: String?
    let subscription: Bool

    static let unconfigured = WorkPiAuthStatus(
        configured: false,
        type: nil,
        source: nil,
        subscription: false
...
/// 一个 Provider 可用的认证入口。
struct WorkPiAuthMethod: Codable, Equatable, Identifiable, Sendable {
    let type: WorkPiAuthType
    let name: String
...
struct WorkPiAuthProvider: Codable, Equatable, Identifiable, Sendable {
    let id: String
    let name: String
    let authTypes: [WorkPiAuthMethod]
    let status: WorkPiAuthStatus?

    var isConfigured: Bool { status?.configured == true }
    var configuredType: WorkPiAuthType? { status?.type }
    ...
    func method(_ type: WorkPiAuthType) -> WorkPiAuthMethod? {
...
/// 可用模型的非敏感目录项。
struct WorkPiAuthModel: Codable, Equatable, Identifiable, Sendable {
...
/// 认证桥接返回的完整快照。
struct WorkPiAuthSnapshot: Codable, Equatable, Sendable {
    var providers: [WorkPiAuthProvider]
    var credentials: [WorkPiStoredCredential]
    var models: [WorkPiAuthModel]
    var modelsTruncated: Bool

    static let empty = WorkPiAuthSnapshot(
        providers: [],
        credentials: [],
        models: [],
    ...
    func provider(id: String) -> WorkPiAuthProvider? {
        providers.first { $0.id == id }
    }
}

/// `auth.json` 中的凭据列表只返回 provider 和类型，不返回秘密字段。
struct WorkPiStoredCredential: Codable, Equatable, Identifiable, Sendable {
    let providerId: String
    let type: WorkPiAuthType
...
struct WorkPiAuthRefreshWarning: Equatable, Sendable {
    let providerId: String
    let message: String
}

/// 一次 sidecar 操作的结果。只携带状态和模型元数据，不携带凭据。
struct WorkPiAuthBridgeResult: Sendable {
    let snapshot: WorkPiAuthSnapshot
    let refreshAborted: Bool
    let refreshWarnings: [WorkPiAuthRefreshWarning]
    let changedProviderIDs: [String]
    /// 只包含 auth.json 的设备/文件元数据，不包含文件内容或凭据指纹。
    let authStorageRevision: String?

    init(
        snapshot: WorkPiAuthSnapshot,
        refreshAborted: Bool,
        refreshWarnings: [WorkPiAuthRefreshWarning],
...
struct WorkPiAuthEvent: Codable, Equatable, Identifiable, Sendable {
    let id: UUID
    let type: String
    let message: String?
    let url: URL?
...
struct WorkPiAuthEventLink: Codable, Equatable, Identifiable, Sendable {
    let id: UUID
    let url: URL
    let label: String?
...
struct WorkPiAuthPrompt: Codable, Equatable, Identifiable, Sendable {
...
enum WorkPiAuthPhase: Equatable, Sendable {
    case idle
    case checking
    case loggingIn(providerID: String, type: WorkPiAuthType)
...
enum WorkPiAuthError: LocalizedError, Equatable, Sendable {
...
extension Notification.Name {
    ...
    static let workPiAuthenticationChanged = Notification.Name("WorkPi.authenticationChanged")
    /// 打开设置并切换到账号页，用于 Runtime 认证错误恢复。
    static let workPiOpenAccountSettings = Notification.Name("WorkPi.openAccountSettings")
}
