...
struct WorkPiMarkdownBlockEditor: NSViewRepresentable {
    ...
    let onExitList: () -> Void
    /// 用于把文档级移动的光标放到目标块首/尾。
    let cursorPlacement: WorkPiMarkdownCursorPlacement?
    let onCursorPlacementConsumed: () -> Void
    /// 上下方向键越过块边界。
    let onMoveUp: () -> Void
    let onMoveDown: () -> Void

    func makeNSView(context: Context) -> WorkPiBlockTextView {
        let view = WorkPiBlockTextView()
        view.configure(coordinator: context.coordinator)
        return view
    }

    func updateNSView(_ nsView: WorkPiBlockTextView, context: Context) {
        ...
        nsView.update(
            ...
            style: WorkPiBlockStyle(kind: block.kind),
            isFocused: isFocused,
    ...
    final class Coordinator: NSObject, NSTextViewDelegate {
        var owner: WorkPiMarkdownBlockEditor
        /// 视图当前真正承载的块。与 owner.block.id 不一致说明正处于复用切换中。
        var blockID: UUID
        weak var blockTextView: WorkPiBlockTextView?
        ...
        init(owner: WorkPiMarkdownBlockEditor) {
            self.owner = owner
        ...
        private func flushMarkedTextIfNeeded(from textView: WorkPiBlockTextView) {
        ...
        func applyInlineFormat(
            ...
            guard textView.isEditable,
                  let result = WorkPiMarkdownInlineEditing.apply(
                      format,
                      to: textView.string,
                      selectedRange: textView.selectedRange()
                  )
            else { return }
            if let ownerCoordinator = textView.delegate as? WorkPiMarkdownBlockEditor.Coordinator {
