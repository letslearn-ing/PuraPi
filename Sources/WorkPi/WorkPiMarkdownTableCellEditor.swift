...
@MainActor
struct WorkPiMarkdownTableCellEditor: NSViewRepresentable {
    let text: String
    let isHeader: Bool
    let isFocused: Bool
    ...
    func makeNSView(context: Context) -> WorkPiTableCellTextField {
        let field = WorkPiTableCellTextField()
    ...
    func updateNSView(_ nsView: WorkPiTableCellTextField, context: Context) {
    ...
    final class Coordinator: NSObject, NSTextFieldDelegate {
        var owner: WorkPiMarkdownTableCellEditor
        var suppressCallbacks = false

        init(owner: WorkPiMarkdownTableCellEditor) {
            self.owner = owner
        }
...
@MainActor
final class WorkPiTableCellTextField: NSTextField {
