...
/// Sidebar 的两种模式。
enum WorkPiSidebarMode: String, CaseIterable, Identifiable {
    case files
    case sessions
...
struct WorkPiSidebarPane: View {
    @Environment(\.workPiSidebarTint) private var sidebarTint
    ...
    /// 模式由标题栏工具栏项切换，因此状态存在 `WorkPiLayoutState` 上而非本地。
    private var mode: WorkPiSidebarMode { layoutState.sidebarMode }
    /// 待命名的新建项。
    ...
    var body: some View {
            ...
            Color.clear
                .frame(
                    height: usesNativeSidebarChrome
                        ? WorkPiLayoutState.nativeSidebarContentTopInset
                        : WorkPiLayoutState.sidebarContentTopInset
                )
        ...
        .workPiSidebarSurface(tint: sidebarTint)
...
/// 最容易出错的地方。
@MainActor
struct WorkPiCreateEntrySheet: View {
    @Environment(\.workPiTheme) private var theme
    ...
    var body: some View {
        ...
        .padding(22)
        .frame(width: 360)
        .workPiGlassSurface(role: .glass, cornerRadius: 18, interactive: true)
        .tint(theme.accent)
        .workPiPresentationGlassChrome()
    }
