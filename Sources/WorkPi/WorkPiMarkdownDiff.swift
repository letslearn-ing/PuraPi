...
enum WorkPiMarkdownDiffBuildFailure: Error, Equatable, Sendable {
...
struct WorkPiMarkdownDiffEnvelope: Codable, Equatable, Sendable {
    static let currentSchemaVersion = 1
    static let kind = "workpi.markdown.diff"
    static let source = "workpi.inspector.user-edit"
...
struct WorkPiMarkdownDiffContext: Equatable, Sendable {
    let snapshot: WorkPiMarkdownChangeSnapshot
    let envelope: WorkPiMarkdownDiffEnvelope
    let promptFragment: String
    /// nil 表示仅构建完成、尚未经过用户确认；非 nil 才能进入发送/队列边界。
    let approvalToken: UUID?

    fileprivate init(
        snapshot: WorkPiMarkdownChangeSnapshot,
        envelope: WorkPiMarkdownDiffEnvelope,
    ...
    var approvalFingerprint: WorkPiMarkdownApprovalFingerprint {
        WorkPiMarkdownApprovalFingerprint(
...
struct WorkPiMarkdownApprovalFingerprint: Equatable, Sendable {
    let url: URL
    let baseHash: String
    let currentHash: String
...
struct WorkPiMarkdownApprovedChange: Equatable, Sendable {
    let context: WorkPiMarkdownDiffContext
    let editorRevision: UInt64
    let confirmationToken: UUID
}

struct WorkPiMarkdownDiffReview: Identifiable, Equatable {
    let id: UUID
    let context: WorkPiMarkdownDiffContext
    let editorRevision: UInt64
    ...
    init(
        id: UUID = UUID(),
        context: WorkPiMarkdownDiffContext,
...
struct WorkPiMarkdownCollaborationItem: Identifiable, Equatable {
    let id: URL
    let fileName: String
...
struct WorkPiMarkdownCollaborationDisplayState: Equatable {
    let items: [WorkPiMarkdownCollaborationItem]
    let fileName: String
    let changedBlockCount: Int
...
final class WorkPiMarkdownDiffCancellationToken: @unchecked Sendable {
...
enum WorkPiMarkdownDiffBuilder {
    ...
    static func validateSavedSnapshot(
        _ snapshot: WorkPiMarkdownChangeSnapshot,
        workspaceRoot: URL
    ) throws {
        do {
            ...
            guard text == snapshot.currentText else {
                throw WorkPiMarkdownDiffBuildFailure.invalidBaseline
            }
    ...
    static func build(
        snapshot: WorkPiMarkdownChangeSnapshot,
        workspaceRoot: URL
    ) throws -> WorkPiMarkdownDiffContext {
        try build(
            snapshot: snapshot,
            workspaceRoot: workspaceRoot,
            cancellationCheck: {}
    ...
    static func build(
        ...
        guard combinedByteCount <= maximumCombinedSourceBytes else {
            throw WorkPiMarkdownDiffBuildFailure.sourceTooLarge(
        ...
        guard totalLineCount <= maximumLineCount else {
            throw WorkPiMarkdownDiffBuildFailure.tooManyLines(
        ...
        guard diffByteCount <= maximumDiffBytes else {
            throw WorkPiMarkdownDiffBuildFailure.diffTooLarge(
        ...
        let envelope = WorkPiMarkdownDiffEnvelope(
            schemaVersion: WorkPiMarkdownDiffEnvelope.currentSchemaVersion,
            kind: WorkPiMarkdownDiffEnvelope.kind,
            source: WorkPiMarkdownDiffEnvelope.source,
            path: path,
            baseHash: snapshot.baseHash,
            currentHash: snapshot.currentHash,
        ...
        do {
            ...
            throw WorkPiMarkdownDiffBuildFailure.encodingFailed
        }
        guard jsonData.count <= maximumEnvelopeBytes else {
            throw WorkPiMarkdownDiffBuildFailure.envelopeTooLarge(
        ...
        let json = String(decoding: jsonData, as: UTF8.self)
            ...
            .replacingOccurrences(of: "&", with: "\\u0026")
        let promptFragment = """
        [WorkPi Markdown change context]
        ...
        [End WorkPi Markdown change context]
        """
        ...
        return WorkPiMarkdownDiffContext(
            snapshot: snapshot,
            envelope: envelope,
            promptFragment: promptFragment
        )
    ...
    private static func relativePath(of url: URL, to root: URL) throws -> String {
        ...
        guard lexicalInside else {
            throw WorkPiMarkdownDiffBuildFailure.invalidWorkspacePath
        }
