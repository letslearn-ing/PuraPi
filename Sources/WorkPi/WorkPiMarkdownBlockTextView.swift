...
struct WorkPiBlockStyle: Equatable {
...
/// 承载单块的 NSTextView。
@MainActor
private final class WorkPiInlineTextView: NSTextView {
    var onBecomeFirstResponder: (() -> Void)?
    var onMarkedTextWillResign: (() -> Void)?
    var onResignFirstResponder: (() -> Void)?
    var onFormat: ((WorkPiMarkdownInlineFormat) -> Void)?
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
        ...
        let format: WorkPiMarkdownInlineFormat?
...
@MainActor
final class WorkPiBlockTextView: NSView {
    private let textView = WorkPiInlineTextView()
    private var appliedStyle: WorkPiBlockStyle?
    /// 最近一次传给 NSTextView 的块正文（聚焦时含行内标记，失焦时为展示文本）。
    private var appliedBodyText: String?
    private var appliedPresentation: WorkPiMarkdownInlinePresentation.Result?
    private var appliedParagraphStyle: NSParagraphStyle?
    private var appliedShowsMarkers = false
    private var appliedCursorPlacement: WorkPiMarkdownCursorPlacement?
    ...
    private var pendingCursorPlacement: WorkPiMarkdownCursorPlacement?
    ...
    private weak var coordinator: WorkPiMarkdownBlockEditor.Coordinator?
    ...
    func configure(coordinator: WorkPiMarkdownBlockEditor.Coordinator) {
    ...
    func update(
        blockID: UUID,
        text: String,
        style: WorkPiBlockStyle,
        isFocused: Bool,
        cursorPlacement: WorkPiMarkdownCursorPlacement?,
        ...
        let presentation = WorkPiMarkdownInlinePresentation.render(
            source: text,
            baseFont: style.font,
    ...
    private func scheduleCursorPlacement(_ placement: WorkPiMarkdownCursorPlacement) {
        let generation = focusTaskGeneration
        DispatchQueue.main.async { [weak self] in
