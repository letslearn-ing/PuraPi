...
extension PiSessionController {
    func openWorkspace(_ url: URL) {
        ...
        let requiresAuthorization = WorkPiProjectAuthorization.requiresAuthorization(for: normalizedURL)
        if requiresAuthorization {
            projectAuthorizationState = WorkPiProjectAuthorization.isRemembered(for: normalizedURL)
                ? .approved
                : .needsDecision
        } else {
        ...
        workspaceLoadTask = Task { [weak self] in
            do {
                ...
                self.lastError = WorkPiSensitiveText.redacted(error.localizedDescription)
            }
        }
    }
    ...
    private func selectFileImmediately(_ url: URL) {
        ...
        let isMarkdown = WorkPiMarkdownEditorState.canEdit(selectedURL)
    ...
    func retryFilePreview() {
        ...
        let isMarkdown = WorkPiMarkdownEditorState.canEdit(selectedFileURL)
