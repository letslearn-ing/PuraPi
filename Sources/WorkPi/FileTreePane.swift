...
struct FileTreePane: View {
    ...
    /// 是否自行保留交通灯安全区。
    ///
    /// 放进 `WorkPiSidebarPane` 后安全区由容器统一保留；子视图再加一次会把
    /// 模式切换标签推出可视区。
    ...
    var body: some View {
        contentBody
            // 放进 `WorkPiSidebarPane` 时表面由容器统一提供，覆盖交通灯安全区并与
            // 会话树保持一致；这里再画一层会让顶部安全区露出不同底色。
            .workPiSidebarSurface(enabled: providesOwnSurface, tint: sidebarTint)
        ...
        if let root {
            VStack(spacing: 0) {
                ...
                if reservesTopSafeArea {
                    Color.clear
                        .frame(height: WorkPiLayoutState.sidebarContentTopInset)
                }
    ...
    private func sidebarScroll(_ root: FileNode) -> some View {
        ...
        .workPiSidebarScrollEdgeEffect(enabled: usesNativeSidebarChrome)
