...
/// `WorkPiMarkdownBlockEditor` 承担。所有变更都表达为 `MarkdownBlockEdit`
/// 交给 `WorkPiMarkdownEditorState`，不直接写文件。
enum WorkPiMarkdownDrag {
    static let type = UTType(exportedAs: "com.workpi.markdown-block")
}

@MainActor
struct WorkPiMarkdownEditorView: View {
    @Environment(\.workPiTheme) private var theme
    @ObservedObject var state: WorkPiMarkdownEditorState
    ...
    @StateObject private var findReplace = WorkPiMarkdownFindReplaceState()
    var findReplaceState: WorkPiMarkdownFindReplaceState { findReplace }
    ...
    var body: some View {
        blockList
            ...
            .onReceive(NotificationCenter.default.publisher(for: .workPiSaveDocument)) { _ in
                        ...
                        blockRow(block)
                            .onDrag {
                                ...
                                provider.registerDataRepresentation(
                                    forTypeIdentifier: WorkPiMarkdownDrag.type.identifier,
                                    visibility: .all
                                ) { completion in
                            ...
                            .onDrop(
                                of: [WorkPiMarkdownDrag.type],
                                isTargeted: $isBlockDropTargeted
                            ) { _, _ in
                                guard block.acceptsCursor,
                                      let draggingBlockID else { return false }
                                self.draggingBlockID = nil
                                guard draggingBlockID != block.id else { return false }
                                state.moveBlock(id: draggingBlockID, before: block.id)
                                return true
                            }
    ...
    private var outlineItems: [WorkPiMarkdownOutlineItem] {
        WorkPiMarkdownOutline.items(from: state.blocks)
    }
    ...
    private var outlineMenuTopPadding: CGFloat {
        inspectorTopInset == nil
            ? 6
            : WorkPiInspectorHeaderMetrics.totalHeight + 6
    }
    ...
    private func handleEmptyDocumentChange(_ displayText: String) {
        ...
        let converted = insertion == nil
            ? WorkPiMarkdownBlockConverter.convert(
                block: placeholder,
    ...
    private func editableBlockRow(_ block: MarkdownBlock) -> some View {
        VStack(alignment: .leading, spacing: 2) {
            ...
            if let reference = WorkPiMarkdownImageReference.parse(block.source),
               let root = state.workspaceRootURL,
               let imageURL = reference.resolvedURL(workspaceRoot: root) {
                WorkPiMarkdownEmbeddedImageView(
                    url: imageURL,
                    workspaceRoot: root,
