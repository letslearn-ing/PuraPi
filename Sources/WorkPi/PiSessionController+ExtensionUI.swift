...
///
/// Pi RPC 允许多个扩展对话请求在同一条 Runtime 流上等待；WorkPi 只显示一个
...
extension PiSessionController {
    func handleExtensionUIRequest(_ record: PiRPCRecord) {
        ...
        case "notify":
            let notification = WorkPiExtensionNotification(
        ...
        case "setTitle":
            // WorkPi 的窗口/项目标签标题由 AppKit 工作区管理；扩展的终端标题
            // 不应覆盖原生窗口标题。
            break
        default:
            // Pi 当前的 `custom()` 在 RPC 中不会发出请求；若未来版本新增了
            // 未知方法，必须回传取消而不是让扩展永久等待。
            lastError = "Pi Extension 请求了 Pura Pi 尚未支持的 UI 方法：\(request.method)"
            respondToExtensionUI(requestID: request.id, result: .cancelled)
        }
    }

    func resolveExtensionUI(_ requestID: String, _ result: WorkPiExtensionUIResult) {
        ...
        Task { [weak self] in
            ...
            do {
                ...
                self.lastError = "无法回应 Pi Extension UI 请求：\(WorkPiSensitiveText.redacted(error.localizedDescription))"
                self.cancelPendingExtensionUIRequests()
                return
            }
        ...
        Task { @MainActor [weak self] in
            ...
            Task { @MainActor [weak self] in
                ...
                do {
                    ...
                    self.lastError = "无法取消 Pi Extension UI 请求：\(WorkPiSensitiveText.redacted(error.localizedDescription))"
                }
            }
        }
    }
    ...
    private func rejectOverflowingExtensionUIDialog(_ requestID: String) {
        ...
        Task { [weak self] in
            do {
                ...
                self.lastError = "无法取消超出的 Pi Extension UI 请求：\(WorkPiSensitiveText.redacted(error.localizedDescription))"
            }
        }
    }

    private func respondToExtensionUI(requestID: String, result: WorkPiExtensionUIResult) {
        guard markExtensionUIRequestCompleted(requestID) else { return }
        let command: PiRPCCommand
        switch result {
        case .value(let value):
