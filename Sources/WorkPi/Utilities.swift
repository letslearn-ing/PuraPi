...
/// 为 Toolbar 叠加第二套动态占位。
@MainActor
final class WorkPiLayoutState: ObservableObject {
    static let sidebarVisibleKey = WorkPiPreferences.Key.sidebarVisible
    static let sidebarWidthKey = WorkPiPreferences.Key.sidebarWidth
    static let inspectorWidthKey = WorkPiPreferences.Key.inspectorWidth
    ...
    ///
    /// 放在这里而不是 `WorkPiSidebarPane` 的 `@State`：模式切换图标位于
    /// 标题栏工具栏中，与 Sidebar 内容处于两棵不同的视图树，必须共享状态。
    @Published var sidebarMode: WorkPiSidebarMode = .files
    ...
    func toggleSidebar() {
        ...
        WorkPiPreferences.shared.set(sidebarVisible, forKey: Self.sidebarVisibleKey)
    }
    ...
    func persistSidebarWidth() {
        WorkPiPreferences.shared.set(Double(sidebarWidth), forKey: Self.sidebarWidthKey)
    }
    ...
    func persistInspectorWidth() {
        WorkPiPreferences.shared.set(Double(inspectorWidth), forKey: Self.inspectorWidthKey)
    }
...
extension Notification.Name {
    static let workPiNewWorkspace = Notification.Name("WorkPi.newWorkspace")
    static let workPiOpenWorkspace = Notification.Name("WorkPi.openWorkspace")
    static let workPiCloseSelectedTab = Notification.Name("WorkPi.closeSelectedTab")
    static let workPiCloseAllProjects = Notification.Name("WorkPi.closeAllProjects")
    static let workPiToggleSidebar = Notification.Name("WorkPi.toggleSidebar")
    static let workPiToggleInspectorDetached = Notification.Name("WorkPi.toggleInspectorDetached")
}
