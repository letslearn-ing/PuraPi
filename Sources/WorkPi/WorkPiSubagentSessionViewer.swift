...
enum WorkPiSubagentSessionReader {
...
@MainActor
struct WorkPiSubagentSessionViewer: View {
    ...
    var body: some View {
        ...
        .background(WorkPiAdaptiveContentBackground(legacyColor: theme.contentBackground))
    ...
    private var header: some View {
        ...
        .workPiGlassSurface(role: .hud, cornerRadius: 0)
    }
    ...
    private func trustedSessionsRoot() -> URL {
        guard let configured = task.sessionRootPath,
              !configured.isEmpty
        else { return WorkPiSubagentSessionReader.defaultSessionsRoot }
        ...
        guard candidate.path == agentRoot.path || candidate.path.hasPrefix(agentRoot.path + "/") else {
            return WorkPiSubagentSessionReader.defaultSessionsRoot
        }
        return candidate
    }
