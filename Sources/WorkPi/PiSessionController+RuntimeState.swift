...
enum WorkPiRuntimeTerminationReason {
    case eof
...
extension PiSessionController {
    ...
    /// 开始一个新的 Agent 回合。Pi 的事件本身没有回合 id，WorkPi 用本地
    ...
    func handleTerminalTransportSendFailure(
        ...
        handleRuntimeTermination(
            .transportError(WorkPiSensitiveText.redacted(error.localizedDescription)),
            generation: generation
        )
        return true
    }
    ...
    func handleRuntimeTermination(
        _ reason: WorkPiRuntimeTerminationReason,
        generation: UUID
    ) {
        guard self.generation == generation,
              !runtimeTerminationHandled
        ...
        let failed = !wasCancelled
        let reasonWithDiagnostic: WorkPiRuntimeTerminationReason = {
        ...
        let message = WorkPiSensitiveText.redacted(reasonWithDiagnostic.errorMessage)
