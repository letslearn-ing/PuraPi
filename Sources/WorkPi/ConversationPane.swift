...
struct ConversationPane: View {
    ...
    let queuedPrompts: [WorkPiQueuedPrompt]
    ...
    let runtimeStatusSnapshot: WorkPiRuntimeStatusSnapshot?
    let runtimeStatusLoading: Bool
    let runtimeStatusPanelError: String?
    let runtimeControlError: String?
    let autoRetryEnabled: Bool?
    let retryWaitState: WorkPiRetryWaitState?
    let turnRecords: [WorkPiTurnRecord]
    ...
    let markdownCollaboration: () -> WorkPiMarkdownCollaborationDisplayState
    ...
    let extensionNotifications: [WorkPiExtensionNotification]
    let extensionStatuses: [String: String]
    let extensionWidgets: [WorkPiExtensionWidget]
    ...
    var body: some View {
        ...
        return VStack(spacing: 0) {
            ...
            // `WorkPiConversationLayout` 的同一公式算偏移，否则会错位。
            // 它在滚动区外，因此逐帧重绘不会触发正文重排。
            if let activity {
                GeometryReader { proxy in
                    WorkPiActivityIndicator(
                        activity: activity,
                        language: language,
                    ...
                    .padding(
                        .leading,
                        WorkPiConversationLayout.rowLeadingOffset(
                            availableWidth: proxy.size.width
                        )
                    )
                    .frame(maxWidth: .infinity, alignment: .leading)
                }
                .frame(height: WorkPiActivityIndicator.preferredHeight)
                .padding(.bottom, 6)
                .transition(.opacity)
            }
            ...
            if markdownState.isVisible {
                WorkPiMarkdownCollaborationBanner(
                    state: markdownState,
                    language: language,
        ...
        .onReceive(NotificationCenter.default.publisher(for: .workPiFindInConversation)) { _ in
            searchState.isPresented = true
        }
        .sheet(item: $selectedSubagentTask) { task in
            WorkPiSubagentSessionViewer(task: task, language: language)
        }
    }
    ...
    private var conversationScrollRegion: some View {
        ...
        .overlay(alignment: .top) {
            if showsSearchBar, searchState.isPresented {
                WorkPiConversationSearchBar(state: searchState, language: language)
                    .padding(.top, 8)
...
private struct ContinueRecentSessionPrompt: View {
    ...
    var body: some View {
        HStack(spacing: 11) {
            ...
            if state != .loading {
                WorkPiGlassButton(prominent: true, action: action) {
