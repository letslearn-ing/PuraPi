...
struct WorkPiThemePackage: Identifiable, Equatable, Sendable {
    ...
    var theme: WorkPiTheme { WorkPiTheme(definition: definition) }
...
enum WorkPiThemeStoreError: LocalizedError, Equatable, Sendable {
...
/// 用户主题包的安全存储。
///
/// `.workpitheme` 只是一种数据目录：加载器不会调用 `Process`、动态加载库、
/// 执行脚本，也不会把主题包路径当成工作区或 Pi 权限来源。所有文件都会经过
/// 路径、符号链接、大小、权限、schema 和可选 checksum 校验。
struct WorkPiThemeStore: Sendable {
    static let packageExtension = "workpitheme"
    ...
    static var defaultDirectoryURL: URL {
        ...
        return applicationSupport
            .appendingPathComponent("WorkPi", isDirectory: true)
    ...
    init(
        directoryURL: URL = WorkPiThemeStore.defaultDirectoryURL,
    ...
    func scan() -> [WorkPiThemePackage] {
        ...
        return entries
            ...
            .compactMap { try? loadPackage(at: $0) }
            .sorted { $0.id.localizedStandardCompare($1.id) == .orderedAscending }
    }

    /// `scan` 的语义别名，方便设置页和导入逻辑表达意图。
    func packages() -> [WorkPiThemePackage] { scan() }

    func loadPackage(id: String) throws -> WorkPiThemePackage {
        guard isValidThemeID(id) else {
            throw WorkPiThemeStoreError.invalidPackage("主题 id 不合法")
        }
        let url = directoryURL.appendingPathComponent(
            "\(id).\(Self.packageExtension)",
            isDirectory: true
        )
        guard FileManager.default.fileExists(atPath: url.path) else {
            throw WorkPiThemeStoreError.packageNotFound(id)
        }
        return try loadPackage(at: url)
    }

    func loadPackage(at url: URL) throws -> WorkPiThemePackage {
        let packageURL = url.standardizedFileURL
        guard packageURL.pathExtension.lowercased() == Self.packageExtension else {
            throw WorkPiThemeStoreError.invalidPackage("目录必须以 .\(Self.packageExtension) 结尾")
        }
        guard isDirectory(packageURL), !isSymbolicLink(packageURL) else {
            throw WorkPiThemeStoreError.unsafeResource(packageURL.path)
        }

        let packageID = packageURL.deletingPathExtension().lastPathComponent
        guard isValidThemeID(packageID) else {
            throw WorkPiThemeStoreError.invalidPackage("目录名不是安全的主题 id")
        }
        ...
        guard files.keys.allSatisfy({
            ...
            throw WorkPiThemeStoreError.invalidPackage("根目录包含未支持的文件")
        }
        let themeURL = packageURL.appendingPathComponent("theme.json")
        guard files["theme.json"] != nil else {
            throw WorkPiThemeStoreError.invalidPackage("缺少 theme.json")
        }
        ...
        if let preview = files["preview.png"] {
            ...
            guard pngSignature == [
                ...
                throw WorkPiThemeStoreError.invalidPackage("preview.png 不是有效 PNG")
            }
            previewURL = preview
        } else {
            previewURL = nil
        ...
        for (relativePath, fileURL) in files where relativePath.hasPrefix("assets/") {
            ...
            guard assetCount <= 1_024 else {
                throw WorkPiThemeStoreError.invalidPackage("assets 文件数量超过 1024 个")
            }
            ...
            guard size <= maximumAssetBytes else {
                throw WorkPiThemeStoreError.resourceTooLarge(
                    path: relativePath,
                    byteCount: size
                )
            }
    ...
    /// 返回内置主题加上所有通过校验的用户主题。
    func themes() -> [WorkPiTheme] {
        let builtInIDs = Set(WorkPiThemeCatalog.builtInThemes.map(\.id))
        let external = scan()
            .filter { !builtInIDs.contains($0.id) }
            .map(\.theme)
        return WorkPiThemeCatalog.builtInThemes + external
    }

    func theme(for id: String) -> WorkPiTheme {
        themes().first(where: { $0.id == id }) ?? .default
    }

    /// 将一个外部主题包安全复制到用户主题目录，并替换同 id 的旧包。
    @discardableResult
    func merge(packageAt sourceURL: URL) throws -> WorkPiThemePackage {
        let source = try loadPackage(at: sourceURL)
        guard !WorkPiThemeCatalog.builtInThemes.contains(where: { $0.id == source.id }) else {
            throw WorkPiThemeStoreError.invalidPackage("用户包不能覆盖内置主题")
        }
        ...
        let temporary = directoryURL.appendingPathComponent(
            ".workpi-import-\(UUID().uuidString).\(Self.packageExtension)",
            isDirectory: true
        )
        let backup = directoryURL.appendingPathComponent(
            ".workpi-backup-\(UUID().uuidString).\(Self.packageExtension)",
            isDirectory: true
        )
        ...
        do {
            ...
            if let storeError = error as? WorkPiThemeStoreError {
                throw storeError
            }
            throw WorkPiThemeStoreError.io(error.localizedDescription)
        }
    }

    /// 对 schema 0 或缺省 schema 的包执行显式迁移；读取本身不会改写用户文件。
    @discardableResult
    func migrate(packageAt url: URL) throws -> WorkPiThemePackage {
        let package = try loadPackage(at: url)
        guard package.needsMigration else { return package }
        let data = try JSONEncoder.workPiThemeEncoder.encode(package.definition)
        ...
        } catch let error as WorkPiThemeStoreError {
    ...
    func delete(id: String) throws {
        guard !WorkPiThemeCatalog.builtInThemes.contains(where: { $0.id == id }) else {
            throw WorkPiThemeStoreError.cannotDeleteBuiltIn(id)
        }
        guard isValidThemeID(id) else {
            throw WorkPiThemeStoreError.invalidPackage("主题 id 不合法")
        }
        ...
        guard isInside(packageURL, root: directoryURL),
              FileManager.default.fileExists(atPath: packageURL.path)
        else { throw WorkPiThemeStoreError.packageNotFound(id) }
        guard isDirectory(packageURL), !isSymbolicLink(packageURL) else {
            throw WorkPiThemeStoreError.unsafeResource(packageURL.path)
        }
        do {
    ...
    private func decodeDefinition(
        ...
        do {
            ...
            throw WorkPiThemeStoreError.invalidPackage("theme.json 无法规范化")
        }
        do {
            ...
            guard definition.id == expectedID else {
                throw WorkPiThemeStoreError.invalidPackage("theme id 与目录名不一致")
            }
            return (definition, needsMigration)
        } catch let error as WorkPiThemeStoreError {
            throw error
        } catch {
            throw WorkPiThemeStoreError.invalidPackage(
                "theme.json 缺少必需字段：\(error.localizedDescription)"
            )
        }
    }

    private func validate(_ definition: WorkPiThemeDefinition) throws {
        guard definition.schemaVersion == Self.currentSchemaVersion else {
            throw WorkPiThemeStoreError.unsupportedSchema(definition.schemaVersion)
        }
        guard isValidThemeID(definition.id) else {
            throw WorkPiThemeStoreError.invalidPackage("主题 id 只能包含字母、数字、点、短横线和下划线")
        }
        guard definition.names.values.contains(where: {
            !$0.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        }) else {
            throw WorkPiThemeStoreError.invalidPackage("至少需要一个非空主题名称")
        }
        ...
        guard metrics.chromeCornerRadius.isFinite,
            ...
            throw WorkPiThemeStoreError.invalidPackage("主题视觉指标超出安全范围")
        }
    }

    private func validate(_ colors: WorkPiThemeColors) throws {
        for token in [
            ...
            guard token.opacity.isFinite, (0...1).contains(token.opacity) else {
                throw WorkPiThemeStoreError.invalidPackage("颜色透明度必须在 0 到 1 之间")
            }
            if case .rgba(let rgba) = token.source {
                guard [rgba.red, rgba.green, rgba.blue, rgba.alpha].allSatisfy({
                    $0.isFinite && (0...1).contains($0)
                }) else {
                    throw WorkPiThemeStoreError.invalidPackage("RGBA 颜色通道必须在 0 到 1 之间")
                }
            }
        }
    }
    ...
    private func enumerateFiles(in root: URL) throws -> [String: URL] {
        guard let enumerator = FileManager.default.enumerator(
            ...
            throw WorkPiThemeStoreError.io("无法枚举主题包目录")
        }
        ...
        for case let url as URL in enumerator {
            ...
            guard isInside(item, root: root) else {
                throw WorkPiThemeStoreError.unsafeResource(item.path)
            }
            if isSymbolicLink(item) {
                throw WorkPiThemeStoreError.unsafeResource(relativePath(item, root: root))
            }
    ...
    private func validateResourceFile(_ url: URL, relativePath: String) throws {
        guard isSafeRelativePath(relativePath) else {
            throw WorkPiThemeStoreError.unsafeResource(relativePath)
        }
    ...
    private func readDeclaredChecksums(
        ...
        for name in jsonManifestNames where files[name] != nil {
            ...
            else { throw WorkPiThemeStoreError.invalidPackage("\(name) 不是 JSON 对象") }
            ...
            for candidate in candidates {
                for (path, value) in candidate {
                    guard let hash = value as? String else {
                        throw WorkPiThemeStoreError.invalidPackage("\(name) 中的校验和格式无效")
                    }
        ...
        if fileManager.fileExists(atPath: checksumURL.path) {
            ...
            guard let text = String(data: data, encoding: .utf8) else {
                throw WorkPiThemeStoreError.invalidPackage("checksum.sha256 不是 UTF-8")
            }
    ...
    private func mergeChecksum(
        ...
        else { throw WorkPiThemeStoreError.unsafeResource(rawPath) }
        let hash = rawHash.lowercased()
        if let old = checksums[path], old != hash {
            throw WorkPiThemeStoreError.checksumMismatch(path)
        }
        checksums[path] = hash
    }
    ...
    private func fileAttributes(
        ...
        do {
            ...
            throw WorkPiThemeStoreError.io("无法读取 \(relativePath)")
        }
    }
    ...
    func readBoundedData(
        ...
        do {
            ...
            throw WorkPiThemeStoreError.io("无法读取 \(relativePath)：\(error.localizedDescription)")
        }
    }
...
private extension JSONEncoder {
    static var workPiThemeEncoder: JSONEncoder {
