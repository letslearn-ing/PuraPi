...
extension PiSessionController {
    ...
    func consume(_ event: PiRPCTransportEvent, generation: UUID) {
        ...
        case .diagnostic(let text):
            ...
            let trimmed = WorkPiSensitiveText.redacted(
    ...
    func consume(_ record: PiRPCRecord) {
        ...
        case "auto_retry_start":
            ...
            runOutcome = .running
            terminalRunStatus = nil
            retryWaitState = WorkPiRetryWaitState(
                id: UUID(),
    ...
    func requestPiCommands() {
        ...
        Task { [weak self] in
            do {
                ...
                self.lastError = "无法读取 Pi 命令目录：\(WorkPiSensitiveText.redacted(error.localizedDescription))"
            }
        }
    }
