...
/// 认证 sidecar 的内置脚本资源。资源由 WorkPi 自己打包，不接受用户可执行主题或项目文件。
enum WorkPiAuthBridgeResources {
    static var scriptURL: URL? {
        Bundle.module.url(forResource: "WorkPiAuthBridge", withExtension: "mjs")
    }
}

/// 启动官方认证 SDK sidecar 所需的固定路径。这里不包含任何凭据内容。
struct WorkPiAuthBridgeConfiguration: Equatable, Sendable {
    let nodeURL: URL
...
/// 把 WorkPi 选择的 Pi 安装映射到同一发行版中的 Node 和官方 SDK 入口。
enum WorkPiAuthStorageSecurity {
    ...
    static func ensureUserOnlyPermissions(at url: URL) throws {
        ...
        guard descriptor >= 0 else {
            ...
            throw WorkPiAuthError.unavailable("无法安全访问 Pi auth.json；请检查文件权限。")
        }
        ...
        guard Darwin.fstat(descriptor, &information) == 0,
            ...
            throw WorkPiAuthError.unavailable("Pi auth.json 必须是普通文件，认证桥接已停止。")
        }
        guard information.st_size <= 1 * 1024 * 1024 else {
            throw WorkPiAuthError.unavailable("Pi auth.json 超过认证桥接的安全大小限制。")
        }
        guard !hasExtendedACL(descriptor) else {
            throw WorkPiAuthError.unavailable("Pi auth.json 含有额外访问控制列表；请移除 ACL 后重试。")
        }
        if (information.st_mode & 0o077) != 0 {
            guard Darwin.fchmod(descriptor, mode_t(0o600)) == 0 else {
                throw WorkPiAuthError.unavailable("Pi auth.json 权限过宽且无法修复；请将权限设为 600。")
            }
            guard Darwin.fstat(descriptor, &information) == 0,
                  (information.st_mode & 0o077) == 0
            else {
                throw WorkPiAuthError.unavailable("无法确认 Pi auth.json 的安全权限。")
            }
        }
    }
    ...
    private static func ensureParentDirectory(at url: URL) throws {
        ...
        while true {
            ...
            if descriptor < 0 {
                ...
                throw WorkPiAuthError.unavailable("无法安全访问 Pi auth.json 所在目录。")
            }
            ...
            guard directoryIsSafe else {
                throw WorkPiAuthError.unavailable("Pi auth.json 所在目录的访问权限不安全。")
            }
...
enum WorkPiAuthBridgeLocator {
    static func locate(
        selection: WorkPiRuntimeSelectionSnapshot? = nil,
        locations: WorkPiRuntimeLocations = WorkPiRuntimeLocations(),
        homeDirectory: URL = FileManager.default.homeDirectoryForCurrentUser,
        environment: [String: String] = ProcessInfo.processInfo.environment
    ) -> Result<WorkPiAuthBridgeConfiguration, WorkPiAuthError> {
        ...
        let managedStore = WorkPiRuntimeManagedStore(locations: locations)
        ...
        nodeCandidates.append(contentsOf: WorkPiRuntimeCandidatePaths.nodeCandidates(
        ...
        guard WorkPiAuthBridgeResources.scriptURL != nil else {
            return .failure(.unavailable("Pura Pi 认证桥接资源缺失，请重新构建应用。"))
        }
        ...
        return .success(WorkPiAuthBridgeConfiguration(
    ...
    private static func findSDKEntry(for executableURL: URL) -> URL? {
        ...
        for _ in 0..<10 {
            ...
            if WorkPiAuthFileSystem.isRegularFile(at: packageManifest),
               let manifestSize,
               manifestSize.intValue <= 128 * 1024,
               let data = boundedData(at: packageManifest, maximumBytes: 128 * 1024),
               let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
               object["name"] as? String == WorkPiRuntimePolicy.piPackageName,
               let versionText = object["version"] as? String,
               isPackageVersion(versionText),
               let version = WorkPiRuntimeVersion(versionText),
               version >= WorkPiRuntimePolicy.minimumSupportedPiVersion {
                ...
                guard resolvedEntry.path.hasPrefix(rootPath),
                      WorkPiAuthFileSystem.isRegularFile(at: resolvedEntry),
...
struct WorkPiAuthBridgeRequest: Encodable, Sendable {
    let id: String
    let type: String
    let provider: String?
    let authType: WorkPiAuthType?
    ...
    static func login(id: String, provider: String, type: WorkPiAuthType) -> Self {
...
struct WorkPiAuthBridgePromptResponse: Encodable, Sendable {
...
struct WorkPiAuthBridgeMessage: Decodable, Sendable {
    let type: String
    let id: String?
    ...
    let event: WorkPiAuthEventPayload?
    let providers: [WorkPiAuthProvider]?
    let credentials: [WorkPiStoredCredential]?
    let models: [WorkPiAuthModel]?
    let modelsTruncated: Bool?
    let changedProviderId: String?
    let changedProviderIds: [String]?
    let changedAuthType: WorkPiAuthType?
    let refreshAborted: Bool?
    let refreshErrors: [WorkPiAuthRefreshError]?
    let authErrors: [WorkPiAuthRefreshError]?
    let availabilityErrors: [WorkPiAuthRefreshError]?
...
struct WorkPiAuthRefreshError: Decodable, Equatable, Sendable {
    let providerId: String
    let message: String
}

/// 先解码为中间载荷，再严格验证 URL 和字段长度，最后交给主线程状态。
struct WorkPiAuthEventPayload: Decodable, Sendable {
    let type: String
    let message: String?
    let url: String?
    ...
    let links: [WorkPiAuthEventLinkPayload]?

    func materialize() -> WorkPiAuthEvent? {
        ...
        let parsedLinks: [WorkPiAuthEventLink] = (links ?? []).compactMap { link in
            guard let url = Self.safeURL(link.url) else { return nil }
            return WorkPiAuthEventLink(
                url: url,
                label: link.label.map { WorkPiSensitiveText.redacted(String($0.prefix(512)), limit: 512) }
            )
        }
        return WorkPiAuthEvent(
            type: type,
            message: message.map { WorkPiSensitiveText.redacted(String($0.prefix(4 * 1024))) },
            url: parsedURL,
            instructions: instructions.map { WorkPiSensitiveText.redacted(String($0.prefix(4 * 1024))) },
            userCode: userCode.map { WorkPiSensitiveText.redacted(String($0.prefix(512)), limit: 512) },
...
struct WorkPiAuthEventLinkPayload: Decodable, Sendable {
    let url: String
    let label: String?
}

extension WorkPiAuthPrompt {
...
extension WorkPiAuthBridgeMessage {
    func operationResult() -> WorkPiAuthBridgeResult? {
        ...
        let warnings = (refreshErrors ?? []).map {
            WorkPiAuthRefreshWarning(providerId: $0.providerId, message: $0.message)
        } + (authErrors ?? []).map {
            WorkPiAuthRefreshWarning(providerId: $0.providerId, message: $0.message)
