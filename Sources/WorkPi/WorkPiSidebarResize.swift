...
struct WorkPiSidebarResizeHandle: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> WorkPiSidebarResizeNSView {
        let view = WorkPiSidebarResizeNSView()
        update(view)
        return view
    }

    func updateNSView(_ nsView: WorkPiSidebarResizeNSView, context: Context) {
        update(nsView)
    }

    private func update(_ view: WorkPiSidebarResizeNSView) {
...
final class WorkPiSidebarResizeNSView: NSView {
    var currentWidth: CGFloat = WorkPiLayoutState.defaultSidebarWidth
