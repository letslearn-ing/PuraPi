...
enum WorkPiMarkdownCursorPlacement: Equatable {
    case beginning
    case end
}

struct WorkPiMarkdownCursorRequest: Equatable {
    let blockID: UUID
...
extension Notification.Name {
    /// ⌘S：保存当前文档。
    static let workPiSaveDocument = Notification.Name("WorkPi.saveDocument")
}
...
/// 状态边界读取已确认的快照。
@MainActor
final class WorkPiMarkdownEditorState: ObservableObject {
    ...
    @Published var selection: WorkPiMarkdownSelection?
    /// 文档级光标移动的一次性请求；由目标块消费后清除。
    @Published var cursorRequest: WorkPiMarkdownCursorRequest? = nil
    ...
    /// 写入后被新快照误判为外部修改。
    let backgroundSaveCoordinator = WorkPiMarkdownSaveCoordinator()
    /// 与磁盘保存基线分开：它代表 Agent 协作层最后确认的版本，自动保存不会推进它。
    let changeBuffer = WorkPiMarkdownChangeBuffer()
    ...
    var pendingCollaborationSnapshot: WorkPiMarkdownChangeSnapshot? {
    ...
    func setLocalSelection(blockID: UUID, range: NSRange) {
        ...
        cursorRequest = nil
        selection = WorkPiMarkdownSelection(
            anchor: WorkPiMarkdownSelectionEndpoint(blockID: blockID, offset: start),
            focus: WorkPiMarkdownSelectionEndpoint(blockID: blockID, offset: end)
        )
    }
    ...
    func setSelection(
        anchor: WorkPiMarkdownSelectionEndpoint,
        ...
        let clampedAnchor = WorkPiMarkdownSelectionEndpoint(
            blockID: anchor.blockID,
            offset: min(anchorBlock.displayText.utf16.count, max(0, anchor.offset))
        )
        let clampedFocus = WorkPiMarkdownSelectionEndpoint(
            blockID: focus.blockID,
            offset: min(focusBlock.displayText.utf16.count, max(0, focus.offset))
        )
        selection = WorkPiMarkdownSelection(anchor: clampedAnchor, focus: clampedFocus)
    ...
    func selectedMarkdownSource() -> String? {
        ...
        for index in startIndex...endIndex {
            ...
            parts.append(WorkPiMarkdownBlockConverter.composeSource(
                kind: block.kind,
    ...
    /// 请求把焦点移动到指定块的首部或尾部。
    func requestCursorPlacement(for blockID: UUID, at placement: WorkPiMarkdownCursorPlacement) {
        guard blocks.contains(where: { $0.id == blockID && $0.acceptsCursor }) else { return }
        focusedBlockID = blockID
        cursorRequest = WorkPiMarkdownCursorRequest(blockID: blockID, placement: placement)
    }
