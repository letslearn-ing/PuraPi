...
struct WorkPiMarkdownImageReference: Equatable, Sendable {
    let altText: String
    ...
    func resolvedURL(workspaceRoot: URL) -> URL? {
        ...
        guard WorkPiMarkdownImageInsertion.validatedImageURL(
            candidate,
...
@MainActor
struct WorkPiMarkdownEmbeddedImageView: View {
    ...
    private func loadImage() async {
        ...
        let imageData: Data? = await Task.detached(priority: .utility) { () async -> Data? in
            ...
            guard !Task.isCancelled,
                  let data = WorkPiMarkdownImageInsertion.readImageData(
                      at: url,
                      workspaceRoot: workspaceRoot
                  )
            else { return nil }
            return WorkPiMarkdownImageInsertion.safePreviewData(from: data)
        }.value
        guard !Task.isCancelled,
              let imageData,
