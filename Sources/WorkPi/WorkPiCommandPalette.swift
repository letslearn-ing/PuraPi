...
enum WorkPiCommandPaletteScrollPolicy {
...
/// Composer 输入 `/` 后显示的原生命令选择器。
struct WorkPiCommandPalette: View {
    ...
    let onSelect: (WorkPiCommandItem) -> Void

    @State private var firstVisibleIndex = 0

    private var filteredCommands: [WorkPiCommandItem] {
        WorkPiCommandCatalog.filtered(commands, query: query)
    }
            ...
            if filteredCommands.isEmpty {
                ...
                ScrollViewReader { proxy in
                    ScrollView {
                        ...
                        // 第六项调用 scrollTo 时无效。
                        VStack(spacing: WorkPiCommandPaletteScrollPolicy.rowSpacing) {
                            ForEach(Array(filteredCommands.enumerated()), id: \.element.id) { index, command in
                                Button {
                                    ...
                                    .frame(
                                        height: WorkPiCommandPaletteScrollPolicy.rowHeight,
                                        alignment: .center
                                    )
                                    .background(
                                        index == selectedIndex
                        ...
                        .padding(WorkPiCommandPaletteScrollPolicy.contentPadding)
                    }
                    .frame(height: WorkPiCommandPaletteScrollPolicy.viewportHeight)
    ...
    private func scrollToSelection(
        ...
        guard filteredCommands.indices.contains(index),
              let targetFirst = WorkPiCommandPaletteScrollPolicy.targetFirstVisibleIndex(
                  selectedIndex: index,
    ...
    func command(at index: Int) -> WorkPiCommandItem? {
