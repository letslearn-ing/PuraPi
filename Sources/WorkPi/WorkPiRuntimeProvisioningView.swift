...
private enum WorkPiRuntimeProvisioningPresentation {
    case compact
    case settings
}

/// Runtime 供应状态的可见入口。安装按钮只触发确认弹窗后的显式安装，不会在
/// View 出现时执行网络请求或 Shell；发现任务由 `WorkPiRuntimeProvisioner` 管理。
@MainActor
struct WorkPiRuntimeProvisioningView: View {
    @Environment(\.workPiTheme) private var theme
    @ObservedObject var provisioner: WorkPiRuntimeProvisioner
    let language: WorkPiInterfaceLanguage
    private let presentation: WorkPiRuntimeProvisioningPresentation
    private let showsManualChoice: Bool
    @State private var isConfirmingInstall = false

    init(
        provisioner: WorkPiRuntimeProvisioner,
    ...
    var body: some View {
        ...
        .alert(
            ...
            Text(
                isEnglish
                    ? "Pura Pi will install the pinned Pi version in its user-level Application Support directory. If needed, it will also download and verify a private Node.js copy. It will not replace an existing Pi or edit your shell profile."
                    : "Pura Pi 会把已验证版本安装到自己的用户级 Application Support 目录；必要时还会下载并校验私有 Node.js。不会覆盖已有 Pi，也不会修改 Shell 配置。"
            )
        }
    }
            ...
            if let installation = provisioner.installation {
                VStack(alignment: .leading, spacing: 5) {
                    ...
                    detailRow(
                        ...
                        value: isEnglish
                            ? (installation.source == .existing ? "Existing installation" : "Managed by Pura Pi")
    ...
    private var statusSubtitle: String {
        ...
        case .incompatible:
            return isEnglish
                ? "Pura Pi will leave the existing installation untouched and install a managed copy."
                : "Pura Pi 不会覆盖已有安装，而会准备一份独立的用户级版本。"
        case .installing:
    ...
    private func nodeSubtitle(_ node: WorkPiRuntimeNodeState) -> String {
        switch node {
        case .unknown:
            return isEnglish ? "Node.js status is not known yet." : "尚未取得 Node.js 状态。"
        case .missing:
            return isEnglish
                ? "Node.js 22.19.0+ will be prepared in Pura Pi's user directory if needed."
                : "如果需要，Pura Pi 会把 Node.js 22.19.0+ 准备到自己的用户目录。"
    ...
    private func phaseEnglishTitle(_ phase: WorkPiRuntimeInstallPhase) -> String {
        switch phase {
