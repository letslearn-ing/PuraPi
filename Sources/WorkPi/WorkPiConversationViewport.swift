...
struct WorkPiConversationViewport: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> WorkPiConversationViewportView {
        WorkPiConversationViewportView()
    }

    func updateNSView(
        _ nsView: WorkPiConversationViewportView,
        context: Context
    ) {
...
@MainActor
final class WorkPiConversationViewportView: NSView {
    private let scrollView = NSScrollView()
    private let documentView = WorkPiConversationDocumentView()
    private let scrollConfiguration: WorkPiScrollViewConfigurationView
    ...
    private var rowHosts: [UUID: WorkPiConversationRowHost] = [:]
    /// 行内文案（如「思考过程」）随语言变化，必须参与行的相等判定。
    private var language: WorkPiInterfaceLanguage = .chinese
    private var theme: WorkPiTheme = .default
    ...
    override init(frame frameRect: NSRect) {
        if #available(macOS 26.1, *) {
            scrollConfiguration = WorkPiScrollViewConfigurationView(
        ...
        // WorkPiScrollEdgeEffectView fallback。外层 viewport 仍负责裁剪到底部边界，
    ...
    private func requestNativeScrollEdgeAccessoryInstall() {
        guard bounds.width > 1,
              ...
              let nativeScrollContainer = nativeScrollContainer as? WorkPiNativeScrollEdgeContainerController
        else { return }
        ...
        if nativeScrollContainer.installScrollEdgeAccessory() {
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                guard self.window != nil,
                      let container = self.nativeScrollContainer as? WorkPiNativeScrollEdgeContainerController
                else { return }
    ...
    private func makeRowHost(for item: ConversationItem) -> WorkPiConversationRowHost {
        let host = WorkPiConversationRowHost(
            item: item,
            language: language,
            theme: theme
        )
...
struct WorkPiConversationRowContent: View, Equatable {
...
@MainActor
final class WorkPiConversationRowHost: NSHostingView<WorkPiConversationRowContent> {
    private(set) var item: ConversationItem
    private(set) var contentWidth: CGFloat = 1
    private(set) var language: WorkPiInterfaceLanguage
    private(set) var theme: WorkPiTheme
    ...
    init(
        ...
        self.theme = theme
        super.init(
            rootView: WorkPiConversationRowContent(
                item: item,
                contentWidth: 1,
                language: language,
                theme: theme
    ...
    required init(rootView: WorkPiConversationRowContent) {
...
final class WorkPiConversationDocumentView: NSView {
    ...
    private let horizontalInset = WorkPiConversationLayout.rowHorizontalInset
    private let maximumContentWidth = WorkPiConversationLayout.rowMaximumContentWidth
    ...
    func update(
        ...
        hosts: [UUID: WorkPiConversationRowHost]
    ) {
    ...
    override func layout() {
        ...
        let rowWidth = WorkPiConversationLayout.rowWidth(availableWidth: availableWidth)
        let rowX = WorkPiConversationLayout.rowLeadingOffset(availableWidth: availableWidth)
        var y: CGFloat = 0
