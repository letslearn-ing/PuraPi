...
extension WorkPiDefaultRuntimeProvisioningClient {
    func install(
        ...
        try Task.checkCancellation()
        let node: WorkPiRuntimeNodeInfo
        ...
        let environment = WorkPiRuntimeEnvironment.environment(
        ...
        let packageSpecifier = "\(WorkPiRuntimePolicy.piPackageName)@\(managedPiVersion)"
        ...
        do {
            result = try await runner.run(WorkPiProcessRequest(
                ...
                timeout: 900,
                outputLimit: WorkPiRuntimePolicy.maxProcessOutputBytes
            ))
        } catch {
            throw WorkPiRuntimeProvisioningError.commandFailed(
        ...
        guard fileManager.isExecutableFile(atPath: stagedExecutable.path) else {
            throw WorkPiRuntimeProvisioningError.verificationFailed(
        ...
        guard versionResult.succeeded,
              let installedVersion = WorkPiRuntimeVersion(versionResult.stdoutText),
    ...
    private func installManagedNode() async throws -> WorkPiRuntimeNodeInfo {
        guard let architecture = WorkPiRuntimeCandidatePaths.nodeArchitecture else {
            throw WorkPiRuntimeProvisioningError.unsupportedPlatform
        }
        let fileManager = FileManager.default
        ...
        let checksumRemote = WorkPiRuntimePolicy.nodeDownloadBaseURL
            .appendingPathComponent("SHASUMS256.txt")
        try await downloader.download(
            url: checksumRemote,
            to: checksumURL,
            maximumBytes: WorkPiRuntimePolicy.maxNodeChecksumBytes
        )
        guard let checksumData = WorkPiRuntimeBoundedFile.data(
            at: checksumURL,
            maximumBytes: WorkPiRuntimePolicy.maxNodeChecksumBytes
        ),
        ...
        let archiveRemote = WorkPiRuntimePolicy.nodeDownloadBaseURL
            .appendingPathComponent(artifact.fileName)
        try await downloader.download(
            url: archiveRemote,
            to: archiveURL,
            maximumBytes: WorkPiRuntimePolicy.maxNodeArchiveBytes
        )
        try WorkPiNodeArtifact.verify(archiveURL: archiveURL, expectedSHA256: artifact.sha256)
        try Task.checkCancellation()

        let listingResult = try await runner.run(WorkPiProcessRequest(
            executableURL: URL(fileURLWithPath: "/usr/bin/tar"),
            arguments: ["-tf", archiveURL.path],
            environment: WorkPiRuntimeEnvironment.environment(
                base: WorkPiRuntimeEnvironment.safeEnvironment(
                    base: environment,
                    homeDirectory: homeDirectory
                ),
        ...
        let tarResult = try await runner.run(WorkPiProcessRequest(
            executableURL: URL(fileURLWithPath: "/usr/bin/tar"),
        ...
        guard let node = managedStore.currentNode() else {
            throw WorkPiRuntimeProvisioningError.verificationFailed("Node.js 启用后无法再次读取。")
        }
        return node
    }

    private func validatedNodeDirectory(in extraction: URL, version: WorkPiRuntimeVersion) throws -> URL {
        let fileManager = FileManager.default
        ...
        guard directories.count == 1,
            ...
            throw WorkPiRuntimeProvisioningError.verificationFailed("Node.js 压缩包目录结构不符合预期。")
        }
        guard fileManager.isExecutableFile(
            atPath: directory.appendingPathComponent("bin/node").path
        ) else {
            throw WorkPiRuntimeProvisioningError.verificationFailed("Node.js 压缩包缺少可执行文件。")
        }
        return directory
    }
...
struct WorkPiNodeArtifact: Equatable, Sendable {
    ...
    static func parse(
        ...
        for line in checksumText.split(whereSeparator: \.isNewline) {
            ...
            guard let version = WorkPiRuntimeVersion(versionText),
...
enum WorkPiRuntimeCandidatePaths {
