...
struct WorkPiIncrementalMarkdownSnapshot: Equatable, Sendable {
    let stableBlocks: [WorkPiMarkdownBlock]
    let tailText: String
}
...
/// （列表、表格、引用）在最终排版上仍然正确。
struct WorkPiIncrementalMarkdownState: Sendable {
    private(set) var stableBlocks: [WorkPiMarkdownBlock] = []
    ...
    var snapshot: WorkPiIncrementalMarkdownSnapshot {
        WorkPiIncrementalMarkdownSnapshot(
    ...
    mutating func finish(_ source: String? = nil) {
        ...
        stableBlocks = reindex(WorkPiMarkdownParser.parse(rawText), startingAt: 0)
    ...
    private mutating func commitLines(through index: Int) {
        ...
        let parsed = WorkPiMarkdownParser.parse(source)
    ...
    private mutating func splitOverlongCurrentLineIfNeeded() {
        ...
        let parsed = WorkPiMarkdownParser.parse(committed)
        stableBlocks.append(contentsOf: reindex(parsed, startingAt: stableBlocks.count))
    }

    /// 稳定块的 id 必须全局递增，否则 SwiftUI 会因 identity 重复而复用错行。
    private func reindex(
        _ blocks: [WorkPiMarkdownBlock],
        startingAt offset: Int
    ) -> [WorkPiMarkdownBlock] {
        blocks.enumerated().map { index, block in
            WorkPiMarkdownBlock(id: offset + index, kind: block.kind)
        }
    }
...
struct WorkPiIncrementalMarkdownView: View {
    let text: String

    @State private var state: WorkPiIncrementalMarkdownState

    init(text: String) {
        self.text = text
        var initialState = WorkPiIncrementalMarkdownState()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if !state.tailText.isEmpty {
                WorkPiIncrementalMarkdownTailView(text: state.tailText)
            }
        }
        .onAppear {
            state.update(text)
...
private struct WorkPiIncrementalMarkdownTailView: View {
    let text: String

    var body: some View {
        let blocks = WorkPiMarkdownParser.parse(text)
        let hasUnclosedInlineSyntax = WorkPiIncrementalMarkdownSyntax.hasUnclosedInlineSyntax(text)
        ...
        if !hasUnclosedInlineSyntax, !blocks.isEmpty {
            ...
            Text(verbatim: WorkPiMarkdownSanitizer.inlinePreview(text))
...
private enum WorkPiIncrementalMarkdownSyntax {
