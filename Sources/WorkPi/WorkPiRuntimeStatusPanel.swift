...
@MainActor
struct WorkPiRuntimeStatusPanel: View {
    @Environment(\.workPiTheme) private var theme

    let snapshot: WorkPiRuntimeStatusSnapshot?
    let loading: Bool
    let error: String?
    let controlError: String?
    let autoRetryEnabled: Bool?
    let retryWait: WorkPiRetryWaitState?
    let turns: [WorkPiTurnRecord]
    ...
    private func stateGrid(_ snapshot: WorkPiRuntimeStatusSnapshot) -> some View {
    ...
    private func retryLabel(_ retry: WorkPiRetryWaitState) -> String {
        let attempt: String
    ...
    private func turnOutcomeLabel(_ outcome: WorkPiTurnOutcome) -> String {
        switch outcome {
