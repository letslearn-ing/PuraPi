...
extension WorkPiMarkdownEditorView {
    // MARK: - 编辑处理
    ...
    func handleChange(block: MarkdownBlock, displayText: String) {
        ...
        if block.kind == .paragraph,
           let insertion = WorkPiMarkdownBlockConverter.slashInsertion(displayText) {
            state.apply(.retype(
                id: block.id,
                kind: insertion.kind,
        ...
        if let converted = WorkPiMarkdownBlockConverter.convert(
            block: block,
        ...
        let source = WorkPiMarkdownBlockConverter.composeSource(
            kind: block.kind,
            displayText: displayText
        )
    ...
    func cursorPlacement(for blockID: UUID) -> WorkPiMarkdownCursorPlacement? {
    ...
    func selectAllMarkdown() {
        ...
        state.setSelection(
            anchor: WorkPiMarkdownSelectionEndpoint(blockID: first.id, offset: 0),
            focus: WorkPiMarkdownSelectionEndpoint(
                blockID: last.id,
    ...
    func replaceDocumentSelection(with replacement: String) -> Bool {
        ...
        let combinedSource = WorkPiMarkdownBlockConverter.composeSource(
    ...
    func extendSelection(
        ...
        let anchor: WorkPiMarkdownSelectionEndpoint
        let focus: WorkPiMarkdownSelectionEndpoint
        if let existing,
           existing.anchor.blockID == block.id || existing.focus.blockID == block.id {
            ...
            if direction < 0 {
                anchor = WorkPiMarkdownSelectionEndpoint(blockID: block.id, offset: end)
                focus = WorkPiMarkdownSelectionEndpoint(blockID: block.id, offset: start)
            } else {
                anchor = WorkPiMarkdownSelectionEndpoint(blockID: block.id, offset: start)
                focus = WorkPiMarkdownSelectionEndpoint(blockID: block.id, offset: end)
            }
        }
        ...
        if direction < 0 {
            ...
            state.setSelection(
                anchor: anchor,
                focus: WorkPiMarkdownSelectionEndpoint(blockID: target.id, offset: 0)
            )
        }
        return true
    }
        ...
        let dropTask = Task { @MainActor [weak editorState] in
            ...
            for provider in orderedProviders {
                ...
                let markdown = await Task.detached(priority: .utility) {
                    WorkPiMarkdownImageInsertion.markdown(for: url, workspaceRoot: root)
                }.value
    ...
    func handlePaste(
        ...
        for (index, pastedBlock) in pastedBlocks.enumerated() {
            ...
            pending.append(PendingBlock(
                kind: kind,
                source: WorkPiMarkdownBlockConverter.composeSource(
                    kind: kind,
                    displayText: displayText
                ),
    ...
    func handleSplit(block: MarkdownBlock, head: String, tail: String) {
        state.withUndoGroup {
            let headSource = WorkPiMarkdownBlockConverter.composeSource(
                kind: block.kind,
                displayText: head
            )
            let continuationKind = WorkPiMarkdownBlockConverter.continuationKind(for: block.kind)
            let tailSource = WorkPiMarkdownBlockConverter.composeSource(
            ...
            // 中间插空行反而会断开列表。
            if WorkPiMarkdownBlockConverter.requiresBlankSeparator(continuationKind) {
        ...
        state.withUndoGroup {
            if block.displayText.isEmpty {
                ...
                if let replacement {
                    ...
                    let placement: WorkPiMarkdownCursorPlacement =
