...
enum WorkPiRuntimeRequestPurpose: String, Hashable, Sendable {
    case bootstrap
    case refresh
    case rebuild
    case operation
...
struct WorkPiRuntimeRequest: Equatable, Sendable {
    /// 即使调用方错误地复用同一个 RPC id，每次登记仍有独立身份；旧 timeout
    /// 不能因结构体字段相同而结算新请求。
    let registrationID: UUID
    let id: String
    let command: String
    let purpose: WorkPiRuntimeRequestPurpose
    let generation: UUID
    let sessionEpoch: UUID
    let itemID: UUID?
    let queuedPrompt: WorkPiQueuedPrompt?
}
...
extension PiSessionController {
    ...
    func registerRuntimeRequest(
        ...
        purpose: WorkPiRuntimeRequestPurpose,
        itemID: UUID? = nil,
        queuedPrompt: WorkPiQueuedPrompt? = nil,
        ...
        let request = WorkPiRuntimeRequest(
            registrationID: UUID(),
            id: id,
    ...
    func runtimeRequestCandidates(
        for command: String,
        purposes: Set<WorkPiRuntimeRequestPurpose> = []
    ) -> [WorkPiRuntimeRequest] {
    ...
    @discardableResult
    func settleRuntimeRequest(_ request: WorkPiRuntimeRequest) -> Bool {
    ...
    func handleRuntimeRequestTimeout(_ request: WorkPiRuntimeRequest) {
    ...
    private func isCurrentSessionOperation(_ request: WorkPiRuntimeRequest) -> Bool {
        switch request.command {
