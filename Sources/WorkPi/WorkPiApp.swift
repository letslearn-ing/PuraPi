...
/// WorkPi 主窗口。双击透明标题栏的空白拖拽区时使用 AppKit 原生 Zoom，
/// 填满当前屏幕可用区域；再次双击由系统还原原窗口 frame。
@MainActor
final class WorkPiWindow: NSWindow {
...
@MainActor
final class WorkPiApplicationDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    private var window: NSWindow?
    private var toolbar: NSToolbar?
    private var toolbarDelegate: WorkPiToolbarDelegate?
    private var localEventMonitor: Any?
    private var appearanceWindow: NSWindow?
    private var appearanceCancellable: AnyCancellable?
    private var languageCancellable: AnyCancellable?
    private var openAccountSettingsObserver: NSObjectProtocol?
    private let runtimeProvisioner: WorkPiRuntimeProvisioner
    private let authCoordinator: WorkPiAuthCoordinator
    ...
    override init() {
        ...
        runtimeProvisioner = WorkPiRuntimeProvisioner(selection: selection)
        authCoordinator = WorkPiAuthCoordinator(selection: selection)
        tabs = WorkPiTabManager(makeTransport: { mode in
            let snapshot = selection.snapshot()
            let executableURL = snapshot.executableURL
                ?? (snapshot.automaticResolutionAllowed
                    ? nil
                    : WorkPiRuntimeSelection.unavailableExecutableURL)
        ...
        inspectorCoordinator = WorkPiInspectorWindowCoordinator(
            tabs: tabs,
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        openAccountSettingsObserver = NotificationCenter.default.addObserver(
            forName: .workPiOpenAccountSettings,
            object: nil,
            queue: .main
        ) { [weak self] _ in
        ...
        window.setFrameAutosaveName("WorkPi.mainWindow")

        let toolbarDelegate = WorkPiToolbarDelegate(
            manager: tabs,
            layoutState: layoutState,
            appearanceState: appearanceState
        )
        let toolbar = NSToolbar(identifier: "WorkPi.projectTabs")
    ...
    func applicationWillTerminate(_ notification: Notification) {
        ...
        // 防止 OAuth 回调服务器或 Node 子进程在 WorkPi 退出后继续运行。
    ...
    private func installKeyboardShortcutMonitor() {
        localEventMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
            guard let self, WorkPiKeyboardShortcut.isToggleSidebar(event) else { return event }
    ...
    @objc private func newWorkspaceFromMenu(_ sender: Any?) {
        NotificationCenter.default.post(name: .workPiNewWorkspace, object: nil)
    }
    ...
    @objc private func closeSelectedTabFromMenu(_ sender: Any?) {
        NotificationCenter.default.post(name: .workPiCloseSelectedTab, object: nil)
    }

    @objc private func closeAllProjectsFromMenu(_ sender: Any?) {
        NotificationCenter.default.post(name: .workPiCloseAllProjects, object: nil)
    }

    @objc private func saveDocumentFromMenu(_ sender: Any?) {
        NotificationCenter.default.post(name: .workPiSaveDocument, object: nil)
    }

    @objc private func findInConversationFromMenu(_ sender: Any?) {
        NotificationCenter.default.post(name: .workPiFindInConversation, object: nil)
    }

    @objc private func toggleSidebarFromMenu(_ sender: Any?) {
        NotificationCenter.default.post(name: .workPiToggleSidebar, object: nil)
    }

    @objc private func toggleInspectorDetachedFromMenu(_ sender: Any?) {
        NotificationCenter.default.post(name: .workPiToggleInspectorDetached, object: nil)
    }
    ...
    private func presentSettings(tab: WorkPiSettingsTab) {
        ...
        let settingsView = WorkPiAppearanceSettingsView(
            appearanceState: appearanceState,
            runtimeProvisioner: runtimeProvisioner,
            authCoordinator: authCoordinator,
            initialTab: tab
        ...
        settingsWindow.title = appearanceState.language == .english ? "Pura Pi Settings" : "Pura Pi 设置"
    ...
    private func scheduleWindowAppearance(_ mode: WorkPiAppearanceMode) {
        // Picker 的状态写入先完成，再统一更新两个 WorkPi 窗口。连续选择时只应用
    ...
    private func applyWindowAppearance(_ mode: WorkPiAppearanceMode) {
...
@MainActor
final class WorkPiToolbarDelegate: NSObject, NSToolbarDelegate {
    let manager: WorkPiTabManager
    let layoutState: WorkPiLayoutState
    let appearanceState: WorkPiAppearanceState
    let tabsItemIdentifier = NSToolbarItem.Identifier("WorkPi.projectTabs.item")
    /// Sidebar 文件树 / 会话树切换图标。
    ///
    /// 必须做成工具栏项：统一工具栏的 `NSToolbarView` 在视图层级上位于
    /// contentView 之上，直接把 SwiftUI 按钮顶进标题栏区域时点击会被它拦住。
    let sidebarModeItemIdentifier = NSToolbarItem.Identifier("WorkPi.sidebarMode.item")
    /// Sidebar 折叠开关。展开时贴 Sidebar 右上角，折叠后自动落到交通灯右侧。
    let sidebarToggleItemIdentifier = NSToolbarItem.Identifier("WorkPi.sidebarToggle.item")
    ...
    func toolbar(
        ...
        let width = WorkPiTitlebarTabs.toolbarWidth(for: manager)
        let hostingView = NSHostingView(
            rootView: WorkPiTitlebarTabs(
                manager: manager,
                appearanceState: appearanceState
            )
        )
        hostingView.frame = NSRect(x: 0, y: 0, width: width, height: WorkPiTitlebarTabs.tabBarHeight)
    ...
    private func makeSidebarModeItem() -> NSToolbarItem {
        ...
        let hostingView = NSHostingView(
            rootView: WorkPiSidebarModeSwitcher(
                layoutState: layoutState,
                appearanceState: appearanceState
            )
        )
        hostingView.frame = NSRect(
            x: 0,
            y: 0,
            width: WorkPiSidebarModeSwitcher.width,
            height: WorkPiSidebarModeSwitcher.height
        )
        hostingView.setContentHuggingPriority(.required, for: .horizontal)
        hostingView.setContentCompressionResistancePriority(.required, for: .horizontal)
        item.view = hostingView
    ...
    private func makeSidebarToggleItem() -> NSToolbarItem {
        ...
        hostingView.frame = NSRect(
            x: 0,
            y: 0,
            width: WorkPiSidebarToggle.width,
            height: WorkPiSidebarToggle.height
        )
        hostingView.setContentHuggingPriority(.required, for: .horizontal)
        hostingView.setContentCompressionResistancePriority(.required, for: .horizontal)
        item.view = hostingView
...
struct WorkPiMain {
    static func main() {
        ...
        let delegate = WorkPiApplicationDelegate()
