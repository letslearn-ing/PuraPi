...
enum WorkPiFirstLaunchStep: Equatable {
...
enum WorkPiFirstLaunchStepResolver {
    static func resolve(
        runtime: WorkPiRuntimeAvailability,
        authenticationLoaded: Bool,
        authenticationPhase: WorkPiAuthPhase,
        authentication: WorkPiAuthSnapshot
    ) -> WorkPiFirstLaunchStep {
        switch runtime {
...
@MainActor
struct WorkPiFirstLaunchWizard: View {
    ...
    private var step: WorkPiFirstLaunchStep {
        WorkPiFirstLaunchStepResolver.resolve(
    ...
    private var header: some View {
        VStack(spacing: 5) {
            Text(isEnglish ? "Set up Pura Pi" : "开始使用 Pura Pi")
                .font(.system(size: 22, weight: .semibold))
            Text(
                isEnglish
                    ? "Pura Pi will check the local Runtime first, then help you configure an account before opening a project."
                    : "Pura Pi 会先检查本机 Runtime，再帮助你配置账号，然后打开项目。"
            )
    ...
    private var projectActions: some View {
        WorkPiGlassContainer(spacing: 10) {
            VStack(spacing: 10) {
                HStack(spacing: 10) {
                    WorkPiEmptyActionButton(
