...
/// 因此实际鼠标命中代理由 `WorkPiWorkspaceSplitViewController` 放在 pane
/// 父视图上；这里集中保存它与 SwiftUI 标题栏必须一致的尺寸。
enum WorkPiInspectorHeaderMetrics {
...
@MainActor
final class WorkPiInspectorHeaderInteractionView: NSView {
    private let maximizeButton = WorkPiInspectorProxyButton()
    private let menuButton = WorkPiInspectorProxyButton()
    private let closeButton = WorkPiInspectorProxyButton()
    ...
    override func layout() {
        super.layout()
        let y = max(0, (bounds.height - WorkPiInspectorHeaderMetrics.buttonHeight) / 2)
        var x: CGFloat = 0
        ...
        if isDetached {
            ...
            maximizeButton.frame = NSRect(
                x: x,
                y: y,
                width: WorkPiInspectorHeaderMetrics.maximizeWidth,
                height: WorkPiInspectorHeaderMetrics.buttonHeight
            )
            x += WorkPiInspectorHeaderMetrics.maximizeWidth
                + WorkPiInspectorHeaderMetrics.buttonSpacing
        }

        menuButton.frame = NSRect(
            x: x,
            y: y,
            width: WorkPiInspectorHeaderMetrics.menuWidth,
            height: WorkPiInspectorHeaderMetrics.buttonHeight
        )
        x += WorkPiInspectorHeaderMetrics.menuWidth
            + WorkPiInspectorHeaderMetrics.buttonSpacing

        closeButton.frame = NSRect(
            x: x,
            y: y,
            width: WorkPiInspectorHeaderMetrics.closeWidth,
            height: WorkPiInspectorHeaderMetrics.buttonHeight
        )
    }
...
@MainActor
struct WorkPiInspectorHeaderInteractionRepresentable: NSViewRepresentable {
    let previewURL: URL?
    ...
    func makeNSView(context: Context) -> WorkPiInspectorHeaderInteractionView {
        let view = WorkPiInspectorHeaderInteractionView()
        view.update(
    ...
    func updateNSView(
        _ nsView: WorkPiInspectorHeaderInteractionView,
        context: Context
    ) {
        nsView.update(
            previewURL: previewURL,
...
@MainActor
final class WorkPiInspectorDetachInteractionView: NSView {
    private let button = WorkPiInspectorProxyButton()
    private var isEnglish = false
    private var onToggleDetached: () -> Void = {}
    ...
    override func layout() {
        super.layout()
        let height = WorkPiInspectorHeaderMetrics.buttonHeight
        button.frame = NSRect(
            x: 0,
...
@MainActor
struct WorkPiInspectorDetachInteractionRepresentable: NSViewRepresentable {
    let showsFileActions: Bool
    let isDetached: Bool
    let language: WorkPiInterfaceLanguage
    let onToggleDetached: () -> Void
    ...
    func makeNSView(context: Context) -> WorkPiInspectorDetachInteractionView {
        let view = WorkPiInspectorDetachInteractionView()
        view.update(
            showsFileActions: showsFileActions,
            isDetached: isDetached,
            language: language,
    ...
    func updateNSView(
        _ nsView: WorkPiInspectorDetachInteractionView,
        context: Context
    ) {
        nsView.update(
            showsFileActions: showsFileActions,
...
@MainActor
struct WorkPiInspectorHeaderInteractionStack: View {
    let previewURL: URL?
    let showsFileActions: Bool
    let isDetached: Bool
    let language: WorkPiInterfaceLanguage
    ...
    var body: some View {
        HStack(spacing: WorkPiInspectorHeaderMetrics.detachSpacing) {
            WorkPiInspectorDetachInteractionRepresentable(
                showsFileActions: showsFileActions,
                isDetached: isDetached,
                language: language,
                onToggleDetached: onToggleDetached
            ...
            .frame(
                ...
                height: WorkPiInspectorHeaderMetrics.totalHeight
            )
            WorkPiInspectorHeaderInteractionRepresentable(
                previewURL: previewURL,
                showsFileActions: showsFileActions,
                isDetached: isDetached,
                language: language,
...
@MainActor
private final class WorkPiInspectorProxyButton: NSButton {
    override var mouseDownCanMoveWindow: Bool { false }
