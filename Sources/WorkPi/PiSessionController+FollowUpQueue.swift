...
///
/// 队列由 WorkPi 维护，而不是立刻发 Pi 的 `follow_up`：Pi 的 RPC 接口没有
...
extension PiSessionController {
    ...
    func enqueueFollowUp(
        ...
        inspectorChange: WorkPiMarkdownDiffContext? = nil,
        inspectorChanges: [WorkPiMarkdownDiffContext]? = nil
    ) -> UUID? {
...
/// 队列中的一条待执行任务。
struct WorkPiQueuedPrompt: Identifiable, Equatable, Sendable {
    let id: UUID
    let text: String
    ...
    var inspectorChange: WorkPiMarkdownDiffContext? {
