...
extension PiSessionController {
    func submitPrompt() {
        ...
        if isAgentBusy,
           WorkPiCommandCatalog.slashQuery(for: text) == nil,
           WorkPiCommandCatalog.shellCommand(for: text) == nil {
            let inspectorChanges: [WorkPiMarkdownDiffContext]
    ...
    @discardableResult
    func submitPrompt(_ queuedPrompt: WorkPiQueuedPrompt) -> Bool {
        submitPromptText(
            queuedPrompt.text,
    ...
    func submitPromptText(
        _ rawText: String,
        attachments explicitAttachments: [WorkPiAttachment]? = nil,
        queuedPromptID: UUID? = nil,
        inspectorChange explicitInspectorChange: WorkPiMarkdownDiffContext? = nil,
        explicitInspectorChanges: [WorkPiMarkdownDiffContext]? = nil
    ) -> Bool {
        ...
        if let shellCommand = WorkPiCommandCatalog.shellCommand(for: text) {
        ...
        if let action = WorkPiCommandCatalog.action(for: text) {
            draftPrompt = ""
            let itemID = appendCommandActivity(text)
            executeWorkPiCommand(action, text: text, commandItemID: itemID)
            return true
        }

        if let discovered = WorkPiCommandCatalog.discoveredCommand(
            for: text,
        ...
        if let query = WorkPiCommandCatalog.slashQuery(for: text), !query.isEmpty {
            draftPrompt = ""
        ...
        if let queuedPromptID {
            activeQueuedPrompt = WorkPiQueuedPrompt(
                id: queuedPromptID,
                text: text,
