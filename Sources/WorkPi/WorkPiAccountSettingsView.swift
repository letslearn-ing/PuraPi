...
/// 账号与认证设置页。凭据由 Pi 官方 `AuthStorage` 保存，WorkPi 只展示非敏感状态。
@MainActor
struct WorkPiAccountSettingsView: View {
    @Environment(\.workPiTheme) private var theme
    @ObservedObject var coordinator: WorkPiAuthCoordinator
    ...
    @State private var loginType: WorkPiAuthType?
    ...
    private var statusArea: some View {
        ...
        if let event = coordinator.currentEvent {
            WorkPiAuthEventView(event: event, language: language)
        }
    ...
    private var storageNote: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(alignment: .top, spacing: 8) {
                ...
                Text(
                    isEnglish
                        ? "Pi stores credentials in its official auth storage (\(coordinator.authStorageDisplayPath)) with user-only permissions. Pura Pi never puts tokens or API keys in Session JSONL, project files, logs, or its own preferences."
                        : "Pi 会将凭据保存在官方认证存储中（\(coordinator.authStorageDisplayPath)），并使用仅用户可读写的权限。Pura Pi 不会把令牌或 API Key 写入 Session JSONL、项目文件、日志或 Pura Pi 偏好。"
                )
                .font(.system(size: 11))
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
    ...
    private func beginLogin(provider: WorkPiAuthProvider, type: WorkPiAuthType) {
...
@MainActor
private struct WorkPiAuthProviderCard: View {
    @Environment(\.workPiTheme) private var theme
    let provider: WorkPiAuthProvider
    let language: WorkPiInterfaceLanguage
    var compact = false
    let isDisabled: Bool
    let onLogin: (WorkPiAuthType) -> Void
    let onLogout: () -> Void
    ...
    private func methodTitle(_ method: WorkPiAuthMethod) -> String {
...
@MainActor
private struct WorkPiAuthEventView: View {
    @Environment(\.workPiTheme) private var theme
    let event: WorkPiAuthEvent
    let language: WorkPiInterfaceLanguage
...
@MainActor
private struct WorkPiAuthPromptSheet: View {
    @ObservedObject var coordinator: WorkPiAuthCoordinator
