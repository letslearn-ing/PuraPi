...
enum PuraPiSettingsTab: Hashable {
    case appearance
    case themes
    case runtime
    case accounts
...
/// PuraPi 的设置窗口，使用持久化状态控制外观、主题、Runtime 和账号认证。
@MainActor
struct PuraPiAppearanceSettingsView: View {
    ...
    @State private var selectedTab: PuraPiSettingsTab = .appearance

    init(
        appearanceState: PuraPiAppearanceState,
        runtimeProvisioner: PuraPiRuntimeProvisioner,
        authCoordinator: PuraPiAuthCoordinator,
        initialTab: PuraPiSettingsTab = .appearance
    ) {
    ...
    @MainActor
    init(appearanceState: PuraPiAppearanceState) {
        self.init(
            appearanceState: appearanceState,
            runtimeProvisioner: PuraPiRuntimeProvisioner()
        )
    ...
    var body: some View {
        TabView(selection: $selectedTab) {
            appearancePage
                .tag(PuraPiSettingsTab.appearance)
                .tabItem {
                    Label(
            ...
            PuraPiThemeSettingsView(appearanceState: appearanceState)
                .tag(PuraPiSettingsTab.themes)
                .tabItem {
                    Label(
            ...
            PuraPiAccountSettingsView(
                coordinator: authCoordinator,
                language: appearanceState.language
            )
            .tag(PuraPiSettingsTab.accounts)
            .tabItem {
                Label(
        ...
        .onReceive(NotificationCenter.default.publisher(for: .puraPiOpenAccountSettings)) { _ in
        ...
        Form {
            Section("外观") {
                Picker(
                    ...
                    ForEach(PuraPiAppearanceMode.allCases) { mode in
            ...
            Section("语言 / Language") {
                Picker(
                    ...
                    ForEach(PuraPiInterfaceLanguage.allCases) { language in
...
/// 主题选择页。主题定义是数据而不是可执行插件；外部 `.purapitheme` 包只会
/// 经过 `PuraPiThemeStore` 校验后加入列表。
struct PuraPiThemeSettingsView: View {
    ...
    private var externalThemes: [PuraPiTheme] {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                VStack(alignment: .leading, spacing: 5) {
                    ...
                    Text(
                        isEnglish
                            ? "Themes change semantic colors while preserving PuraPi's layout and interactions."
                            : "主题只改变语义颜色和视觉令牌，不改变 PuraPi 的布局与交互。"
                    )
                    .font(.system(size: 11.5))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text(
                        isEnglish
                            ? "Packages contain data and assets only. PuraPi validates their schema, paths, sizes, permissions, and optional checksums; packages cannot access sessions, files, commands, or security settings."
                            : "主题包只包含数据和资源。PuraPi 会校验 schema、路径、大小、权限和可选校验和；主题包不能访问会话、文件、命令或安全设置。"
                    )
                    .font(.system(size: 11.5))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
...
private struct PuraPiThemeCard: View {
...
private struct PuraPiPaneTintPicker: View {
    @Environment(\.puraPiTheme) private var theme

    let title: String
    let selection: PuraPiPaneTint
    let language: PuraPiInterfaceLanguage
    let onSelect: (PuraPiPaneTint) -> Void
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(alignment: .center, spacing: 12) {
                LazyVGrid(columns: columns, alignment: .leading, spacing: 8) {
                    ForEach(PuraPiPaneTint.allCases) { tint in
                        Button {
                            onSelect(tint)
                        } label: {
                            swatch(for: tint)
    ...
    @ViewBuilder
    private func swatch(for tint: PuraPiPaneTint) -> some View {
        ZStack {
...
/// 决定，必须可见且可逆。
struct PuraPiTrustedProjectsSettings: View {
    @Environment(\.puraPiTheme) private var theme
    @State private var paths: [String] = PuraPiProjectAuthorization.rememberedProjectPaths()
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            paths = PuraPiProjectAuthorization.rememberedProjectPaths()
        }
    }

    private func revoke(_ path: String) {
        PuraPiProjectAuthorization.forget(URL(fileURLWithPath: path))
        paths = PuraPiProjectAuthorization.rememberedProjectPaths()
    }
