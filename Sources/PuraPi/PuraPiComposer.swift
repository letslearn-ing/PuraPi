...
struct Composer: View {
    ...
    var body: some View {
        PuraPiGlassContainer(spacing: 7) {
            VStack(alignment: .leading, spacing: 7) {
                if isCommandPaletteVisible {
                    PuraPiCommandPalette(
                        commands: commands,
                        query: commandQuery,
    ...
    private var isShellCommandMode: Bool {
        PuraPiCommandCatalog.isShellCommandMode(text)
    }

    private var inputSurface: some View {
        VStack(alignment: .leading, spacing: 7) {
            if !attachments.isEmpty {
                PuraPiAttachmentList(
    ...
    private func updateCommandPalette(for value: String) {
        guard let query = PuraPiCommandCatalog.slashQuery(for: value) else {
        ...
        commandQuery = query
        guard !PuraPiCommandCatalog.hasCommandArgument(value) else {
            isCommandPaletteVisible = false
            selectedCommandIndex = 0
            return
        }
        ...
        let filteredCount = PuraPiCommandCatalog.filtered(commands, query: query).count
        selectedCommandIndex = min(selectedCommandIndex, max(filteredCount - 1, 0))
    }

    private func selectedCommand() -> PuraPiCommandItem? {
        let filtered = PuraPiCommandCatalog.filtered(commands, query: commandQuery)
        guard filtered.indices.contains(selectedCommandIndex) else { return filtered.first }
        return filtered[selectedCommandIndex]
    }

    private func insertCommand(_ command: PuraPiCommandItem) {
    ...
    private func handleCommandKeyPress(_ keyPress: KeyPress) -> KeyPress.Result {
        ...
        switch keyPress.key {
        case .upArrow:
            let count = PuraPiCommandCatalog.filtered(commands, query: commandQuery).count
            selectedCommandIndex = PuraPiCommandCatalog.movedSelection(
    ...
    private func submitCommand(_ command: PuraPiCommandItem) {
