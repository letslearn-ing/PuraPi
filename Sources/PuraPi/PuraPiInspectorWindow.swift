...
@MainActor
final class PuraPiInspectorWindowCoordinator {
    ...
    private var windows: [UUID: PuraPiInspectorWindowController] = [:]
    ...
    func isDetached(for tab: PuraPiProjectTab) -> Bool {
    ...
    func detachedWindow(for tab: PuraPiProjectTab) -> NSWindow? {
        windows[tab.id]?.window
    }

    /// 切换当前标签的 Inspector 展示位置。
    func toggle(tab: PuraPiProjectTab) {
    ...
    func requestDetach(tab: PuraPiProjectTab) {
    ...
    func detach(tab: PuraPiProjectTab) {
        guard !isShuttingDown,
              tabs.tabs.contains(where: { $0.id == tab.id }),
              tab.session.selectedFileURL != nil
        else { return }
    ...
    private func detach(
        ...
        let tabID = tab.id
        let controller = PuraPiInspectorWindowController(
            tabID: tabID,
            tabTitle: tab.title,
            session: tab.session,
    ...
    /// Markdown 保存闸门。
    func reattach(tab: PuraPiProjectTab) {
    ...
    func close(tab: PuraPiProjectTab) {
        let tabID = tab.id
        cancelLiftHint(for: tabID)
        pendingSourceFrames.removeValue(forKey: tabID)
        if let controller = windows.removeValue(forKey: tabID) {
    ...
    func applyWindowAppearance(_ mode: PuraPiAppearanceMode) {
    ...
    private func inspectorSourceFrame(for tab: PuraPiProjectTab) -> NSRect? {
    ...
    ) -> PuraPiWorkspaceSplitViewController? {
        var responder: NSResponder? = view
        while let current = responder {
            if let controller = current as? PuraPiWorkspaceSplitViewController,
    ...
    private func reconcile(tabs: [PuraPiProjectTab]) {
    ...
    private func tab(withID id: UUID) -> PuraPiProjectTab? {
        tabs.tabs.first(where: { $0.id == id })
    }

    private func rememberFrame(
        of controller: PuraPiInspectorWindowController,
        for tabID: UUID
    ) {
...
@MainActor
final class PuraPiInspectorWindowController: NSWindowController, NSWindowDelegate {
    let tabID: UUID
    ...
    private let layoutState: PuraPiLayoutState
    private let appearanceState: PuraPiAppearanceState
    private let hostingController: NSHostingController<PuraPiDetachedInspectorView>
    ...
    init(
        ...
        self.hostingController = NSHostingController(
            rootView: PuraPiDetachedInspectorView(
                session: session,
                layoutState: layoutState,
                appearanceState: appearanceState,
...
/// 独立窗口中的 Inspector 内容。
///
/// 它与主栏使用同一个 `PiSessionController` 和 `PuraPiMarkdownEditorState`，因此
/// 在两个窗口之间切换时，Markdown 光标、撤销栈、冲突状态和预览请求都保持一致。
@MainActor
struct PuraPiDetachedInspectorView: View {
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            ...
            PuraPiInspectorHeaderInteractionStack(
            ...
            .padding(.top, PuraPiLayoutState.inspectorInset)
            .padding(
                .trailing,
                PuraPiLayoutState.inspectorInset + PuraPiInspectorHeaderMetrics.trailingPadding
            )
        }
