...
struct PuraPiMarkdownDocumentView: View {
    let blocks: [PuraPiMarkdownBlock]
...
struct PuraPiMarkdownBlockView: View {
    let block: PuraPiMarkdownBlock

    var body: some View {
        switch block.kind {
        case .heading(let level, let text):
            PuraPiMarkdownInlineText(
                text: text,
        ...
        case .paragraph(let text, let continuation):
            PuraPiMarkdownInlineText(
                text: text,
                font: .system(size: PuraPiMarkdownTypography.bodySize),
                color: .primary,
                lineSpacing: PuraPiMarkdownTypography.bodyLineSpacing
            )
            .padding(
                .top,
                block.id == 0 || continuation ? 0 : PuraPiMarkdownTypography.paragraphSpacing
            )

        case .unorderedList(let values):
            VStack(alignment: .leading, spacing: PuraPiMarkdownTypography.listRowSpacing) {
                ForEach(Array(values.enumerated()), id: \.offset) { _, value in
                    PuraPiMarkdownListRow(marker: "•", text: value)
                }
            }
        ...
        case .orderedList(let values):
            VStack(alignment: .leading, spacing: PuraPiMarkdownTypography.listRowSpacing) {
                ForEach(Array(values.enumerated()), id: \.offset) { index, value in
                    PuraPiMarkdownListRow(marker: "\(index + 1).", text: value)
                }
            }
            .padding(.top, topSpacing(for: .list))

        case .table(let headers, let rows):
            PuraPiMarkdownTableView(headers: headers, rows: rows)
        ...
        case .code(let language, let text):
            PuraPiMarkdownCodeBlock(language: language, text: text)
                .padding(.top, topSpacing(for: .regular))
    ...
    private func topSpacing(for kind: SpacingKind) -> CGFloat {
        ...
        case .list:
            return PuraPiMarkdownTypography.compactBlockSpacing
        case .regular:
            return PuraPiMarkdownTypography.blockSpacing
        }
    }
...
private struct PuraPiMarkdownCodeBlock: View {
    let language: String?
    let text: String
...
private struct PuraPiMarkdownTableView: View {
    let headers: [String]
    let rows: [[String]]

    var body: some View {
        Grid(alignment: .topLeading, horizontalSpacing: 0, verticalSpacing: 0) {
            PuraPiMarkdownTableRow(values: headers, emphasized: true)
            Divider()
            ForEach(Array(rows.enumerated()), id: \.offset) { index, values in
                PuraPiMarkdownTableRow(values: values, emphasized: false)
...
private struct PuraPiMarkdownTableRow: View {
    let values: [String]
    let emphasized: Bool
...
private struct PuraPiMarkdownListRow: View {
    let marker: String
    let text: String
...
struct PuraPiMarkdownInlineText: View {
    ...
    private var attributedText: AttributedString {
        PuraPiMarkdownInlineRenderer.render(text, accent: theme.accent)
    }
}

@MainActor
enum PuraPiMarkdownInlineRenderer {
    static func render(
        _ source: String,
        accent: Color = PuraPiTheme.default.accent
    ) -> AttributedString {
        ...
        guard var value = try? AttributedString(markdown: source, options: options) else {
            return AttributedString(PuraPiMarkdownSanitizer.inlinePreview(source))
        }
