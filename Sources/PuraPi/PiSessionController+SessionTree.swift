    ...
    func switchToSession(_ summary: PiSessionSummary) -> Bool {
        ...
        Task { [weak self] in
            do {
                ...
                self.lastError = "无法切换会话：\(PuraPiSensitiveText.redacted(error.localizedDescription))"
            }
        }
        return true
    }
    ...
    func forkSession(from turn: PiSessionTurn) {
        ...
        Task { [weak self] in
            do {
                ...
                self.lastError = "无法分叉会话：\(PuraPiSensitiveText.redacted(error.localizedDescription))"
            }
        }
    }
    ...
    func cloneCurrentSession() {
        ...
        Task { [weak self] in
            do {
                ...
                self.lastError = "无法复制会话：\(PuraPiSensitiveText.redacted(error.localizedDescription))"
            }
        }
    }
    ...
    func renameCurrentSession(to name: String) {
        ...
        Task { [weak self] in
            do {
                ...
                self.lastError = "无法重命名会话：\(PuraPiSensitiveText.redacted(error.localizedDescription))"
            }
        }
    }
        ...
        sessionRebuildTask = Task { [weak self] in
            do {
                ...
                self.failSessionRebuild(
                    message: "无法重建会话内容：\(PuraPiSensitiveText.redacted(error.localizedDescription))"
                )
            }
        }
        reloadSessionList()
