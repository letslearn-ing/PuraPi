...
@MainActor
extension PuraPiWorkspaceSplitViewController {
    ...
    func measureSidebarWidth() {
        ...
        let clampedWidth = min(
            max(measuredWidth, PuraPiLayoutState.minimumSidebarWidth),
            PuraPiLayoutState.maximumSidebarWidth
        )
    ...
    func measureInspectorWidth() {
        ...
        let clampedWidth = min(
            max(measuredWidth, PuraPiLayoutState.minimumInspectorWidth),
    ...
    func visibleInset(ofPaneAt index: Int) -> CGFloat {
        ...
        if item === sidebarItem { return PuraPiLayoutState.sidebarInset }
        if item === inspectorItem { return PuraPiLayoutState.inspectorInset }
        return 0
    }
    ...
    func installDividerWindowDragBlocker() {
        ...
        let blocker = PuraPiDividerWindowDragBlocker(frame: parent.bounds)
    ...
    func updateInspectorThicknessLimit() {
        ...
        guard !layoutState.inspectorMaximized else {
            inspectorItem.maximumThickness = PuraPiLayoutState.maximumInspectorWidth
            return
        }
        ...
        let limit = min(
            PuraPiLayoutState.maximumInspectorWidth + inspectorSlotPadding,
            max(PuraPiLayoutState.minimumInspectorWidth + inspectorSlotPadding, available)
        )
