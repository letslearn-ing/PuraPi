...
extension Notification.Name {
    static let puraPiEditMessage = Notification.Name("PuraPi.editMessage")
    static let puraPiRetryMessage = Notification.Name("PuraPi.retryMessage")
}
...
/// 常显按钮会让长对话显得嘈杂。
struct PuraPiMessageActionBar: View {
    ...
    private var copyButton: some View {
        actionButton(
            ...
            PuraPiMessageActions.copy(text)
            // 剪贴板没有系统反馈，给一个短暂确认。
            didCopy = true
            Task {
    ...
    private var retryButton: some View {
        actionButton(
            ...
            PuraPiMessageActions.retry(text)
        }
    }
    ...
    private var editButton: some View {
        actionButton(
            ...
            PuraPiMessageActions.edit(text)
        }
    }
...
/// 给消息行挂上操作条。
struct PuraPiMessageActionsModifier: ViewModifier {
    let text: String
    let createdAt: Date
    let allowsRetry: Bool
