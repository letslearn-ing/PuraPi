...
struct FileInspectorPane: View {
    @Environment(\.puraPiInspectorTint) private var inspectorTint
    ...
    @ObservedObject var editorState: PuraPiMarkdownEditorState
    ...
    var body: some View {
        ...
        .frame(maxWidth: .infinity, maxHeight: .infinity)
        .puraPiInspectorSurface(enabled: providesOwnSurface, tint: inspectorTint)
        // 仅在父容器已经提供圆角外壳的嵌入场景补主题色薄膜；原生 Inspector
        // 自己拥有完整表面，不重复叠加材质。
        .puraPiInspectorAccentWash(enabled: !providesOwnSurface, tint: inspectorTint)
        .overlay {
            if isLiftHintActive {
                InspectorLiftEffect()
...
private struct InspectorLiftEffect: View {
    ...
    var body: some View {
        ...
        RoundedRectangle(
            cornerRadius: PuraPiLayoutState.chromeCornerRadius,
            style: .continuous
        )
        .stroke(
...
private struct FileInspectorHeader: View {
    let preview: FilePreview
    /// 编辑器的保存状态；只读预览时为 nil。
    let saveState: PuraPiMarkdownEditorState.SaveState?
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Spacer(minLength: 4)
            if let saveState {
                PuraPiSaveStateBadge(state: saveState, language: language)
            }
        ...
        .padding(.horizontal, PuraPiInspectorHeaderMetrics.horizontalPadding)
        // 标题栏内容需要与中间项目标签的文字基线对齐；上下采用对称的 6pt
        // 间距，让标题到圆角表面上边缘和到下方分隔线的距离保持一致。
        .padding(.top, PuraPiInspectorHeaderMetrics.verticalPadding)
        .padding(.bottom, PuraPiInspectorHeaderMetrics.verticalPadding)
        .contextMenu {
            Button("复制绝对路径") { onCopyPath(preview.url) }
            Button("复制相对路径") { onCopyRelativePath(preview.url) }
            Divider()
            ...
            Group {
                ...
                case .markdown:
                    if let text = preview.text {
                        PuraPiMarkdownMessageView(text: text, isStreaming: false)
                            .font(.system(size: 14))
...
@MainActor
struct PuraPiSaveStateBadge: View {
    @Environment(\.puraPiTheme) private var theme

    let state: PuraPiMarkdownEditorState.SaveState
    let language: PuraPiInterfaceLanguage
...
@MainActor
struct PuraPiInspectorTopInsetView: View {
    let preview: FilePreview
    @ObservedObject var editorState: PuraPiMarkdownEditorState
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Color.clear
                .frame(height: PuraPiInspectorHeaderMetrics.separatorHeight)
        }
        ...
        .zIndex(1)
        .frame(height: PuraPiInspectorHeaderMetrics.totalHeight, alignment: .top)
