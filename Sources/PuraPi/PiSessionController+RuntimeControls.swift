...
extension PiSessionController {
    ...
    func requestRuntimeStatus() {
        ...
        Task { [weak self] in
            do {
                ...
                self.runtimeStatusPanelError = PuraPiSensitiveText.redacted(
    ...
    func setAutoRetry(enabled: Bool) {
        ...
        Task { [weak self] in
            do {
                ...
                self.runtimeControlError = PuraPiSensitiveText.redacted(
    ...
    func consumeRuntimeStatusResponse(_ record: PiRPCRecord) -> Bool {
        ...
        guard record.success == true else {
            runtimeStatusPanelError = PuraPiSensitiveText.redacted(
                record.string(at: "error") ?? "Runtime 状态获取失败。"
            )
            return true
        }
        runtimeStatusSnapshot = PuraPiRuntimeStatusSnapshot(
