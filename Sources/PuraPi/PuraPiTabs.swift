...
@MainActor
final class PuraPiProjectTab: ObservableObject, Identifiable {
    let id = UUID()
    let rootURL: URL
...
@MainActor
final class PuraPiTabManager: ObservableObject {
    @Published private(set) var tabs: [PuraPiProjectTab] = []
    ...
    var selectedTab: PuraPiProjectTab? {
    ...
    func openProject(at url: URL) {
        ...
        let tab = PuraPiProjectTab(rootURL: normalized, makeTransport: makeTransport)
        tabs.append(tab)
        selectedTabID = tab.id
    }

    func continueRecentSession(for tab: PuraPiProjectTab? = nil) {
    ...
    func select(_ tab: PuraPiProjectTab) {
        guard tabs.contains(where: { $0.id == tab.id }) else { return }
        selectedTabID = tab.id
    }

    func close(_ tab: PuraPiProjectTab) {
    ...
    private func closeSynchronously(_ tab: PuraPiProjectTab) {
    ...
    @discardableResult
    func closeAsync(_ tab: PuraPiProjectTab) async -> Bool {
    ...
    private func performCloseAsync(_ tab: PuraPiProjectTab) async -> Bool {
