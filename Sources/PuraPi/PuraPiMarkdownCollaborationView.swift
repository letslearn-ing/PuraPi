...
/// 混入普通 Prompt。
@MainActor
struct PuraPiMarkdownCollaborationBanner: View {
    @Environment(\.puraPiTheme) private var theme

    let state: PuraPiMarkdownCollaborationDisplayState
            ...
            ForEach(state.items) { item in
                HStack(spacing: 7) {
                    ...
                    if item.hasQueuedChange {
                        ...
                        PuraPiGlassButton(prominent: true, action: {
                            onReview(item.id)
                        }) {
...
@MainActor
struct PuraPiMarkdownDiffReviewSheet: View {
    @Environment(\.dismiss) private var dismiss

    let review: PuraPiMarkdownDiffReview
    let language: PuraPiInterfaceLanguage
    let onApprove: () -> Void

    private var isEnglish: Bool { language == .english }
    private var envelope: PuraPiMarkdownDiffEnvelope { review.context.envelope }
