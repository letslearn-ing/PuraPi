...
@MainActor
final class PuraPiNativeScrollEdgeContainerController: NSSplitViewController {
    ...
    func installScrollEdgeAccessory() -> Bool {
        ...
        scrollItem.addTopAlignedAccessoryViewController(
            PuraPiNativeScrollEdgeAccessoryViewController(
...
@available(macOS 26.1, *)
@MainActor
private final class PuraPiNativeScrollEdgeAccessoryView: NSView {
    let height: CGFloat
...
@available(macOS 26.1, *)
@MainActor
private final class PuraPiNativeScrollEdgeAccessoryViewController: NSSplitViewItemAccessoryViewController {
    init(width: CGFloat) {
        super.init(nibName: nil, bundle: nil)
        ...
        let accessoryView = PuraPiNativeScrollEdgeAccessoryView(
...
/// `PuraPiScrollEdgeEffectView` 负责桥接。旧系统由显式 attachment 提供兼容效果。
extension View {
    @ViewBuilder
    func puraPiTopScrollEdgeEffect() -> some View {
    ...
    @ViewBuilder
    func puraPiSidebarScrollEdgeEffect(enabled: Bool) -> some View {
        if enabled {
    ...
    @ViewBuilder
    func puraPiTopOnlyScrollEdgeEffect() -> some View {
        if #available(macOS 26.0, *) {
            scrollEdgeEffectStyle(.soft, for: .top)
        } else {
            self
...
struct PuraPiLegacyScrollEdgeEffectMarker: View {
    var body: some View {
        if #available(macOS 26.0, *) {
            Color.clear
                .frame(width: 0, height: 0)
        } else {
            PuraPiScrollEdgeEffectAttachment()
                .frame(width: 0, height: 0)
...
/// inset、滚动条或文档尺寸。
struct PuraPiScrollEdgeEffectAttachment: NSViewRepresentable {
    var enabled: Bool = true

    func makeNSView(context: Context) -> PuraPiScrollEdgeEffectAttachmentView {
        PuraPiScrollEdgeEffectAttachmentView(enabled: enabled)
    }

    func updateNSView(
        _ nsView: PuraPiScrollEdgeEffectAttachmentView,
        context: Context
    ) {
...
final class PuraPiScrollEdgeEffectAttachmentView: NSView {
    private var enabled: Bool
    private weak var attachedScrollView: NSScrollView?
    private var effectView: PuraPiScrollEdgeEffectView?
    ...
    func attachWhenPossible() {
        ...
        let effect = PuraPiScrollEdgeEffectView()
        effect.autoresizingMask = [.width]
        effect.frame = NSRect(
            x: 0,
            y: 0,
            width: scrollView.bounds.width,
            height: PuraPiScrollEdgeEffectView.height
        )
...
struct PuraPiScrollViewConfiguration: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> PuraPiScrollViewConfigurationView {
        PuraPiScrollViewConfigurationView(
    ...
    func updateNSView(
        _ nsView: PuraPiScrollViewConfigurationView,
        context: Context
    ) {
        nsView.update(
            managesTitlebarContentInsets: managesTitlebarContentInsets,
...
final class PuraPiScrollViewConfigurationView: NSView {
    ...
    private var scrollEdgeEffect: PuraPiScrollEdgeEffectView?
...
struct PuraPiEmptyActionButton: View {
...
private struct PuraPiEmptyActionButtonStyle: ButtonStyle {
    let prominent: Bool
    let isHovering: Bool
    let accent: Color
