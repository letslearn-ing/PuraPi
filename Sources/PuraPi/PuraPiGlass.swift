...
enum PuraPiMaterialRole: Equatable {
    case titlebar
    case sidebar
    case inspector
    case glass
...
/// 而不是一个单独的灰色圆角背景视图。
struct PuraPiGlassSurfaceModifier: ViewModifier {
    @Environment(\.puraPiTheme) private var theme

    let role: PuraPiMaterialRole
...
extension View {
    func puraPiGlassSurface(
        role: PuraPiMaterialRole,
        cornerRadius: CGFloat,
        interactive: Bool = false,
        tint: Color? = nil
    ) -> some View {
        modifier(
            PuraPiGlassSurfaceModifier(
                role: role,
                cornerRadius: cornerRadius,
...
struct PuraPiGlassContainer<Content: View>: View {
...
private struct PuraPiLegacyMaterialSurface: NSViewRepresentable {
    let material: NSVisualEffectView.Material
    let blendingMode: NSVisualEffectView.BlendingMode
    let emphasized: Bool
    let cornerRadius: CGFloat

    func makeNSView(context: Context) -> PuraPiLegacyMaterialHostView {
        PuraPiLegacyMaterialHostView(
            material: material,
    ...
    func updateNSView(_ nsView: PuraPiLegacyMaterialHostView, context: Context) {
        nsView.update(
            material: material,
            blendingMode: blendingMode,
            emphasized: emphasized,
...
private final class PuraPiLegacyMaterialHostView: NSVisualEffectView {
...
struct PuraPiGlassButton<Label: View>: View {
    @Environment(\.puraPiTheme) private var theme
...
private struct PuraPiLegacyButtonStyle: ButtonStyle {
    let prominent: Bool
    let accent: Color
    let cornerRadius: CGFloat
...
struct PuraPiWindowBackdrop: View {
    @Environment(\.puraPiTheme) private var theme
...
/// 旧系统继续使用稳定的不透明颜色。
struct PuraPiAdaptiveContentBackground: View {
    @Environment(\.puraPiTheme) private var theme
...
private struct PuraPiSidebarTintKey: EnvironmentKey {
    static let defaultValue: PuraPiPaneTint = .purple
}

private struct PuraPiInspectorTintKey: EnvironmentKey {
    static let defaultValue: PuraPiPaneTint = .purple
}

extension EnvironmentValues {
    var puraPiSidebarTint: PuraPiPaneTint {
        get { self[PuraPiSidebarTintKey.self] }
        set { self[PuraPiSidebarTintKey.self] = newValue }
    }

    var puraPiInspectorTint: PuraPiPaneTint {
        get { self[PuraPiInspectorTintKey.self] }
        set { self[PuraPiInspectorTintKey.self] = newValue }
    }
}

extension View {
    /// 把两栏主题色注入到子树。
    func puraPiPaneTints(
        sidebar: PuraPiPaneTint,
        inspector: PuraPiPaneTint
    ) -> some View {
        environment(\.puraPiSidebarTint, sidebar)
            .environment(\.puraPiInspectorTint, inspector)
    }
}

// 所有主题颜色都通过 `PuraPiTheme` 提供；不再保留散落在 `Color` 命名空间里的
// 视觉常量，避免第三方主题只能覆盖部分组件。

extension View {
    /// Inspector 的悬浮圆角表面。
    ///
    /// 与 `puraPiSidebarSurface` 共用同一套圆角与材质。颜色由设置状态决定；
    /// Inspector 选择“无颜色”时走中性底，不创建系统材质，避免出现蓝色底光。
    func puraPiInspectorSurface(
        enabled: Bool = true,
        tint: PuraPiPaneTint = .purple
    ) -> some View {
        modifier(
            PuraPiInspectorSurfaceModifier(
                enabled: enabled,
                tint: tint
            )
        )
    ...
    /// 现在直接使用自己的 `puraPiInspectorSurface`，因此不会重复叠加这层材质。
    func puraPiInspectorAccentWash(
        enabled: Bool = true,
        tint: PuraPiPaneTint = .purple
    ) -> some View {
        modifier(
            PuraPiInspectorAccentWashModifier(
                enabled: enabled,
                tint: tint
            )
        )
    ...
    func puraPiSidebarSurface(
        enabled: Bool = true,
        tint: PuraPiPaneTint = .purple
    ) -> some View {
        modifier(
            PuraPiSidebarSurfaceModifier(
                enabled: enabled,
                tint: tint
            )
        )
...
private struct PuraPiInspectorSurfaceModifier: ViewModifier {
    @Environment(\.puraPiTheme) private var theme

    let enabled: Bool
    let tint: PuraPiPaneTint
...
private struct PuraPiInspectorAccentWashModifier: ViewModifier {
    @Environment(\.puraPiTheme) private var theme
...
private struct PuraPiSidebarSurfaceModifier: ViewModifier {
    @Environment(\.puraPiTheme) private var theme
...
private struct PuraPiPresentationGlassChromeModifier: ViewModifier {
    @Environment(\.puraPiTheme) private var theme
...
extension View {
    ...
    func puraPiPresentationGlassChrome() -> some View {
        modifier(PuraPiPresentationGlassChromeModifier())
    }
}
