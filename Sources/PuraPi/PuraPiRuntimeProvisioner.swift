...
/// PuraPi 自己的用户级目录，不覆盖 PATH 中已有的 Pi，也不修改 Shell 配置。
@MainActor
final class PuraPiRuntimeProvisioner: ObservableObject {
    @Published private(set) var availability: PuraPiRuntimeAvailability = .checking
    @Published private(set) var diagnostics: [String] = []
    @Published private(set) var installPhase: PuraPiRuntimeInstallPhase?
    @Published private(set) var selectedExecutableURL: URL?
    @Published private(set) var discoveredNodeState: PuraPiRuntimeNodeState = .unknown
    ...
    private let client: any PuraPiRuntimeProvisioningClient
    ...
    init(
        ...
        client: (any PuraPiRuntimeProvisioningClient)? = nil
    ) {
        self.locations = locations
        self.selection = selection
        self.defaults = defaults
        let preferredExecutableURL = defaults.string(forKey: PuraPiPreferences.Key.runtimeExecutablePath)
            .map { URL(fileURLWithPath: $0) }
        self.selectedExecutableURL = preferredExecutableURL
        self.client = client ?? PuraPiDefaultRuntimeProvisioningClient(
            locations: locations,
    ...
    var nodeState: PuraPiRuntimeNodeState {
        ...
        case .available(let installation):
            if let nodeURL = installation.nodeURL {
                return .available(PuraPiRuntimeNodeInfo(
                    nodeURL: nodeURL,
                    npmURL: nil,
    ...
    func refresh() {
        ...
        discoveryTask = Task { [weak self, client] in
            ...
            if let installation = snapshot.installation {
                if installation.version >= PuraPiRuntimePolicy.minimumSupportedPiVersion {
    ...
    func chooseExecutable(language: PuraPiInterfaceLanguage = .chinese) {
        guard installTask == nil else { return }
        let panel = NSOpenPanel()
        let isEnglish = language == .english
        panel.title = isEnglish ? "Choose Pi executable" : "选择 Pi 可执行文件"
        panel.message = isEnglish
            ? "Choose an installed pi file. PuraPi will only run --version to verify it."
            : "选择已安装的 pi 文件；PuraPi 只会运行它的 --version 进行验证。"
    ...
    func install() {
        ...
        installTask = Task { [weak self, client] in
            ...
            do {
                let progress = AsyncStream<PuraPiRuntimeInstallPhase>.makeStream()
                let progressGate = PuraPiRuntimeProgressGate()
                ...
                self.discoveredNodeState = installation.nodeURL.map {
                    .available(PuraPiRuntimeNodeInfo(
                        nodeURL: $0,
                        npmURL: nil,
                        version: PuraPiRuntimePolicy.minimumNodeVersion
                    ))
                ...
                guard let self, self.operationID == id else { return }
                self.installPhase = nil
                if Task.isCancelled || (error as? PuraPiRuntimeProvisioningError) == .cancelled {
                    self.availability = .failed(.cancelled)
                } else if let provisioningError = error as? PuraPiRuntimeProvisioningError {
    ...
    func openPiWebsite() {
        NSWorkspace.shared.open(PuraPiRuntimePolicy.piWebsiteURL)
    }

    func openNodeDownloadPage() {
        NSWorkspace.shared.open(PuraPiRuntimePolicy.nodeDownloadURL)
    }

    /// 只复制官方 npm 命令，不把它静默执行；适合用户希望在自己的终端审阅命令时使用。
    func copyOfficialInstallCommand() {
        let command = "npm install -g --ignore-scripts \(PuraPiRuntimePolicy.piPackageName)@\(PuraPiRuntimePolicy.managedPiVersion)"
        NSPasteboard.general.clearContents()
        NSPasteboard.general.setString(command, forType: .string)
    }
}

private final class PuraPiRuntimeProgressGate: @unchecked Sendable {
