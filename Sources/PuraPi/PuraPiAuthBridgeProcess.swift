...
protocol PuraPiAuthBridgeClient: Sendable {
    func perform(
        configuration: PuraPiAuthBridgeConfiguration,
        request: PuraPiAuthBridgeRequest,
        onPrompt: @escaping @Sendable (PuraPiAuthPrompt) async throws -> String,
        onEvent: @escaping @Sendable (PuraPiAuthEvent) async -> Void
    ) async throws -> PuraPiAuthBridgeResult
}

enum PuraPiAuthBridgeNodeValidator {
    static func validatedConfiguration(
        _ configuration: PuraPiAuthBridgeConfiguration,
        runner: any PuraPiProcessRunner = PuraPiDefaultProcessRunner()
    ) async throws -> PuraPiAuthBridgeConfiguration {
        var foundVersion: PuraPiRuntimeVersion?
        for candidate in configuration.nodeCandidates {
            guard PuraPiAuthFileSystem.isRegularFile(at: candidate),
            ...
            do {
                ...
                guard version >= PuraPiRuntimePolicy.minimumNodeVersion else { continue }
                ...
                return PuraPiAuthBridgeConfiguration(
                ...
                throw PuraPiAuthError.cancelled
        ...
        if let foundVersion {
            throw PuraPiAuthError.unavailable(
                "认证需要 Node.js \(PuraPiRuntimePolicy.minimumNodeVersion) 或更高版本，当前为 \(foundVersion)。"
            )
        }
        throw PuraPiAuthError.unavailable("无法读取认证所需的 Node.js 版本。")
    }
...
struct PuraPiDefaultAuthBridgeClient: PuraPiAuthBridgeClient {
    func perform(
        configuration: PuraPiAuthBridgeConfiguration,
        request: PuraPiAuthBridgeRequest,
        onPrompt: @escaping @Sendable (PuraPiAuthPrompt) async throws -> String,
        ...
        return try await withThrowingTaskGroup(of: PuraPiAuthBridgeResult.self) { group in
            group.addTask {
            ...
            group.addTask {
                ...
                throw PuraPiAuthError.timedOut
            }
    ...
    private func performWithoutTimeout(
        ...
        // 在发现权限问题前接触凭据文件。
        try PuraPiAuthStorageSecurity.ensureUserOnlyPermissions(at: configuration.authPath)
        let validatedConfiguration = try await PuraPiAuthBridgeNodeValidator.validatedConfiguration(configuration)
        let process = PuraPiAuthBridgeProcess(
                ...
                for try await message in messages {
                    ...
                    case "ready":
                        guard message.protocolVersion == 1 else {
                            throw PuraPiAuthError.invalidMessage("认证桥接协议版本不受支持。")
                        }
                        continue
                    ...
                    case "auth_event":
                        guard message.operationId == request.id,
                            ...
                            throw PuraPiAuthError.invalidMessage("认证事件字段无效。")
                        }
                        await onEvent(event)
                    ...
                    case "prompt":
                        guard message.operationId == request.id,
                            ...
                            throw PuraPiAuthError.invalidMessage("认证输入请求缺少或超出限制。")
                        }
                        let value = try await onPrompt(prompt)
                        try await process.send(PuraPiAuthBridgePromptResponse(
                            id: prompt.id,
                            value: value
                        ))
                    ...
                    case "result":
                        guard message.id == request.id,
                            ...
                            throw PuraPiAuthError.invalidMessage("认证结果缺少完整状态快照。")
                        }
                        await process.stop()
                        return result
                    ...
                    case "error":
                        guard message.id == request.id else {
                            throw PuraPiAuthError.invalidMessage("认证错误的 request id 不匹配。")
                        }
                        let errorMessage = PuraPiSensitiveText.redacted(
                        ...
                        throw PuraPiAuthError.requestFailed(
                            errorMessage,
                    ...
                    default:
                        throw PuraPiAuthError.invalidMessage(
                            "未知消息类型：\(PuraPiSensitiveText.redacted(message.type, limit: 80))"
                        )
                    }
                }
                let diagnostic = await process.diagnostic()
                throw PuraPiAuthError.launchFailed(
                    diagnostic.isEmpty ? "认证桥接提前退出。" : diagnostic
                )
            } catch is CancellationError {
                await process.stop()
                throw PuraPiAuthError.cancelled
            } catch let error as PuraPiAuthError {
                await process.stop()
                throw error
            } catch {
                await process.stop()
                if Task.isCancelled { throw PuraPiAuthError.cancelled }
                throw PuraPiAuthError.launchFailed(
                    PuraPiSensitiveText.redacted(error.localizedDescription)
                )
            }
        }, onCancel: {
            process.cancel()
...
private final class PuraPiAuthPendingWrite: @unchecked Sendable {
    let id: UUID
    let data: Data
...
final class PuraPiAuthBridgeProcess: @unchecked Sendable {
    private static let maximumRecordBytes = 1024 * 1024

    private let configuration: PuraPiAuthBridgeConfiguration
    private let readOnlyAuth: Bool
    private let stateQueue = DispatchQueue(label: "PuraPi.AuthBridgeProcess.state")
    private let writeQueue = DispatchQueue(label: "PuraPi.AuthBridgeProcess.write")
    ...
    private var continuation: AsyncThrowingStream<PuraPiAuthBridgeMessage, Error>.Continuation?
    ...
    private var pendingWrites: [UUID: PuraPiAuthPendingWrite] = [:]
    private var activeWriteID: UUID?
    private var cancelledWriteIDs: Set<UUID> = []
    private var lifecycleID = UUID()

    init(configuration: PuraPiAuthBridgeConfiguration, readOnlyAuth: Bool = false) {
        self.configuration = configuration
        self.readOnlyAuth = readOnlyAuth
        PuraPiAuthBridgeProcessRegistry.shared.insert(self)
    }

    deinit {
        PuraPiAuthBridgeProcessRegistry.shared.remove(self)
    }

    /// 应用退出时使用；不等待异步 Task，直接杀掉当前 sidecar 并清理管道。
    static func terminateAllImmediately() {
        PuraPiAuthBridgeProcessRegistry.shared.terminateAll()
    }

    func start() throws -> AsyncThrowingStream<PuraPiAuthBridgeMessage, Error> {
        let stream = AsyncThrowingStream<PuraPiAuthBridgeMessage, Error> { continuation in
                        ...
                        do {
                            ...
                            self.stateQueue.sync {
                                ...
                                self.finishWriteLocked(
                                    pending,
                                    error: PuraPiAuthError.launchFailed(
                                        "无法向认证 sidecar 写入请求。"
                                    )
                                )
                            }
    ...
    private func launchLocked() throws {
        guard PuraPiAuthFileSystem.isRegularFile(at: configuration.nodeURL),
              FileManager.default.isExecutableFile(atPath: configuration.nodeURL.path)
        else {
            throw PuraPiAuthError.launchFailed("找不到 Node.js：\(configuration.nodeURL.path)")
        }
        guard PuraPiAuthFileSystem.isRegularFile(at: configuration.entryURL),
              FileManager.default.isReadableFile(atPath: configuration.entryURL.path)
        else {
            throw PuraPiAuthError.launchFailed("找不到 Pi 官方 SDK：\(configuration.entryURL.path)")
        }
        guard let bridgeURL = PuraPiAuthBridgeResources.scriptURL,
              FileManager.default.isReadableFile(atPath: bridgeURL.path) else {
            throw PuraPiAuthError.launchFailed("认证桥接脚本不可读。")
        }
        ...
        let cacheDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("PuraPiAuth-\(UUID().uuidString)", isDirectory: true)
        do {
            ...
            throw PuraPiAuthError.launchFailed("无法创建认证缓存目录。")
        }
    ...
    private func installReadSource(
        ...
        source.setEventHandler { [weak self] in
            ...
            if count > 0 {
                ...
                if isStdout {
                    self.stdoutSource?.cancel()
                    self.failLocked(PuraPiAuthError.invalidMessage("无法读取认证 sidecar 输出。"))
                } else {
                    self.stderrSource?.cancel()
                }
            }
    ...
    private func consumeStdoutLocked(_ data: Data) {
        ...
        guard stdoutBuffer.count <= Self.maximumRecordBytes || stdoutBuffer.contains(0x0A) else {
            failLocked(PuraPiAuthError.invalidMessage("认证消息超过大小限制。"))
            return
        }
        ...
        while let newline = stdoutBuffer.firstIndex(of: 0x0A) {
            ...
            do {
                let message = try JSONDecoder().decode(
                    PuraPiAuthBridgeMessage.self,
                    from: payload
                )
                continuation?.yield(message)
            } catch {
                failLocked(PuraPiAuthError.invalidMessage(
    ...
    private func stdoutDidEndLocked() {
        ...
        if !stdoutBuffer.isEmpty {
            failLocked(PuraPiAuthError.invalidMessage("认证 sidecar 输出没有完整换行记录。"))
            return
        }
        finishStreamLocked()
    }
    ...
    private func finishWriteLocked(_ pending: PuraPiAuthPendingWrite, error: Error?) {
        ...
        if let error {
            ...
            pending.resume(throwing: PuraPiAuthError.cancelled)
        } else {
            pending.resume()
        }
    }
...
private final class PuraPiAuthBridgeProcessRegistry: @unchecked Sendable {
    static let shared = PuraPiAuthBridgeProcessRegistry()

    private let lock = NSLock()
    private var processes: [ObjectIdentifier: PuraPiAuthBridgeProcess] = [:]

    func insert(_ process: PuraPiAuthBridgeProcess) {
        lock.lock()
        processes[ObjectIdentifier(process)] = process
        lock.unlock()
    }

    func remove(_ process: PuraPiAuthBridgeProcess) {
        lock.lock()
