...
struct PuraPiNativeWorkspaceSplitView: NSViewControllerRepresentable {
    ...
    func makeNSViewController(context: Context) -> PuraPiWorkspaceSplitViewController {
    ...
    func updateNSViewController(
        _ nsViewController: PuraPiWorkspaceSplitViewController,
        context: Context
    ) {
    ...
    /// NSViewControllerRepresentable，不含任何 PuraPi 代码）实测到两种表现：
    ///
    ...
    /// sidebar 纹丝不动）。对照实验见 `PuraPiHostingSizingContractTests`。
    func sizeThatFits(
        _ proposal: ProposedViewSize,
        nsViewController: PuraPiWorkspaceSplitViewController,
        context: Context
    ) -> CGSize? {
