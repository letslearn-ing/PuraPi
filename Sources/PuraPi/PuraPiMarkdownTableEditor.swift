...
@MainActor
struct PuraPiMarkdownTableEditor: View {
    @Environment(\.puraPiTheme) private var theme
    @State private var table: PuraPiMarkdownTableModel
    @FocusState private var focusedCell: PuraPiMarkdownTableModel.CellID?
    ...
    init(
        ...
        self.onUndo = onUndo
        self.onRedo = onRedo
        _table = State(initialValue: PuraPiMarkdownTableModel(source: block.source))
    }
    ...
    var body: some View {
        ...
        .onChange(of: block.source) { _, newSource in
            let updated = PuraPiMarkdownTableModel(source: newSource)
            if updated != table {
                table = updated
            }
        }
    ...
    private var toolbar: some View {
        HStack(spacing: 5) {
            ...
            Button {
                table.appendRow()
                publishChange()
                focusedCell = PuraPiMarkdownTableModel.CellID(
    ...
    private func cell(row: Int, column: Int, isHeader: Bool) -> some View {
        let cellID = PuraPiMarkdownTableModel.CellID(row: row, column: column)
        return PuraPiMarkdownTableCellEditor(
    ...
    private func move(
        from cell: PuraPiMarkdownTableModel.CellID,
        forward: Bool
    ) {
        ...
        focusedCell = PuraPiMarkdownTableModel.CellID(row: row, column: column)
        onFocus()
    }
