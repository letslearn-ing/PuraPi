...
struct PuraPiMarkdownBlock: Identifiable, Equatable, Sendable {
...
enum PuraPiMarkdownParser {
    static func parse(_ source: String) -> [PuraPiMarkdownBlock] {
        let normalized = source.replacingOccurrences(of: "\r\n", with: "\n")
            .replacingOccurrences(of: "\r", with: "\n")
        let lines = normalized.split(separator: "\n", omittingEmptySubsequences: false).map(String.init)

        var blocks: [PuraPiMarkdownBlock] = []
        ...
        func appendBlock(_ kind: PuraPiMarkdownBlock.Kind) {
            blocks.append(PuraPiMarkdownBlock(id: blocks.count, kind: kind))
        }
...
struct PuraPiMarkdownMessageView: View {
    let text: String
    let isStreaming: Bool
    let streamingText: String?

    @State private var renderResult: PuraPiMarkdownRenderResult?
    ...
    var body: some View {
        Group {
            if isStreaming {
                PuraPiIncrementalMarkdownView(text: streamingText ?? text)
            } else if renderedSource == text, let renderResult {
                if let fallbackBlocks = renderResult.fallbackBlocks {
                    PuraPiMarkdownFallbackView(blocks: fallbackBlocks)
                } else {
                    PuraPiIncrementalMarkdownView(text: text)
                }
                ...
                PuraPiMarkdownPendingView(text: text)
            }
        }
        .task(id: isStreaming ? "streaming" : text) {
            ...
            let result = await Task.detached(priority: .utility) {
                PuraPiMarkdownRenderer.render(source)
            }.value
...
struct PuraPiMarkdownRenderResult: Sendable {
    let attributed: AttributedString?
    let fallbackBlocks: [PuraPiMarkdownBlock]?
}

private struct PuraPiMarkdownPendingView: View {
    let text: String
...
enum PuraPiMarkdownRenderer {
    static func render(_ source: String) -> PuraPiMarkdownRenderResult {
        // 完成态与流式期间必须使用同一套块级排版。
        //
        ...
        //
        // 行内富文本（加粗、链接、行内代码）仍由
        // `PuraPiMarkdownInlineRenderer` 在块内部用 AttributedString 处理。
        PuraPiMarkdownRenderResult(
            attributed: nil,
            fallbackBlocks: PuraPiMarkdownParser.parse(source)
        )
    }
...
private struct PuraPiMarkdownFallbackView: View {
