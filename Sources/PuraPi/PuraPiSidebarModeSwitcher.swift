...
/// 否则圆角与直边的转折处会出现可见硬边。
enum PuraPiTitlebarButtonMetrics {
...
@MainActor
struct PuraPiSidebarModeSwitcher: View {
    /// 两个底板加 2pt 间距。
    static let width: CGFloat = PuraPiTitlebarButtonMetrics.plateWidth * 2 + 2
    static let height: CGFloat = PuraPiTitlebarButtonMetrics.plateHeight
    ...
    var body: some View {
        HStack(spacing: 2) {
            ForEach(PuraPiSidebarMode.allCases) { candidate in
                PuraPiTitlebarIconButton(
...
@MainActor
struct PuraPiSidebarToggle: View {
    static let width = PuraPiTitlebarButtonMetrics.plateWidth
    static let height = PuraPiTitlebarButtonMetrics.plateHeight
...
@MainActor
struct PuraPiTitlebarIconButton: View {
    ...
    var body: some View {
        Button(action: action) {
            Image(systemName: systemImage)
                ...
                .frame(
                    width: PuraPiTitlebarButtonMetrics.plateWidth,
                    height: PuraPiTitlebarButtonMetrics.plateHeight
                )
                // hover/选中底板必须与图标按钮共用同一个连续圆角形状，
                // 并显式裁剪：只给 background 填色时，父级布局仍可能让底色
                // 溢出到形状之外，视觉上就成了方角。
                .background(
                    PuraPiTitlebarButtonMetrics.shape
                        .fill(background)
                )
                .clipShape(PuraPiTitlebarButtonMetrics.shape)
                .contentShape(PuraPiTitlebarButtonMetrics.shape)
        }
        .buttonStyle(.plain)
