...
extension PiSessionController {
    ...
    func executePuraPiCommand(
        _ action: PuraPiCommandAction,
        text: String,
        commandItemID: UUID
    ) {
        switch action {
        ...
        case .compact:
            compactSession(
                customInstructions: PuraPiCommandCatalog.customInstructions(from: text),
        ...
        case .trustProject:
            ...
            guard PuraPiProjectAuthorization.requiresAuthorization(for: workspace.rootURL) else {
                updateConversation(id: commandItemID) { item in
