...
enum PuraPiMarkdownSubmissionChange {
    case none
    case ready([PuraPiMarkdownDiffContext])
    case needsReview
}

private enum PuraPiMarkdownDiffTaskResult {
    case success(PuraPiMarkdownDiffContext)
    case failure(PuraPiMarkdownDiffBuildFailure)
    case cancelled
}

extension PiSessionController {
    func currentMarkdownSnapshot() -> PuraPiMarkdownChangeSnapshot? {
        markdownEditor.pendingCollaborationSnapshot
    }

    func currentMarkdownSnapshot(for url: URL) -> PuraPiMarkdownChangeSnapshot? {
    ...
    private func allMarkdownSnapshots() -> [URL: PuraPiMarkdownChangeSnapshot] {
    ...
    private func snapshotMatchesCurrentState(_ snapshot: PuraPiMarkdownChangeSnapshot) -> Bool {
    ...
    var markdownCollaborationDisplayState: PuraPiMarkdownCollaborationDisplayState {
        ...
        let items = urls.sorted { $0.path < $1.path }.compactMap { url -> PuraPiMarkdownCollaborationItem? in
            ...
            return PuraPiMarkdownCollaborationItem(
                id: url,
        ...
        return PuraPiMarkdownCollaborationDisplayState(
            items: items,
    ...
    func presentMarkdownDiffReview(for requestedURL: URL? = nil) {
        ...
        let cancellation = PuraPiMarkdownDiffCancellationToken()
        markdownDiffCancellationToken = cancellation
        let worker = Task.detached(priority: .userInitiated) {
            () -> PuraPiMarkdownDiffTaskResult in
            do {
                if validateDisk {
                    try cancellation.checkCancellation()
                    try PuraPiMarkdownDiffBuilder.validateSavedSnapshot(
                        snapshot,
                        workspaceRoot: workspaceRoot
                    )
                }
                return .success(try PuraPiMarkdownDiffBuilder.build(
                    snapshot: snapshot,
                    workspaceRoot: workspaceRoot,
        ...
        markdownDiffBuildTask = Task { [weak self] in
            ...
            self.markdownDiffReview = PuraPiMarkdownDiffReview(
                context: context,
    ...
    func approveMarkdownDiffReview(id: UUID) {
        ...
        let approved = PuraPiMarkdownApprovedChange(
    ...
    func canSendExplicitMarkdownChange(_ context: PuraPiMarkdownDiffContext) -> Bool {
        canSendExplicitMarkdownChanges([context])
    }

    func canSendExplicitMarkdownChanges(
        _ contexts: [PuraPiMarkdownDiffContext],
    ...
    func markdownChangeForNextPrompt() -> PuraPiMarkdownSubmissionChange {
        _ = markdownEditor.refreshExternalConflict()
        captureCurrentMarkdownCollaboration()
        ...
        var contexts: [PuraPiMarkdownDiffContext] = []
    ...
    func consumeApprovedMarkdownDiff(for contexts: [PuraPiMarkdownDiffContext]? = nil) {
