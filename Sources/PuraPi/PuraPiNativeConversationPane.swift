...
@MainActor
struct PuraPiNativeConversationSearchOverlayView: View {
    @ObservedObject var state: PuraPiConversationSearchState
    ...
    var body: some View {
        Group {
            if state.isPresented {
                PuraPiConversationSearchBar(state: state, language: language)
                    .padding(.top, 8)
...
@available(macOS 26.0, *)
@MainActor
final class PuraPiNativeConversationPaneController: NSViewController {
    private let session: PiSessionController
    private var isFileSelected: Bool
    private var language: PuraPiInterfaceLanguage
    private var theme: PuraPiTheme
    private var leadingContentInset: CGFloat
    private let viewport = PuraPiConversationViewportView()
    private let searchState: PuraPiConversationSearchState
    private let bottomController: NSHostingController<PuraPiConversationHostView>
    private let searchController: NSHostingController<PuraPiNativeConversationSearchOverlayView>
    ...
    init(
        ...
        let searchState = PuraPiConversationSearchState()
        self.searchState = searchState
        bottomController = NSHostingController(
            rootView: PuraPiConversationHostView(
                session: session,
        ...
        searchController = NSHostingController(
            rootView: PuraPiNativeConversationSearchOverlayView(
                state: searchState,
                language: language,
                theme: theme
            )
    ...
    func update(
        ...
        if rootNeedsUpdate {
            bottomController.rootView = PuraPiConversationHostView(
                session: session,
                isFileSelected: isFileSelected,
                language: language,
                theme: theme,
        ...
        if languageChanged || themeChanged {
            searchController.rootView = PuraPiNativeConversationSearchOverlayView(
                state: searchState,
                language: language,
                theme: theme
            )
