...
/// PuraPi 自己承载的项目授权面板；授权只影响当前项目标签启动的 Pi Runtime。
struct PuraPiProjectAuthorizationView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            Text(isEnglish
                 ? "Allowing it lets Pi load and execute project-local resources. PuraPi will not modify Pi's global trust file; it will only launch this project's Runtime with explicit approval."
                 : "允许后，Pi 才能加载并执行项目本地资源。PuraPi 不会修改 Pi 的全局 trust 文件，只会在明确批准后以授权参数启动当前项目的 Runtime。")
                .font(.system(size: 12.5))
                .foregroundStyle(.secondary)
