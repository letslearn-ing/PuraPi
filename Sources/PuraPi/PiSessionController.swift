...
final class PiSessionController: ObservableObject {
    ...
    /// 待执行的任务队列。由 PuraPi 维护，以便支持取消。
    @Published var queuedPrompts: [PuraPiQueuedPrompt] = []
    /// 当前项目的会话列表，来自磁盘扫描。
    ...
    /// 待随下一条消息发送的附件。
    @Published var pendingAttachments: [PuraPiAttachment] = []
    /// `/status` 查询到的会话统计；nil 表示当前没有可显示的统计结果。
    @Published var sessionStats: PiSessionStats?
    /// `/status` 的一次性 Runtime 状态区块；不放入常驻 HUD。
    @Published var runtimeStatusSnapshot: PuraPiRuntimeStatusSnapshot?
    ...
    @Published var turnRecords: [PuraPiTurnRecord] = []
    @Published var retryWaitState: PuraPiRetryWaitState?
    ...
    let markdownEditor = PuraPiMarkdownEditorState()
    /// Markdown 差异审阅 sheet 当前显示的快照；不会自动打开或自动发送。
    @Published var markdownDiffReview: PuraPiMarkdownDiffReview?
    ...
    /// 丢掉前一个文件的协作差异。
    @Published var pendingMarkdownSnapshots: [URL: PuraPiMarkdownChangeSnapshot] = [:]
    /// 用户已经审阅并选择“附加到下一条消息”的多个文件差异。
    @Published var approvedMarkdownChanges: [URL: PuraPiMarkdownApprovedChange] = [:]
    ...
    @Published var projectAuthorizationState: PuraPiProjectAuthorizationState = .notRequired
    ...
    @Published var extensionNotifications: [PuraPiExtensionNotification] = []
    @Published var extensionStatuses: [String: String] = [:]
    @Published var extensionWidgets: [String: PuraPiExtensionWidget] = [:]
    ...
    var activePromptAttachments: [PuraPiAttachment] = []
    /// 从本地队列取出、但尚未得到 Runtime 接受确认的任务。
    var activeQueuedPrompt: PuraPiQueuedPrompt?
    var generation = UUID()
    /// Pi 的事件没有独立的回合 id；PuraPi 用本地 token 保护异步超时和收束任务。
    ...
    var runtimeRequests: [String: PuraPiRuntimeRequest] = [:]
    ...
    var approvedInspectorChange: PuraPiMarkdownApprovedChange? {
        get {
    ...
    var activePromptMarkdownChanges: [PuraPiMarkdownDiffContext] = []
    var activePromptInspectorChange: PuraPiMarkdownDiffContext? {
    ...
    var pendingInspectorChangeSnapshot: PuraPiMarkdownChangeSnapshot? {
    ...
    var markdownDiffCancellationToken: PuraPiMarkdownDiffCancellationToken?
    ...
    var issuedMarkdownApprovalTokens: [UUID: PuraPiMarkdownApprovalFingerprint] = [:]
    ...
    func approveProjectAuthorization(remember: Bool = false) {
        ...
        if remember {
            PuraPiProjectAuthorization.remember(workspace.rootURL)
        }
    ...
    func denyProjectAuthorization() {
        ...
        lastError = "当前项目包含本地 Extension 或配置；未授权时 PuraPi 不会加载它们。"
    }

    func requestProjectAuthorization() {
        guard PuraPiProjectAuthorization.requiresAuthorization(for: workspace?.rootURL ?? URL(fileURLWithPath: "/")) else {
            return
        }
        stopRuntimeForAuthorization()
