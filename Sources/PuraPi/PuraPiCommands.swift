...
/// PuraPi 当前支持的内置命令动作。
enum PuraPiCommandAction: Equatable, Sendable {
    case newSession
    case continueRecent
    case compact
    case abort
...
enum PuraPiCommandSelectionDirection: Sendable {
    case up
    case down
}

/// 命令选择器展示的一条命令。
struct PuraPiCommandItem: Identifiable, Equatable, Sendable {
    let name: String
    let description: String
    let source: String
    let action: PuraPiCommandAction?
...
enum PuraPiCommandCatalog {
    ...
    static func action(for text: String) -> PuraPiCommandAction? {
    ...
    static func movedSelection(
        currentIndex: Int,
        direction: PuraPiCommandSelectionDirection,
        resultCount: Int
    ) -> Int {
    ...
    private static func builtInItems(language: PuraPiInterfaceLanguage) -> [PuraPiCommandItem] {
...
/// 项目本地资源的安全授权状态。
enum PuraPiProjectAuthorizationState: Equatable, Sendable {
    case notRequired
    case needsDecision
    case approved
    case denied
...
enum PuraPiProjectAuthorization {
    static let rememberedProjectsKey = PuraPiPreferences.Key.rememberedAuthorizedProjects
