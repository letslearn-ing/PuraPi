...
@MainActor
final class PuraPiWorkspaceSplitViewController: NSSplitViewController {
    ...
    var language: PuraPiInterfaceLanguage
    var theme: PuraPiTheme
    var sidebarTint: PuraPiPaneTint
    var inspectorTint: PuraPiPaneTint
    ...
    var sidebarController: NSHostingController<PuraPiNativeSidebarHostView>!
    var conversationController: PuraPiNativeConversationPaneController!
    var sidebarItem: NSSplitViewItem!
    var workspaceItem: NSSplitViewItem!
    var inspectorController: NSHostingController<PuraPiInspectorHostView>?
    ...
    weak var dividerWindowDragBlocker: PuraPiDividerWindowDragBlocker?
    /// 覆盖 AppKit 在旧 pane divider 位置绘制的圆形 grabber，不参与事件。
    weak var dividerVisualMask: PuraPiDividerVisualMask?
    /// 位于 splitView 父视图上的 Inspector 标题按钮命中层；不覆盖正文滚动区域。
    weak var inspectorHeaderInteractionView: PuraPiInspectorHeaderInteractionView?
    /// 独立/挂回按钮使用单独的命中层，以保持原有三按钮代理的几何契约。
    weak var inspectorDetachInteractionView: PuraPiInspectorDetachInteractionView?
    ...
    init(
        ...
        sidebarTint: PuraPiPaneTint = .purple,
    ...
    override func viewDidLoad() {
        ...
        sidebarController = NSHostingController(
            rootView: PuraPiNativeSidebarHostView(
                session: session,
        ...
        sidebarItem.minimumThickness = PuraPiLayoutState.minimumSidebarWidth + sidebarSlotPadding
        sidebarItem.maximumThickness = PuraPiLayoutState.maximumSidebarWidth + sidebarSlotPadding
        ...
        conversationController = PuraPiNativeConversationPaneController(
            session: session,
    ...
    func update(
        ...
        if languageChanged || sidebarTintChanged || themeChanged {
            ...
            sidebarController.rootView = PuraPiNativeSidebarHostView(
                session: session,
                layoutState: layoutState,
                language: language,
                theme: theme,
        ...
        if (inspectorTintChanged || languageChanged || themeChanged), let inspectorController {
            inspectorController.rootView = PuraPiInspectorHostView(
                session: session,
                layoutState: layoutState,
                language: language,
                theme: theme,
    ...
    var diagnosticLayoutState: PuraPiLayoutState { layoutState }
    ...
    var sidebarSlotPadding: CGFloat {
        PuraPiLayoutState.sidebarInset * 2
    }
    ...
    var conversationLeadingContentInset: CGFloat {
        ...
        return PuraPiLayoutState.nativeSidebarCenterLeadingInset(
    ...
    var inspectorSlotPadding: CGFloat {
        PuraPiLayoutState.inspectorInset * 2
    }
    ...
    func installInspectorHeaderInteractionView() {
        ...
        let headerHeight = PuraPiInspectorHeaderMetrics.totalHeight
        let rectInSplit: NSRect
        let surfaceRight = pane.frame.maxX - PuraPiLayoutState.inspectorInset
        ...
        if splitView.isFlipped {
            ...
            rectInSplit = NSRect(
                x: surfaceLeft,
                y: splitView.bounds.minY + PuraPiLayoutState.inspectorInset,
        ...
        let overlay: PuraPiInspectorHeaderInteractionView
        if let existing = inspectorHeaderInteractionView,
           existing.superview === parent {
            overlay = existing
        } else {
            inspectorHeaderInteractionView?.removeFromSuperview()
            let created = PuraPiInspectorHeaderInteractionView(frame: rectInParent)
        ...
        let detachRectInSplit = NSRect(
            x: rectInSplit.minX
                ...
                - PuraPiInspectorHeaderMetrics.detachSpacing,
            y: rectInSplit.minY,
        ...
        let detachOverlay: PuraPiInspectorDetachInteractionView
        if let existing = inspectorDetachInteractionView,
           existing.superview === parent {
            detachOverlay = existing
        } else {
            inspectorDetachInteractionView?.removeFromSuperview()
            let created = PuraPiInspectorDetachInteractionView(frame: detachRectInParent)
            created.autoresizingMask = []
            parent.addSubview(created, positioned: .above, relativeTo: nil)
    ...
    func installInspector() {
        ...
        let controller = NSHostingController(
            rootView: PuraPiInspectorHostView(
                session: session,
                layoutState: layoutState,
                language: language,
                theme: theme,
        ...
        item.minimumThickness = PuraPiLayoutState.minimumInspectorWidth + inspectorSlotPadding
        item.maximumThickness = PuraPiLayoutState.maximumInspectorWidth + inspectorSlotPadding
