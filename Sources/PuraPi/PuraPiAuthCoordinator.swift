...
@MainActor
final class PuraPiAuthCoordinator: ObservableObject {
    @Published private(set) var snapshot: PuraPiAuthSnapshot = .empty
    @Published private(set) var phase: PuraPiAuthPhase = .idle
    @Published private(set) var currentEvent: PuraPiAuthEvent?
    @Published private(set) var pendingPrompt: PuraPiAuthPrompt?
    ...
    private let selection: PuraPiRuntimeSelection
    private let locations: PuraPiRuntimeLocations
    private let homeDirectory: URL
    /// 生产环境不缓存完整环境字典，避免把 ambient API Key 长期留在 Swift 状态。
    private let environmentOverride: [String: String]?
    private let bridgeClient: any PuraPiAuthBridgeClient
    private let configurationOverride: PuraPiAuthBridgeConfiguration?
    ...
    init(
        selection: PuraPiRuntimeSelection = PuraPiRuntimeSelection(),
        locations: PuraPiRuntimeLocations = PuraPiRuntimeLocations(),
        homeDirectory: URL = FileManager.default.homeDirectoryForCurrentUser,
        environment: [String: String]? = nil,
        bridgeClient: (any PuraPiAuthBridgeClient)? = nil,
        configurationOverride: PuraPiAuthBridgeConfiguration? = nil,
        ...
        self.bridgeClient = bridgeClient ?? PuraPiDefaultAuthBridgeClient()
    ...
    var featuredProviders: [PuraPiAuthProvider] {
        snapshot.providers
    ...
    var otherProviders: [PuraPiAuthProvider] {
        snapshot.providers
    ...
    var orphanedCredentials: [PuraPiStoredCredential] {
    ...
    func refreshStatus() {
        start(
            phase: .checking,
            request: PuraPiAuthBridgeRequest.status(id: UUID().uuidString),
    ...
    func validateCredentials() {
        start(
            phase: .checking,
            request: PuraPiAuthBridgeRequest.validate(id: UUID().uuidString),
            changedProviderID: nil,
            changedAuthType: nil
        )
    }
    ...
    func refreshAvailableModels() {
        start(
            phase: .refreshing,
            request: PuraPiAuthBridgeRequest.refresh(
                id: UUID().uuidString,
                allowNetwork: true,
                force: true
            ),
    ...
    func login(providerID: String, type: PuraPiAuthType) {
        ...
        start(
            ...
            request: PuraPiAuthBridgeRequest.login(
                id: UUID().uuidString,
                provider: providerID,
                type: type
            ),
    ...
    func logout(providerID: String) {
        ...
        start(
            ...
            request: PuraPiAuthBridgeRequest.logout(
                id: UUID().uuidString,
                provider: providerID
            ),
            changedProviderID: providerID,
    ...
    func cancelPendingPrompt(for expectedPromptID: String? = nil) {
        ...
        pendingPrompt = nil
        promptValidationMessage = nil
        continuation.resume(throwing: PuraPiAuthError.cancelled)
    }
    ...
    private func start(
        phase newPhase: PuraPiAuthPhase,
        request: PuraPiAuthBridgeRequest,
        changedProviderID: String?,
        changedAuthType: PuraPiAuthType?
    ) {
        guard operationTask == nil else { return }
            ...
            do {
                let result = try await client.perform(
                    ...
                    onPrompt: { [weak self] prompt in
                        guard let self else { throw PuraPiAuthError.cancelled }
                ...
                guard !Task.isCancelled else { throw PuraPiAuthError.cancelled }
                await MainActor.run {
                    self?.finish(
                        operationID: id,
                        result: result,
    ...
    private func makeConfiguration() -> PuraPiAuthBridgeConfiguration? {
        ...
        switch PuraPiAuthBridgeLocator.locate(
    ...
    private func waitForPrompt(_ prompt: PuraPiAuthPrompt) async throws -> String {
        try await withTaskCancellationHandler(operation: {
            try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<String, Error>) in
                if promptContinuation != nil {
                    ...
                    cancelPendingPrompt()
                    continuation.resume(throwing: PuraPiAuthError.invalidMessage("同时收到多个认证输入请求。"))
                    return
                }
                promptID = prompt.id
    ...
    private func receive(event: PuraPiAuthEvent, operationID: UUID) {
    ...
    private func finish(
        operationID: UUID,
        result: PuraPiAuthBridgeResult,
        changedProviderID: String?,
        changedAuthType: PuraPiAuthType?
    ) {
        guard self.operationID == operationID else { return }
        if cancelledOperationID == operationID {
            fail(operationID: operationID, error: PuraPiAuthError.cancelled)
            return
        }
        cancelledOperationID = nil
        ...
        refreshWarnings = result.refreshWarnings.map { warning in
            let message = PuraPiSensitiveText.redacted(warning.message)
    ...
    private func changedCredentialProviderIDs(
        from previous: [PuraPiStoredCredential],
        to current: [PuraPiStoredCredential]
    ) -> [String] {
        var previousByProvider: [String: PuraPiAuthType] = [:]
        for credential in previous {
            previousByProvider[credential.providerId] = credential.type
        }
        var currentByProvider: [String: PuraPiAuthType] = [:]
    ...
    private func mayHaveCommittedCredential(after error: Error) -> Bool {
        ...
        guard let authError = error as? PuraPiAuthError else { return true }
        switch authError {
        case .unavailable:
    ...
    private func publishAuthenticationChange(providerIDs: [String]) {
        ...
        NotificationCenter.default.post(
            name: .puraPiAuthenticationChanged,
            object: nil,
            userInfo: userInfo
        )
    }
    ...
    private func fail(operationID: UUID, error: Error) {
        ...
        if error is CancellationError || (error as? PuraPiAuthError) == .cancelled {
            phase = .idle
            ...
            let message = PuraPiSensitiveText.redacted(error.localizedDescription)
            var didPublishMutation = false
            phase = .failed(message)
            operationMessage = message
            if let authError = error as? PuraPiAuthError,
