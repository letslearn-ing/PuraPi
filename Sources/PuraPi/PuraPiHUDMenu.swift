...
/// HUD 下拉项的一条候选。
struct PuraPiHUDMenuOption: Identifiable, Equatable {
    let id: String
    let title: String
    let detail: String
    let isSelected: Bool
...
struct PuraPiHUDMenu: NSViewRepresentable {
    let options: [PuraPiHUDMenuOption]
    let onSelect: (String) -> Void

    func makeNSView(context: Context) -> PuraPiHUDMenuHostView {
        let view = PuraPiHUDMenuHostView()
        view.onSelect = onSelect
        view.options = options
        return view
    }

    func updateNSView(_ nsView: PuraPiHUDMenuHostView, context: Context) {
...
final class PuraPiHUDMenuHostView: NSView {
    var options: [PuraPiHUDMenuOption] = []
